...
| F5 | `edu-ui`: accesibilidad teclado/ARIA (tabs, accordion, menu, tooltip, combobox) + `setDisabledState` faltante en 9 controles CVA | Riesgo | ✅ FE cerrado, awaiting-prod [679](../chats/awaiting-prod/679-audit-f5-edu-ui-accesibilidad-setdisabledstate.md) — hallazgo derivado: bug real de `aria-activedescendant` en `edu-select`/`edu-multi-select` (overlay portado a `body`) encontrado y corregido en vivo |
| F6 | Decidir y aplicar configuración zoneless explícita en `app.config.ts` (paridad con `test-setup.ts`) | Regla violada | ✅ [680](../chats/closed/680-audit-f6-zoneless-config-explicita.md) |
| F7 | Sitio público: formulario de contacto sale de la SPA (Formspree nativo) + meta tags dinámicos por ruta ausentes (SEO) | Bug/Riesgo | ✅ FE cerrado, awaiting-prod [681](../chats/awaiting-prod/681-audit-f7-sitio-publico-contacto-seo.md) — pendiente merge de brief backend 681-B (`chat/681-B-endpoint-contacto` en Educa.API, cerrado pero no mergeado a su `main`) |
| F8 | Performance: `@defer` ausente transversal + bug de lazy-loading barrel en `intranet.routes.ts` (profesor/estudiante) + leak Three.js campus 3D + `monitoreo-hub-badges` 10 llamadas paralelas | Performance | ✅ [682](../chats/closed/682-audit-f8-performance-defer-lazy-loading-leaks.md) — puntos 1/2/3/5/6(parcial) resueltos; punto 4 re-diagnosticado (no eran llamadas duplicadas — requiere endpoint de delta en backend), deuda cross-repo documentada en el brief |
| F9 | Consolidar duplicación de código: mensajería estudiante/profesor, niveles públicos ×3, componentes de asistencia por rol | Observación | ✅ [683](../chats/closed/683-audit-f9-consolidar-duplicacion-codigo.md) — 3 pares consolidados (`MensajeriaPageComponent`, `LevelPageComponent` genérico, `AttendanceDirectorStaffComponent` con `loader` opcional), tests nuevos donde no había cobertura, lint+build+tests verde |
| F10 | Capa de datos: adapters con lógica de negocio filtrada, violación de layering `shared→features`, cast `as never` en `vistas.facade.ts` | Regla violada | ✅ [684](../chats/closed/684-audit-f10-capa-datos-layering-vistas-facade.md) — `resolveModoAsignacion` movido a `data/adapters/academico/`, `CentesimalScale` duplicado eliminado, DTO de justificación movido a `@data/models`, cast `as never` tipado con genéricos |
| F11 | Inconsistencias transversales menores: alias `@env`/`@config`, `CommonModule` sin uso, naming `severity`/`size` en `edu-ui`, `DestroyRef` de servicios root sin efecto real | Inconsistencia | ✅ [685](../chats/closed/685-audit-f11-inconsistencias-transversales-menores.md) — alias `@env` eliminado de `tsconfig.json` (32 archivos consolidados a `@config/environment`); `CommonModule` removido de 154 componentes (113 sin reemplazo, 41 con pipe puntual: `DatePipe`/`DecimalPipe`/`SlicePipe`/`LowerCasePipe`); `EduMessageSeverity`/`EduToastSeverity` unificados a `'danger'` (antes `'error'`), con mapeo explícito en el boundary `ErrorSeverity`→`EduToastSeverity` (`toast-container.component.ts`); `EduButtonSize`/`EduAvatarSize` documentados como escalas intencionalmente distintas; `core/services/destroy/{destroy.service,destroyable.component}.ts` eliminados (dead code confirmado, 0 usos); `rate-limit-events.facade.ts` dejado como está (decisión del usuario: consistente con 10+ facades root del proyecto y con `code-style.md` regla 2). Build+lint+tests verde (2592/2593, 1 timeout flaky no relacionado confirmado en aislamiento)
| F12 | Riesgos menores de seguridad/UX: login rate-limit solo en memoria, `campus-admin` sin `rowVersion`/409, videollamadas sin manejo de expiración de token JaaS | Riesgo | ✅ [686](../chats/closed/686-audit-f12-riesgos-menores-seguridad-ux.md) — punto 1 documentado (BE ya protege login vía rate limiting por IP, 60/min); punto 2 `rowVersion`+409 replicado en `campus-admin` (Piso/Nodo/Bloqueo) siguiendo patrón de `ayuda-tickets`, verificado manualmente con 2 sesiones (2026-09-21); punto 3 aviso+reconexión de expiración JaaS agregado en cliente (BE ya dimensionaba `exp` al fin de la clase + margen, sin handoff a `Educa.API`). Build+lint+tests verde FE+BE. Verificado y cerrado |

## Done-when

- [x] F1-F4 cerrados y verificados.
- [x] F5-F12: briefs 679-686 generados 2026-09-12 a pedido explícito del usuario, superando el soft cap de `chats/open/` ≤5 (excepción documentada en `backlog-hygiene.md` para fases de audit deliberadamente pesadas) — **12/12 implementados**; F5/F7 en `awaiting-prod` pendiente `/verify`, F12 verificado y cerrado 2026-09-21, resto cerrados.
- [x] Plan y maestro sincronizados en cada cierre de fase.
