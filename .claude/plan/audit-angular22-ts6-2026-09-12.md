...
| F12 | Riesgos menores de seguridad/UX: login rate-limit solo en memoria, `campus-admin` sin `rowVersion`/409, videollamadas sin manejo de expiración de token JaaS | Riesgo | ✅ [686](../chats/awaiting-prod/686-audit-f12-riesgos-menores-seguridad-ux.md) — punto 1 documentado (BE ya protege login vía rate limiting por IP, 60/min); punto 2 `rowVersion`+409 replicado en `campus-admin` (Piso/Nodo/Bloqueo) siguiendo patrón de `ayuda-tickets`; punto 3 aviso+reconexión de expiración JaaS agregado en cliente (BE ya dimensionaba `exp` al fin de la clase + margen, sin handoff a `Educa.API`). Build+lint+tests verde FE+BE. `awaiting-prod`, falta QA manual multi-sesión del punto 2 |

## Done-when

- [x] F1-F4 cerrados y verificados.
- [x] F5-F12: briefs 679-686 generados 2026-09-12 a pedido explícito del usuario, superando el soft cap de `chats/open/` ≤5 (excepción documentada en `backlog-hygiene.md` para fases de audit deliberadamente pesadas) — **12/12 implementados**; F5/F7/F12 en `awaiting-prod` pendiente `/verify`, resto cerrados.
