...
>
> **Coordinación cross-plan**: → [`educa-coord/plans/xrepo-asignacion-profesor-salon-curso.md`](../../../educa-coord/plans/xrepo-asignacion-profesor-salon-curso.md) tiene precedencia sobre las Fases 6 (audit `HorarioAsignacionService`) y 7 (Matrícula). El refactor de Fase 6 **debe** consumir los validadores (`TutorPlenoValidator`, `ModoAsignacionResolver`) en lugar de duplicar lógica. Fase 7 Matrícula al asignar estudiante a salón **debe** validar el modo del grado (INV-AS01/02/03).
