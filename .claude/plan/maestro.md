...
1. _(Plan 38 Chat 6 FE cerrado local 2026-05-02 — `awaiting-prod/076` esperando deploy del FE + smoke. BE ya commiteado por Plan 39 Chat B (078). Aporte de este chat: feature flag `emailDeferAlerts`, componente `app-defer-fail-banner` (B9), cableado de `EmailHubService` en `EmailOutboxComponent` con suscripción a los 3 eventos + toast scoped + TTL 5 min para banner tras blacklist creada. 4 tests del banner verdes; 177/177 suite del feature email-outbox.)_
...
4. **[Plan 39 · Chat D · `educa-web` · FE]** — Banner B9 reusable cross-páginas (`/monitoreo/correos/*` 4 sub-páginas) + tile "Aceptado vs Entregado" en auditoría + alineación del listener SignalR de Plan 38 Chat 6 (076) con los 3 eventos del hub. Brief en `.claude/chats/open/080-plan-39-chat-d-fe-banner-b9-cross-paginas-auditoria-tile.md`. **Pre-req**: Chat B (078) + Chat C (079) + Plan 38 Chat 6 (076) deployados.
