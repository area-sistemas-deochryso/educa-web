...
| 22 | Endurecimiento correos de asistencia | BE+FE | `Educa.API/.claude/plan/asistencia-correos-endurecimiento.md` | 🟢 En progreso · Chat 1 (F1) ✅ 2026-04-21 + Chat 2 (F2) ✅ 2026-04-21 — `SmtpErrorClassifier` (enhanced → `FAILED_INVALID_ADDRESS`/`FAILED_MAILBOX_FULL`/`FAILED_REJECTED`/`TRANSIENT`/`FAILED_UNKNOWN`) + columna `EO_TipoFallo` + índice filtrado creados en prueba+prod + política 0/1 por causa en `EmailOutboxWorker.ApplyAsistenciaPolicy` para `Asistencia`/`AsistenciaProfesor` + legacy intacta (5 retries backoff exp) para tipos fuera del alcance + `TestDbContextFactory` compartido (aprendizaje F1) + 13 tests unit classifier + 6 tests integration worker (casos a–f incluida regresión `NotificacionAdmin` + legacy) + tests F1 actualizados (EO_TipoFallo en columna, razón sin prefijo) + 853 tests backend verdes · pendiente Chat 3 (F3.BE — `ErrorLog` fire-and-forget + job Hangfire `ReporteFallosCorreoAsistenciaJob` diario 07:00 AM Lima) | 33% |
...
## 🟢 Plan 25 — Paridad Excel para reportes PDF — RE-ABIERTO 2026-04-21
...
> **Estado**: 🟢 **Re-abierto 2026-04-21**. Chat 1 ✅ + Chat 2 ✅ + Chat 3 ✅ + Chat 4 ✅ + Chat 5A ✅ + Chat 5B ⏳ (5 de 6 chats cerrados, ~95%). Reapertura por feedback visual: los Excel de Chat 2 no seguían el color coding ni la leyenda del PDF, ni usaban nombres de mes en español en los subtítulos. Chat 5A cerró colores + leyenda + nombres de mes. Chat 5B cerrará la división Verano/Regular del reporte anual.
...
- **Chat 5B — BE + FE: división Verano/Regular del reporte anual + espejo PDF de subtítulos** ⏳ **pendiente**. Scope: **(1)** el reporte anual consolidado hoy retorna un único agregado con datos mezclados de Verano (Ene-Feb) y Regular (Mar-Dic), que se evalúan con reglas distintas (INV-C01 umbrales absolutos en regular vs. fórmula inicio+delta en verano). Dividirlo en 2 hojas/secciones separadas. Decisión de diseño pendiente pre-work: ¿query param `periodo=verano|regular|ambos`?, ¿2 hojas automáticas en el xlsx y 2 páginas en el PDF?, ¿agregación independiente por periodo? **(2)** Subtítulos PDF con nombres de mes en español — mismo cambio que Chat 5A hizo en Excel pero en `ReporteAsistenciaConsolidadoPdfService`, `ReporteAsistenciaSalonPdfService`, `ReporteAsistenciaProfesorPdfService`. Cuidado: esos services ya están en 395/320/222 ln respectivamente — puede requerir partial splits. **(3)** Revisar performance de colorear celda por celda en reportes de 500+ filas (medir antes de decidir si vale optimización). **(4)** Smoke test manual de Excel descargado desde Capacitor — verificar que los colores se ven correctamente en móvil. Al cerrar Chat 5B, Plan 25 pasa a ✅ 100%.
