...
2. **[Plan 30b · BE · `Educa.API` · BE]** — agregar al endpoint `GET /api/sistema/asistencia/diagnostico-correos-dia` la lista detallada `EntradasConCorreoEnviado` (simétrica positiva al gap). DTO + JOIN en `DiagnosticoCorreosDiaSnapshotFactory` (filtra `EO_Estado = SENT`, INV-C11 respetado, fail-safe INV-S07), 4 tests nuevos. Brief en `.claude/chats/051-plan-30b-be-entradas-con-correo-enviado.md`. Después: chat FE Plan 30b para espejar DTO + nuevo sub-tab + tabla en el feature `email-outbox-diagnostico`.
