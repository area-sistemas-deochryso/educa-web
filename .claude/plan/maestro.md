...
2. **[Plan 24 · Chat 1 · `Educa.API` · design]** — Sync CrossChex en Background Job (0%, 4 chats diseñados). Promovido al top 3 tras cierre de Plan 22 Chat 6 F4.FE. No requiere sender dedicado (decisión 2026-04-21 en Plan 22 Chat A) — comparte los 7 senders del pool. Es `/design` primero (8 decisiones a validar con usuario) antes de pasar a implementación.
3. **[pendiente]** — el tercer slot queda abierto tras cierre de Plan 22. Candidatos fuera del top 3 que pueden promoverse: Plan 28 Chat 3 BE (bloqueado por validación jefe Plan 27), Plan 26 F3 (espera telemetría ~2 semanas), Design System F5.3 (prioridad más baja). Discutir con el usuario al arrancar Plan 24 qué entra aquí.
