...
#### Plan 22 — Endurecimiento correos de asistencia 🟢 (plan file 2026-04-21 · Chat 1 + Chat 2 cerrados 2026-04-21 · pendiente Chat 3 F3.BE `/execute`)
...
> **Estado**: plan file creado 2026-04-21 con desglose de 6 chats atómicos (4 BE + 2 FE). Secuencia sugerida F1 → F2 → F3.BE → F3.FE → F4.BE → F4.FE. Chat 1 (F1) + Chat 2 (F2) cerrados 2026-04-21.
...
- [ ] **Chat 3 — F3.BE** (BE, 1 chat) — Notificación triple lado backend · helper `EmailFailureLogger` (ErrorLog fire-and-forget) · job Hangfire `ReporteFallosCorreoAsistenciaJob` 07:00 AM Lima · template HTML del resumen · NO vector recursivo
- [ ] **Chat 4 — F3.FE** (FE, 1 chat, repo `educa-web`) — UI admin `/admin/email-outbox` · badge `TipoFallo` + filtro · `disabled` + tooltip en botón "Reintentar" para tipos permanentes
