...
2. **[Plan 28 · Chat 3 · `Educa.API` · BE]** — reportes PDF/Excel + correos + bandeja + notificaciones para `TipoPersona='A'` (Asistente Administrativo). Desbloqueado por Chat 2 BE cerrado 2026-04-22. Requiere esperar validación post-deploy del Plan 27 antes de arrancar (evita PRs simultáneos sobre `AsistenciaPersona` + `EmailNotificationService`).
3. **[Plan 26 · F2 Chat 2 · `Educa.API` · BE]** — aplicar `[RateLimitOverride]` a 14 endpoints de reportes + 2 vistas admin. Desbloqueado (F2 Chat 1 ya cerrado). Bajo riesgo — es solo anotar controllers; luego observar telemetría 1-2 semanas antes de F3.
...
- Frentes abiertos sin bloqueo (Design System F5.3, Carril D Olas 2+) no entran aquí hasta que desplacen prioridad a uno de los 3 ítems.

---

**Foco (actualizado 2026-04-23, post-cierre Plan 29 Chat 4 docs)**: 🟢 **Plan 29 Chat 4 docs ✅ cerrado** (educa-web `main`). `.claude/rules/business-rules.md` gana nueva `§18 Correos Salientes y Protección del Canal SMTP` (5 subsecciones: contexto cPanel, defensas en capas, qué cuenta vs qué no, coordinación con throttle Plan 22, enumeración de invariantes) y `§15.14 Invariantes de Correos Salientes` con los 4 IDs `INV-MAIL01/02/03/04` (wording literal del maestro líneas 275-281, sin parafrasear). `INV-MAIL04` es nuevo en este chat — formaliza el endpoint `defer-fail-status` (Chat 2.6 BE) + widget `DeferFailStatusWidget` (Plan 22 Chat B) + fallback CRITICAL por INV-S07. Checklist final extendido con bloque "CORREOS SALIENTES (Sección 18)" (6 items). Comentarios `<!-- TBD post-OPS -->` sobre el `5/h` en 5 lugares — fácil de encontrar por grep para el swap cuando Chat 3 OPS cierre. Plan 29 fila 29 a ~90% — queda Chat 3 OPS (no es código) + eventual micro-chat de swap si hosting sube el techo. 🟢 **Plan 22 Chat B defer-fail-widget ✅ cerrado 2026-04-23** (+10 tests FE → 1519 verdes; fila 22 a 95%). 🟢 **Plan 29 Chat 2.6 BE ✅ cerrado 2026-04-23** (commit `7b2a962`, +21 tests, 1274 BE verdes). 🟢 **Plan 29 Chat 2.5 BE ✅ cerrado 2026-04-23** (commit `0580983`, +11 tests, 1253 verdes). 🟢 **Plan 29 Chat 2 BE ✅ cerrado 2026-04-22** (commit `674e86a`, +35 tests). 🔴 **Siguiente por prioridad**: Chat 3 OPS del Plan 29 (inspección CrossChex + negociación umbral hosting con el admin cPanel) y luego Plan 28 Chat 3 BE (reportes AA, desbloqueado tras validación Plan 27 post-deploy). Post-deploy Chat 2.6: 48-72h monitoreo 0 `SslHandshakeException` + contador defer/fail cPanel bajando. Señal anotada para chat futuro: retry automático del worker puede ser peligroso con threshold bajo — evaluar migración a retry manual (el widget ya expone `Retrying` prominentemente para facilitar esto). 🟢 **Plan 27 cerrado en docs + código completo** — `INV-C11` formalizado, pendiente solo validación del jefe post-deploy. **Frentes abiertos sin bloqueo**: Plan 26 F2 Chat 2, Design System F5.3, Carril D Olas 2+.
