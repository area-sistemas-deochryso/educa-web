...
**Foco (actualizado 2026-04-22, post-cierre Chat 4)**: 🟢 **Plan 27 Chat 5 `/design` + documentación** — formalizar `INV-C11` en `business-rules.md §15.4`, nueva subsección "Filtro temporal por grado" en §1, plan de reversión, movimiento de chat files a `closed/`. Chat 5 **no cierra** hasta validación del jefe post-deploy. Otros frentes (Plan 22 F4/F5.6 BE, Plan 26 F2 Chat 2, Carril D, Design System F5.3) pueden avanzar en paralelo según capacidad.
...
> **Estado**: 🟢 **Chats 1-4 ✅ cerrados 2026-04-22** — diseño + BE + FE implementados. Chat 4 incluyó BE mínimo (`GraOrden` en DTOs self-service, commit `a967e21`, 1155 verdes) + FE completo (scope constants + banner admin + per-student notice + widget home verificado, commit `3c5061e`, 1507 verdes). Solo queda Chat 5 (documentación formal INV-C11 en `business-rules.md §15.4` + movimiento de chat files a `closed/`). **Chat 5 no cierra hasta validación del jefe post-deploy**.
