...
2. **[Plan 24 · Chat 2 · `Educa.API` · BE]** — SignalR broadcast de progreso del sync CrossChex (reusa `AsistenciaHub`). Promovido a #2 tras cierre de Chat 1 (commit `299db24` en Educa.API master, 2026-04-23). Consume el contrato `CrossChexSyncStatusDto` ya definido — el FE del Chat 3 podrá alternar entre polling (funciona hoy) y push por hub sin romper el shape.
3. **[pendiente]** — el tercer slot sigue abierto. Candidatos fuera del top 3 que pueden promoverse: Plan 28 Chat 3 BE (bloqueado por validación jefe Plan 27), Plan 26 F3 (espera telemetría ~2 semanas), Design System F5.3 (prioridad más baja). Discutir con el usuario al arrancar Plan 24 Chat 2 qué entra aquí.
...
> **Estado**: 🟢 **Chat 1 ✅ cerrado 2026-04-23** (commit `299db24` en `Educa.API master`, +7 tests → 1302 BE verdes). Plan al **~25%**. Faltan Chat 2 (SignalR), Chat 3 (FE progress bar), Chat 4 (validar `Task.Delay`).
