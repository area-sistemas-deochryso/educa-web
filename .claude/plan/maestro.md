...
- **F-022 (brief `200`) — AA self-service regresión `/intranet/asistencia`** 🟠 Cowork 2026-05-19. Reconcilia Plan 28 Chat 4a con Plan 43. Severidad Alto. FE puro, sin BE. Investigar primero qué componente AA tenía antes del brief 143.
...
- 🟠 **F-022 FE pendiente** (brief `200` en `chats/open/`) — AA YARUPAITA pierde vista self-service en `/intranet/asistencia` (regresión del brief 134 parte 4a). Ve panel admin completo (tabs Estudiantes/Profesores/AA + filtro INV-C11 + dropdowns) cuando debería ver self-service. Causa probable: el unificador del brief 143 expandió el componente cross-role a los 4 roles administrativos sin preservar la rama AA. Severidad Alto. Fix: branch por rol — `Director/Promotor/Coord.Académico` → panel admin; `AA` → `AaSelfServiceComponent`. Reconciliar Plan 28 Chat 4a con Plan 43 vista uniforme.
