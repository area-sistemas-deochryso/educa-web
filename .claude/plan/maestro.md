...
2. **[Plan 22 · Chat 5 · `Educa.API` · BE]** — F4.BE endpoint `GET /api/sistema/auditoria-correos-asistencia` para auditoría preventiva de correos con formato inválido (reusa `EmailValidator` de F1). Repository + service + controller + authorization tests. **Pre-work obligatorio (regla DB-SELECT-first)**: mostrar queries de inspección sobre `Estudiante.EST_CorreoApoderado` + `Profesor.PRO_Correo` antes de codificar. Prioridad alta confirmada 2026-04-23 (auditoría preventiva sigue siendo necesaria aún post Plan 29 porque INV-MAIL01 solo rechaza silenciosamente; el endpoint los expone al admin para corregir en la fuente).
3. **[Plan 22 · Chat 6 · `educa-web` · FE]** — F4.FE pantalla `/intranet/admin/auditoria-correos` con feature flag `auditoriaCorreos` + menú Sistema → Monitoreo. Acceso: Director + Asistente Administrativo (sin restricción adicional). Depende del endpoint de Chat 5. **Sesión separada del Chat 5** (decisión 2026-04-23) — menos riesgo de PRs grandes cruzando repos.
...
- Plan 22 Chat B ambos widgets (throttle + defer-fail) están ✅ cerrados — fila 22 al 95%. Chats 5-6 cierran Plan 22 al 100%.
