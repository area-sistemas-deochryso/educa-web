...
- ~~**F-021 (brief `199`)**~~ ✅ implementado 2026-05-19, awaiting-prod. Helper `auto-open-from-query.helper.ts` extendido con rama `?dni=X[&autoOpen=true]` (union discriminado `kind: 'id' | 'dni'`, contrato legacy `openUserId/openUserRol/openUserName` preservado). Effect del componente abandona target DNI si la lista terminó sin match único (defensivo contra 0/2+ matches). 14 tests del helper verdes. Verificación post-deploy listada abajo en `awaiting-prod/`.
- ~~**F-022 (brief `200`)**~~ ✅ closed 2026-05-19 como **falso positivo / wontfix**. Investigación read-only confirmó que el comportamiento actual (AA cae en `<app-attendance-director />`) es el estado deliberado post-brief 145 (commit `b9901da`, "chore(intranet): sync invariants and remove orphan AA self-service code"). El doc Cowork `verify-awaiting-prod-2026-05-19.md §1.4` comparó contra una expectativa pre-145. INV-AD08 fue aflojada deliberadamente — AA muta `AsistenciaPersona` con `TipoPersona='A'` igual que los demás administrativos. Cierre sin código.
- **Plan 45 F2 FE — UI rework `/incidencias/errores`** 🔴 (bumpea regla de orden — prioridad coord 2026-05-19). Sub-plan en [`educa-coord/plans/xrepo-45-monitoreo-incidencias-rework.md`](../../../educa-coord/plans/xrepo-45-monitoreo-incidencias-rework.md) §F2. Corre en paralelo a F5 BE. **F2.1 ✅ verificado 2026-05-19 (brief 197 closed)** — Rework del sub-drawer `error-occurrence-drawer` con 5 tabs PrimeNG (General/Trace/Breadcrumbs/Group/Req-Res). Sin endpoint BE nuevo: `[parentGroup]="selectedGroup()"` pasa el grupo padre desde el page component. Tab Trace muestra stack texto + caja informativa "F5 BE pendiente" para `ErrorLogTrace[]` por capa. Cowork BD-PROD-RO confirmó las 5 tabs. **F2.2 ⏳ pendiente** — JOIN endpoint `/full` + vista por evento `ErrorLog` en `ErrorGroupsViewToggle` + reemplazo del placeholder Trace por tabla por capa cuando F5 BE complete. Estimado coord F2.2: 1 chat FE + 0.5-1 chat BE.
- **Brújula W21 — Pulido intranet académica (2026-05-18 → 2026-05-24)** → sub-plan FE [`intranet-fe-polish-W21.md`](intranet-fe-polish-W21.md). **F1 ✅ closed 2026-05-18 (brief 187)** — matriz `F1.Resultados` poblada con 10 pages auditadas. **F2 ✅ 2026-05-18** — 5 briefs materializados en `chats/open/` ordenados chico→grande: `188` ✅ closed 2026-05-18 (no-op: `appendTo="body"` ya presente en commits previos del attendance-summary-panel), `189` ✅ closed 2026-05-18 (tokens colors — 4 archivos en estudiante/cross-role: 19 hex → tokens Aura; `CURSO_COLORS` marcado como excepción legítima por `darkenColor()` bitwise), `190` ✅ closed 2026-05-18 (Opción A: gate visual ya estaba vía `@if(debugInfo())` + `environment.debug.horarioSync`; 8 hex listados + 4 inline HTML → tokens Aura `--red-500`/`--yellow-500`/`--orange-500`/`--white-color`/`--green-500` + 5 SCSS vars `$debug-*` para paleta Catppuccin del panel dev-only con excepción documentada), `191` ✅ closed 2026-05-18 (aria-label vía `pt`/atributo nativo en day selector de `estudiante/schedules` + `incrementButtonAriaLabel`/`decrementButtonAriaLabel` en `p-inputNumber` del simulador de notas; resto del scope sin botones icon-only), `192` ✅ closed 2026-05-18 (skeletons shared — 4 pages + 3 sub-components migrados: `estudiante/schedules` overlay → grid de 6 `app-skeleton-loader`; `estudiante/notas` `p-progressSpinner` → combo header+filters+card; `estudiante/cursos` + `profesor/cursos` → `app-table-skeleton` con `SkeletonColumnDef[]`; sub-components `student-files-dialog`/`student-task-submissions-dialog`/`calificaciones-panel` → stacks de `rect`; 0 `p-progressSpinner` aislados en scope; `minHeight` declarado en todos). **F3 ✅ closed 2026-05-19 (brief 193)** — split del monolito (290 ln) en `.ts` slim (211 ln) + `.html` + `.scss` separados, siguiendo patrón de `estudiante/notas`. 2× `p-progressSpinner` → `app-skeleton-loader` (variant `rect` + `card`). `p-confirmDialog` movido fuera del `@if (contenido())` (dialogs-sync compliance). Custom dialogs (`app-evaluacion-form-dialog`, `app-calificar-dialog`, `app-periodos-config-dialog`) quedaron dentro del `@if` por dependencia con `contenido()` no-null — fix arquitectural fuera de scope. Lint ✅ · tsc ✅. **F4 ✅ implícito** (100% de los 6 briefs F2 cerrados). **F5 ✅ closed 2026-05-19 (brief 194)** — cierre semanal: matriz F1.Resultados con cierre 1:1 a los 6 briefs ejecutados; deuda residual capturada en `tasks/polish-post-W21-grades-dialogs-sync.md` (3 custom dialogs de `profesor/grades` requieren aceptar `null` en inputs para salir del `@if`). Brújula coord en [`brujula-W21-intranet-polish.md`](../../../educa-coord/plans/brujula-W21-intranet-polish.md). BE corre en paralelo con su propio sub-plan de tests. **Brújula W21 FE ✅ cerrada 2026-05-19**.
...
- **Plan 41 — Correlation Hub** → [`educa-coord/plans/xrepo-41-correlation-hub-observability.md`](../../../educa-coord/plans/xrepo-41-correlation-hub-observability.md). Próximo FE-only: Chat 9 (search global) ⏳ requiere BE. **Chat 3a (186), Chat 3b (185), F5 Chat 10 (180), Chat 11 (195) ✅ verificados prod 2026-05-19** (Cowork BD-PROD-RO).
...
- **FE puros pendientes**: Plan 1 F5 (`137`), Plan 28 Chat 4 (`134`), Plan 43 Chat 2.1 FE G.1+G.2 (`147` — G.3 falló → F-021 brief `199` ✅ implementado 2026-05-19, awaiting-prod), F-021 brief `199` (deep-link `?dni=X` — verificar tras deploy: filtra+abre con `?dni=74125896&autoOpen=true`, no abre con 0/2+ matches, contrato legacy desde auditoría sigue OK), Plan 43 Chat 3.1b FE drawer SMTP (`169` — blacklist OK, quarantine sin data prod), Plan 43 Chat 1.1 vista uniforme (`143` — 4/4 roles OK; F-022 brief `200` cerrado como falso positivo 2026-05-19 — el comportamiento AA = panel admin es deliberado post-145), WAL hard/soft delete audit (`118, 119`), Plan 41 Chat 11 auto-refresh (`195` ✅ verificado 2026-05-19 — keep here hasta `/verify` formal).
- **Verificados 2026-05-19** (movidos a `closed/` por Cowork BD-PROD-RO): `180` Plan 41 Chat 10 export JSON · `184` Plan 43 Chat 4.1b bandeja paginación · `185` Plan 41 Chat 3b related correlation IDs · `186` Plan 41 Chat 3a cross-link buttons · `152` Plan 43 Chat 1.2 sparkline · `195` Plan 41 Chat 11 auto-refresh · `197` Plan 45 F2.1 occurrence drawer tabs · dashboard usuarios estadísticas Promotor + Coord.Académico.
...
- **Cowork verificación post-deploy 2026-05-19** (commit `81c369c`) — Sesión BD-PROD-RO sobre FE main `6b6ace9` + BE master `7cc17a3`. Procesó 10 briefs en `awaiting-prod/`: 8 verificados ✅ y movidos a `closed/` (180 export JSON · 184 bandeja paginación · 185 related correlationIds · 186 cross-link buttons · 152 sparkline · 195 auto-refresh · 197 occurrence drawer tabs · dashboard usuarios stats). 3 quedan en `awaiting-prod/` con anotación: **169** blacklist OK pero quarantine vacío en prod (sin data para badge "reconstruido"); **147 G.3** ❌ deep-link `?dni=X&autoOpen=true` no consume query params → F-021 nuevo brief `199` en `open/`; **143** ⚠️ regresión AA YARUPAITA: ve panel admin completo en `/intranet/asistencia` cuando brief 134 parte 4a definió self-service → F-022 nuevo brief `200` en `open/`. Reporte completo en [`claude-cowork/reporte-cowork-2026-05-19.md`](../claude-cowork/reporte-cowork-2026-05-19.md). Aclaración del 186: el anchor "Abrir en rate-limit" existía desde el commit inicial del hub (`a70b8d3`, 2026-04-25), la nota original "rate-limit sin botón" era confirmación errada — no hubo regresión.
...
## 🔧 Hallazgos Cowork post-deploy 2026-05-19

Hallazgos detectados en verificación BD-PROD-RO sobre FE main `6b6ace9` + BE master `7cc17a3`. Reporte completo en [`claude-cowork/reporte-cowork-2026-05-19.md`](../claude-cowork/reporte-cowork-2026-05-19.md).

- 🟠 **F-021 FE pendiente** (brief `199` en `chats/open/`) — Deep-link `/intranet/admin/usuarios?dni=X&autoOpen=true` no consume query params: tabla sin filtrar + drawer cerrado. Rompe flujo auditoría → usuarios que el brief 147 G.3 había declarado completado. Severidad Alto. Fix: consumir `route.snapshot.queryParamMap` en `ngOnInit` + setear search + esperar 1 match + abrir drawer. Edge cases: 0 matches (no abrir), 2+ matches (filtrar sin abrir).
- ✅ **F-022 closed 2026-05-19 como falso positivo / wontfix** (brief `200` en `chats/closed/`). Investigación read-only confirmó que AA en `/intranet/asistencia` cayendo en `<app-attendance-director />` es el estado deliberado post-brief 145 (commit `b9901da` "remove orphan AA self-service code"). El doc Cowork comparó contra expectativa pre-145 ya invalidada. INV-AD08 fue aflojada deliberadamente — AA muta `AsistenciaPersona` con `TipoPersona='A'` igual que los demás administrativos. Si futuro Cowork rebrota la expectativa de self-service AA, está mirando doc obsoleta. Ver handoff completo dentro del brief 200.
