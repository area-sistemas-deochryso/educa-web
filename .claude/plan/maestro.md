...
5. **[DS1-tests · `educa-web` · FE]** — Tests directos del hot loop de `WalSyncEngine`. Brief en `.claude/chats/open/086-wal-sync-engine-tests-hot-loop.md`. Cubre `init()` race, `processAllPending` drain, `processEntry` failsafe, los 4 paths post-DS1 de `handleError` (conflict / permanent / retryable-ok / retryable-max), `handleCrossTabCommit`. **Razón de prioridad (decisión 2026-05-04)**: post-DS1 el engine quedó con margen 26 al cap 300; cualquier cambio futuro al engine sin red de seguridad es trabajo a ciegas.
