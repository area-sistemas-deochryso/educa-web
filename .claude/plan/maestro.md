...
2. **[Plan 24 · Chat 3 · `educa-web` · FE]** — Frontend con `p-progressBar` + suscripción al `AsistenciaHub` en servicio singleton (sobrevive navegación). Promovido a #2 tras cierre de Chat 2 (commit `513c6cc` en Educa.API master, 2026-04-23). Consume el contrato `CrossChexSyncStatusDto` ya definido + eventos `"SyncProgress"` del hub. Reemplaza spinner bloqueante por barra con mensaje dinámico ("Descargando página 3/5…"), timer y toast al COMPLETED/ERROR. El DTO actual ya tiene todos los campos que el FE necesita (`Pagina/TotalPaginas`, `IniciadoEn`, `Fase`) — feedback visual extra pedido durante Chat 2 se arma aquí.
3. **[pendiente]** — el tercer slot sigue abierto. Candidatos fuera del top 3 que pueden promoverse: Plan 28 Chat 3 BE (bloqueado por validación jefe Plan 27), Plan 26 F3 (espera telemetría ~2 semanas), Plan 24 Chat 4 BE (validar `Task.Delay(30000)` — natural después de Chat 3), Design System F5.3 (prioridad más baja). Discutir con el usuario al arrancar Plan 24 Chat 3 qué entra aquí.
...
> **Estado**: 🟢 **Chat 1 ✅ cerrado 2026-04-23** (commit `299db24` en `Educa.API master`, +7 tests → 1302 BE verdes). 🟢 **Chat 2 ✅ cerrado 2026-04-23** (commit `513c6cc` en `Educa.API master`, +14 tests → 1316 BE verdes). Plan al **~50%**. Faltan Chat 3 (FE progress bar + suscripción hub en servicio singleton), Chat 4 (validar `Task.Delay`).
