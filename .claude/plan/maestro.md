...
| 22 | Endurecimiento correos de asistencia | BE+FE | `Educa.API/.claude/plan/asistencia-correos-endurecimiento.md` | 🟢 En progreso · **Chats 1 (F1) + 2 (F2) + 3 (F3.BE) + A (F5+F6) ✅ cerrados 2026-04-21**. **Chat A (F5+F6 merged — SMTP outbound throttle + multi-sender, 2026-04-21)**: techo efectivo sube de 50/h (single) a 200/h (domain-bound) con 7 buzones en el mismo dominio `@laazulitasac.com`. Sliding window 60 min con **dos counters independientes** (per-sender 50/h + per-dominio 200/h). `IQuotaThrottleService` + `QuotaThrottleService` (lógica pura, `AsNoTracking()`). `EmailOutboxWorker` declarado `partial class` para respetar cap 300ln; archivo `.Sender.cs` aisla `SelectSenderOrApplyQuotaSaturationAsync` + `ApplyQuotaSaturationPolicy` + `GetFallbackSender` (INV-S07 fail-open a sender[0] si el throttle falla). Round-robin stateful con `_lastSenderIndex` en memoria, `(lastIndex+1) mod N`, skip de saturados. Re-enqueue usa `min(FechaEnvio domain window) + 60 min + jitter(1..5 min)` para evitar thundering herd. Tras `ThrottleMaxReintentosCuota=5` diferidos → FAILED con `EO_TipoFallo="FAILED_QUOTA_EXCEEDED"`. **Contador separado `EO_IntentosPorCuota`** (decisión del usuario: Opción 2 — no mezclar con `EO_Intentos` del retry SMTP de F2). `EmailSettings` mantiene sender 1 intacto + agrega 6 slots planos (Address2..7) + 5 props throttle; computed `Senders` filtra slots vacíos (rollback barato — borrar env vars 2-7 revierte a single-sender sin redeploy). `EmailService.SendEmailOnceAsync(Email, SenderConfig)` recibe el buzón seleccionado; `SendViaSmtp`/`SaveToSentFolder` autentican con el buzón elegido. Nueva columna `EO_Remitente` (NVARCHAR 200, nullable) + índice filtrado `IX_EmailOutbox_Remitente_FechaEnvio_Sent` + `EO_IntentosPorCuota INT NOT NULL DEFAULT 0`. Constante `SmtpErrorClassifier.FailedQuotaExceeded` agregada. 13 unit tests (`QuotaThrottleServiceTests`) + 6 integration tests nuevos en `EmailOutboxWorkerTests` (casos g-l: sender available, first saturated, all saturated re-enqueue, domain saturated, FAILED_QUOTA_EXCEEDED, throttle disabled). Pendiente: ejecución de 4 scripts SQL en prueba+prod, 18 env vars (Email__Address2..7 + Password + DisplayName) en App Service, build+test end-to-end (bloqueado por `Educa.API.exe` local corriendo al momento de cerrar el chat). **Chat B (F5.6 FE widget en `/intranet/admin/bandeja-correos` mostrando 7 counters + domain counter)** va en repo `educa-web`. Chat 4 (F3.FE) y Chats 5-6 (F4 BE+FE) también pendientes. | 60% |
...
- [x] **Plan 22 — Endurecimiento correos de asistencia** (60% avanzado, **F5+F6 BE ejecutados 2026-04-21 — Chat A**)
  ...
  - **✅ F5+F6 (throttle saliente + multi-sender) — Chat A BE ejecutado 2026-04-21**: sliding window 60 min per-sender (50/h) + per-dominio (200/h); round-robin entre 7 buzones de `@laazulitasac.com`; re-enqueue con jitter y `FAILED_QUOTA_EXCEEDED` tras N defers. Contador separado `EO_IntentosPorCuota` (Opción 2 — no mezcla con retries SMTP de F2). **Techo efectivo sube de 50/h a 200/h**. Pendiente ejecutar 4 scripts SQL en prueba+prod, configurar 18 env vars (Email__Address2..7 + Password + DisplayName), correr suite completa (bloqueo local por backend corriendo), **Chat B FE (widget `/api/email-outbox/throttle-status`)** en repo `educa-web`.
