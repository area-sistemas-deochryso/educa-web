...
- **Plan 45 F2 FE — UI rework `/incidencias/errores`** 🔴 (bumpea regla de orden — prioridad coord 2026-05-19). Sub-plan en [`educa-coord/plans/xrepo-45-monitoreo-incidencias-rework.md`](../../../educa-coord/plans/xrepo-45-monitoreo-incidencias-rework.md) §F2. Corre en paralelo a F5 BE. **F2.1 ✅ awaiting-prod 2026-05-19 (brief 197)** — Rework del sub-drawer `error-occurrence-drawer` con 5 tabs PrimeNG (General/Trace/Breadcrumbs/Group/Req-Res). Sin endpoint BE nuevo: `[parentGroup]="selectedGroup()"` pasa el grupo padre desde el page component. Tab Trace muestra stack texto + caja informativa "F5 BE pendiente" para `ErrorLogTrace[]` por capa. Tab Group con bloque "Grupo padre" + CorrelationId con link "Ver eventos correlacionados". Breadcrumbs/Req-Res con empty states cruzados según `origen` (FE/BE). Smoke browser local con Cowork: 18/18 checks ✅. Pendiente smoke prod post-deploy Netlify → `/verify 197`. **F2.2 ⏳ pendiente** — JOIN endpoint `/full` + vista por evento `ErrorLog` en `ErrorGroupsViewToggle` + reemplazo del placeholder Trace por tabla por capa cuando F5 BE complete. Estimado coord F2.2: 1 chat FE + 0.5-1 chat BE.
