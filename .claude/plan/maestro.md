...
1. **[Plan 30 · Chat 4 · `Educa.API` · BE]** — Búsqueda diagnóstico por correo específico: endpoint `GET /api/sistema/email-outbox/diagnostico?correo={email}`. Cierra el frente de visibilidad admin sobre el outbox (M1-M8 del Plan 30). **Promovido a #1 tras cierre Plan 30 Chat 3 (2026-04-24, commit `eb92ec2`)**.
2. **[Plan 31 · Chat 2 · `Educa.API` · BE]** — Parser IMAP + Hangfire job + tabla `BounceParserProcessed`. `BounceParserService` + partial `.Imap.cs` + `Ndr3464Parser` + `BounceCorrelator` + `BounceParserJob` recurring `*/5 * * * *`. Procesa `INBOX`/`Junk`/`Trash` de los 7 buzones del pool Plan 22 F6. **Bloqueado hasta validación del header `X-Educa-Outbox-Id` en Roundcube post-deploy del Chat 1 (`c46dfa0` aún sin pushear)**. Brief en `.claude/chats/038-plan-31-chat-2-be-bounce-parser-imap-job.md` listo para cuando desbloquée.
3. **[Plan 30 · FE dashboard Chat 3]** — pantalla admin que consume el nuevo endpoint `diagnostico-correos-dia`. Entra cuando el admin valide el shape del DTO en uso real (hoy solo hay back; el FE se arma después de observar el endpoint funcionando).

**Notas**:

- 🟢 **Plan 30 Chat 3 F3.BE ✅ cerrado 2026-04-24** (commit `eb92ec2` en `Educa.API master`) — gap asistencia-vs-correos. Endpoint admin cruzando 4 tablas (AsistenciaPersona + Estudiante + EmailOutbox + EmailBlacklist) con correlación por destinatario+fecha, INV-C11 aplicado in-memory, DNI/correos enmascarados, fail-safe INV-S07. 19/19 tests del scope verde + 1355 BE verdes (sin regresiones). Plan 30 a ~75%, falta solo Chat 4 BE.
