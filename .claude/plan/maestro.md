...
|---|------|------|------|--------|---|
| 1 | Enforcement de Reglas | FE | `tasks/enforcement-reglas.md` + F5 promocionada a [`plan/enforcement-fase-5-wrappers.md`](./enforcement-fase-5-wrappers.md) (brief 137 en `chats/open/`) | F1-F3 ✅ · F4 parcial ✅ (F4.4-F4.5 🔒) · F5 ⏳ | ~75% |
...
| **42** | **🟢 Normalización de casing en contratos REST FE↔BE** | **BE+FE** | **[plan/case-drift.md](./case-drift.md)** | **🟢 Plan abierto 2026-05-09. Auditoría dual `/investigate` detectó: mito documental en `Program.cs:74` ("camelCase para coincidir con HTTP" pero `AddNewtonsoftJson` no setea `ContractResolver` → default PascalCase) + 4 políticas de serialización distintas en el mismo BE (HTTP REST PascalCase / SignalR camelCase / GlobalException camelCase / PayloadSanitizer camelCase) + CORS Expose-Headers incompleto (solo `Retry-After`; `X-Correlation-Id` y `X-Schema-Version` invisibles en navegador — **posible bug latente que Plan 41 ya está pagando**) + WAL endpoint paths sin normalize en persistencia. **Prerrequisito de Plan 14** (snapshots fallarían sin casing estable). 4 fases (F1 verificación empírica · F2-BE/F2-FE paralelos · F3 anti-regresión · F4 docs INV-CONTRACT01/02/03). Briefs FE+BE en `chats/open/`.** | **0%** |
...
7. **[Plan 1 · F5 · `educa-web` · FE]** — Hardening de wrappers (cerrar escape hatches). Promocionado 2026-05-09 desde `tasks/enforcement-reglas.md`. Plan en [`plan/enforcement-fase-5-wrappers.md`](./enforcement-fase-5-wrappers.md), brief en `chats/open/137-fe-enforcement-f5-wrapper-hardening.md`. **Scope**: 3 zonas críticas (`storage/`, `wal/`, `session/`) — reducir barrel exports a solo el wrapper público + nueva regla `LAYER_RULES` que prohíbe imports a impls internas. Audit grep + migrar consumidores + reducir `index.ts` + lint rule + validar (lint/build/test). **Razón**: F1-F4 ya en producción (lint cross-capa, tests contrato, tipos semánticos, CI). Faltan los barrel exports — hoy un dev dentro de `@core/services/storage` puede importar `SessionStorageService` directo. Cerrar el hatch hace al wrapper el único camino. Estimación ~2-3h, un solo chat. Sin dependencias BE ni features en flight.
