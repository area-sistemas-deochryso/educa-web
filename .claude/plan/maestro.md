...
>
> **~~Plan 44 Chat 1 design~~** ❌ **DESCARTADO 2026-05-12** (brief 141) — el contador hosting mira FAILED, no SENT; Educa ya tiene 0.27% FAILED rate por las defensas. Reducir volumen no mueve la métrica relevante. Scope absorbido por Plan 22 Chat 7 nuevo (guard de cesión automática) sobre la base ya planeada Plan 22 Chat 5/6 (auditoría destinatarios inválidos). CrossChex queda como está (restricción explícita: funcional, sin reemplazo aprobado).

1. **[Plan 22 · Chat 7 · `Educa.API` · BE]** — Guard de cesión automática en `EmailOutboxWorker`. Cuando endpoint `/defer-fail-status` reporta contador ≥80% del techo, pausar correos con `EO_EntidadOrigen ∈ {Asistencia, AsistenciaProfesor}` (informativos) y mantener los críticos administrativos. Sin UI, sin opt-in, transparente. Brief en `chats/open/146` (creado 2026-05-12). Formaliza INV-MAIL10 ya documentada en `business-rules.md §15.14`.
2. **[Plan 22 · Chat 5 · `Educa.API` · BE]** — Auditoría destinatarios inválidos (F4.BE ya planificada). Endpoint `GET /api/sistema/auditoria-correos-asistencia` que barre BD + reusa `EmailValidator`. Pre-requisito conceptual del Chat 7 — primero limpiamos BD, después aplicamos guard.
3. **[Plan 22 · Chat 6 · `educa-web` · FE]** — UI admin de la auditoría (F4.FE ya planificada). Pantalla `/intranet/admin/auditoria-correos`.
...
- **Chat activo en `running/`**: brief 141 (Plan 44 Chat 1 `/design`, retomado 2026-05-12 tras pausa por 144/145 urgentes — conclusión: **Plan 44 descartado**).
- **Plan 29 cerrado al 100% el 2026-05-12** — Chat 3 OPS marcado WONTDO tras denegación del hosting.
- **Plan 44 descartado el 2026-05-12** — el contador del hosting mira FAILED, no SENT; reducir volumen no mueve la aguja. Scope mínimo viable absorbido por Plan 22 (Chat 7 nuevo + Chat 5/6 ya en cola). CrossChex queda como está por restricción explícita del usuario.
