...
| 25 | 🟢 Paridad Excel para reportes PDF | BE+FE | (inline en maestro) | 🟢 En progreso · Chat 1 (F1) ✅ 2026-04-21 — `ClosedXML 0.104.*` + `IAsistenciaExcelService` + `AsistenciaExcelService` (base + partials Estudiantes/Profesores, <300 ln) + `GET /api/reportes-asistencia/excel` con `[EnableRateLimiting("heavy")]` mirror del `/pdf` + 12 tests (877 total, 0 regresiones) · `IReportBuilder` genérico diferido a Plan 2/C.2 (entrega aditiva pura) · pendiente Chat 2 (migrar 5 reportes PDF restantes) + Chat 3 (UI dual FE) + Chat 4 (docs + tests paridad). | 25% |
...
> **Estado**: Chat 1 ✅ (2026-04-21) · Chat 2 ⏳ migrar 5 reportes restantes · Chat 3 ⏳ UI dual FE · Chat 4 ⏳ docs + tests paridad end-to-end.
...
- **Chat 2 — BE: migrar 5 reportes PDF restantes a Excel**: `ReporteAsistenciaConsolidado`, `ReporteAsistenciaSalon`, `ReporteAsistenciaProfesor`, `BoletaNotas` y cualquier otro PDF que quede. Reusar `AsistenciaExcelService` extendiendo o creando services hermanos. Mantener `[EnableRateLimiting("heavy")]` y Content-Type correcto. Si aparece fricción fuerte por duplicar mapeo de columnas, adelantar `IReportBuilder` (Plan 2/C.2).
