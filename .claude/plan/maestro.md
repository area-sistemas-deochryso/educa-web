...
| **32** | **🟢 Centralización de errores vía Correlation ID** | **BE+FE** | **[plan/correlation-id-links.md](./correlation-id-links.md)** | **🟢 Chat 2 BE ✅ cerrado 2026-04-25 (commit `1ca1098` en `Educa.API master`). `EO_CorrelationId NVARCHAR(36) NULL` ya en BD (creada por el usuario el 2026-04-24). Modelo `EmailOutbox` gana propiedad. **Hallazgo clave**: el `CorrelationIdMiddleware` NO escribía a `HttpContext.Items` (solo logging scope + response header) — agregado `public const string CorrelationIdItemKey = "CorrelationId"` + escritura en `Items` antes del `OnStarting`. `EmailOutboxService` gana `IHttpContextAccessor?` opcional en ctor; partial `.Enqueue.cs` resuelve el id vía helper `ResolveCorrelationId` con triple fallback null (sin accessor, sin HttpContext, sin key) y try/catch global INV-S07. `EmailOutboxListaDto` + proyección de `ListarAsync` exponen `CorrelationId` para Chat 4 FE. **+6 tests** (con id, sin HttpContext, sin key, valor no-string boxed → ToString+truncate, ListarAsync hidrata DTO, INV-MAIL01 prevalece sobre el hook) → **1379 BE verdes** (baseline 1373). 6 archivos tocados, cap 300 respetado en todos. Deuda detectada: `EmailFailureLogger.ExtractCorrelationId` lee `X-Correlation-Id` del request header (puede devolver null si el cliente no lo manda) — fuera del scope, anotada para chat futuro. 🟢 Chat 1 `/design` ✅ cerrado 2026-04-24. 11 decisiones + 4 chats. **Siguiente**: Chat 3 BE — SELECT previo sobre `sys.indexes` + hasta 4 CREATE INDEX filtrados + endpoint + service con `CorrelationSnapshotFactory.BuildEmpty` (patrón Plan 30). Chat 4 FE último — hub page + pill + wiring + query param sync en los 4 dashboards.** | **~35%** |
