...
4. **[Plan 43 · Chat 1.2 BE · `Educa.API` · BE]** — Fingerprint v2 (whitelist `SLOW_REQUEST`/`SLOW_REQUEST_FAILED` omiten mensaje + `httpMethod` al hash + separador NUL) + `ErrorGroupBackfillService` idempotente con lock advisory + endpoint admin protegido (rol Director, bulkhead `concurrency:reports`) + tests + update `INV-ET06`. Diseño cerrado en chat 148 + [ADR-0007](../../../Educa.API/.claude/decisions/0007-fingerprint-algorithm-and-merge.md). Brief en [`chats/open/149-be-plan-43-chat-1-2-fingerprint-v2-merge-job.md`](../chats/open/149-be-plan-43-chat-1-2-fingerprint-v2-merge-job.md). Cierra **A8** (340 grupos = 1 bug). Riesgo: job correctivo sobre prod requiere staging primero — el deploy NO ejecuta el job, lo dispara el Director manual con ack.
5. **[Plan 43 · Chat 1.2 FE · `educa-web` · FE]** — Mini-sparkline 30d en cards de `ErrorGroup` + modal ampliado. **Bloqueado por brief 149** (necesita endpoint de trend). Al inicio del chat: decidir si consume el endpoint de 149 o si Plan 43 Chat 5.1 ya entregó `/serie-temporal` genérico y se consolida ahí. Brief en [`chats/open/150-fe-plan-43-chat-1-2-errorgroup-sparkline.md`](../chats/open/150-fe-plan-43-chat-1-2-errorgroup-sparkline.md). Cierra **B2**.
6. **[Plan 43 · Chat 1.3 · BE+FE+E2E]** — Propagación correlation id end-to-end (front → BE → outbox → SMTP → hubs SignalR + RateLimitEvent). Cierra A11+B3. Razón: F3 (diagnóstico SMTP) y F6 (vista unificada) y B10 (bundle telemetría) **dependen** de esto. Hoy el hub correlacionado existe pero llega vacío. Coordinar con Plan 41 F6.
