...
2. **[Plan 22 · Chat 6 · `educa-web` · FE]** — F4.FE pantalla `/intranet/admin/auditoria-correos` con feature flag `auditoriaCorreos` + menú Sistema → Monitoreo. Consume `GET /api/sistema/auditoria-correos-asistencia` (Plan 22 Chat 5 BE ya cerrado, commit `a1082eb`). Acceso: `Roles.Administrativos` (4 roles, mismo que el BE). Cierra Plan 22 al 100%. Depende de que el BE esté desplegado en un ambiente accesible al FE (decidir con el usuario si desarrollar contra staging o mock del endpoint).
3. **[Plan 24 · Chat 1 · `Educa.API` · design]** — Sync CrossChex en Background Job (0%, 4 chats diseñados). Arranca **tras cierre de Chat 6 FE** del Plan 22. No requiere sender dedicado (decisión 2026-04-21 en Plan 22 Chat A) — comparte los 7 senders del pool. Entra al top 3 aunque no puede ejecutarse todavía porque ya no hay otro chat priorizable sobre él tras el cierre del Chat 5.
...
- Plan 22 Chat 5 F4.BE ✅ cerrado 2026-04-23 (commit `a1082eb`, +21 tests → 1295 BE verdes). Fila 22 al 97%. Chat 6 F4.FE cierra Plan 22 al 100%.
...
- **Plan 24** (sync CrossChex background) es necesario/obligatorio pero de prioridad más baja que Plan 22 F4 — arranca tras cierre de Chat 6 FE.
