...
2. **[Plan 40 · F2 · `Educa.API` · BE]** — bulkheads 5 categorías (`pagos`=15, `reports`=8, `notif`=15, `uploads`=10, `bio`=20) + Default implícito. Brief `chats/open/104-be-load-control-f2-bulkheads.md`. Razón de prioridad: capa 3 que asigna sub-pools dentro de N=140; F1 puso la infraestructura del chained limiter, F2 la usa con `[EnableRateLimiting("concurrency:<bulkhead>")]` por endpoint. Aprendizaje de F1 a aplicar: validar empíricamente el orden de stashing cuando un endpoint lleva 2 atributos de rate-limit (rate + bulkhead) — el último factory en correr gana sobre `kind`, así que la concurrency factory debe ser la que sobre-stashea para que un rechazo del bulkhead se reporte como 503.
