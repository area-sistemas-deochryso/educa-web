...
| xP107 | → Entorno de desarrollo orientado a pruebas (coord P107) | F1 FE ✅ cerrado (brief [618](../chats/closed/618-fe-p107-f1-test-tools-surface.md), worktree `chat/618-fe-p107-f1-test-tools-surface`, pendiente `/wt-merge`) — flag `environment.debug.testTools` + ruta `/intranet/herramientas-prueba` + entrada dev-only en header, confirmado ausente del bundle de producción. F1 BE también ✅ (Educa.API brief 617). **F1 completo** — F2/F3 quedan desbloqueadas para diseñarse | ver P107 |
| F619 | Fix: campo "ruta" fantasma en diálogo Nueva/Editar Capability (`vistas.component.html`) | brief [619](../chats/open/619-fix-ruta-fantasma-catalogo-capabilities.md) preparado en `open/`, sin arrancar — hallado verificando P107 F1, sin dependencia con ese plan | libre |
