...
6. **[DS1-smoke · `educa-web` · QA browser]** — Smoke test de integración del WAL en browser (8 casos: optimistic happy path, offline→online recovery, conflict 409, permanent 4xx, retryable 5xx, cross-tab leader election, recovery on reload, schema migration). Brief en `.claude/chats/open/087-wal-integration-smoke-test.md`. Vive en `claude-cowork/` como gating checklist para PRs futuros que toquen `core/services/wal/`. **Razón de prioridad (decisión 2026-05-04)**: subsistema runtime crítico sin tests de integración hoy; vitest cubre unidades aisladas, no el ciclo IndexedDB + HTTP + leader + SW.
