...
| xP107 | → Entorno de desarrollo orientado a pruebas (coord P107) | F1 FE ✅ cerrado (brief [618](../chats/closed/618-fe-p107-f1-test-tools-surface.md), worktree `chat/618-fe-p107-f1-test-tools-surface`, pendiente `/wt-merge`) — flag `environment.debug.testTools` + ruta `/intranet/herramientas-prueba` + entrada dev-only en header, confirmado ausente del bundle de producción. F1 BE también ✅ (Educa.API brief 617). **F1 completo** — F2/F3 quedan desbloqueadas para diseñarse. Verificación en vivo de F1 encontró bug lateral (campo `ruta` fantasma en diálogo Nueva/Editar Capability) → brief [619](../chats/closed/619-fix-ruta-fantasma-catalogo-capabilities.md) ✅ cerrado (2026-09-03), sin dependencia con este plan | ver P107 |
