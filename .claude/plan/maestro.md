...
| **46** | **🟡 Limpieza de drift documental en `.claude/` FE** | **FE** | **[plan/drift-doc-cleanup.md](./drift-doc-cleanup.md)** | **🟡 F1 ✅ (chat 156 2026-05-13) — refs `@data/repositories` removidas de `rules/architecture.md`, `rules/code-style.md`, `CLAUDE.md`, `project-structure/architecture.md`. F2 parcial: audit overlay agregado al `drift-report.md` (15/28 ✅ resueltos, 2 ❌ drift activo, 1 ⚠️ parcial, 10 ⏳ no re-verificados). Drift real C3.1 (`asistencia/`→`attendance/`, `permisos/`→`permissions/`, agregadas `capacitor/` + `feedback/`) detectado y corregido en el mismo chat. **Pendiente**: F2.b re-verificar 10 items + F3 separar ejemplos `[(visible)]` en `dialogs-sync.md` → brief 157.** | **40%** |
| **47** | **🟢 Limpieza de links rotos en `maestro.md`** | **FE** | **[plan/maestro-links-cleanup.md](./maestro-links-cleanup.md)** | **🟢 Plan abierto 2026-05-13. 18 links rotos detectados por audit del Plan 46 F2. Mix de refs a planes archivados (deben apuntar a anclas en `history/planes-cerrados.md`) + refs aspiracionales nunca creados + falsos positivos del regex. Brief 158 pre-staged. ~30 min.** | **0%** |
| **48** | **🟢 Barrido `appendTo="body"` en `p-select`** | **FE** | **[plan/appendto-barrido.md](./appendto-barrido.md)** | **🟢 Plan abierto 2026-05-13. 7 archivos `.html` con `<p-select` sin `appendTo` (bajó de ~30 reportados en abril). Barrido oportunístico ~30 min. Brief 159 pre-staged.** | **0%** |
...
9. **[Plan 46 · Chat único · `educa-web` · FE docs]** — Limpieza de drift documental en `.claude/`. F1 (quick, 30 min): quitar refs muertas a `@data/repositories` de `rules/architecture.md:118,398` y `rules/code-style.md:24` (el alias ya no existe en `tsconfig.json`). F2 (medio): auditar `drift-report.md` marcando cada sección como ✅/⏳/❌ y archivar si >70% obsoleto. F3 (opcional): separar ejemplos `[(visible)]` en `dialogs-sync.md`. **Razón de prioridad**: docs falsas inducen decisiones equivocadas en futuros chats; pero no bloquea features — candidato a hacer oportunísticamente al tocar las reglas afectadas.
