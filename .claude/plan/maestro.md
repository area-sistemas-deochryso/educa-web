...
| BAudit2 | Auditoría buenas prácticas Angular 22/TS 6.0 | F1 ✅ · F2 FE ✅ awaiting-prod (664, BE handoff 667 ya resuelto) · F3 FE ✅ awaiting-prod (665) · F4 brief abierto (666) · **F5-F12 briefs abiertos 2026-09-12** (679-686) | 🟡 |
...
**En curso**: **BAudit2 F6** (Tier 0, pos 0.3) — config zoneless explícita, brief [680](../chats/running/680-audit-f6-zoneless-config-explicita.md), worktree `chat/680-audit-f6-zoneless-config-explicita`. Tier 0 reordenado 2026-09-12 a pedido explícito del usuario para priorizar el resto del audit Angular 22/TS 6.0 (F4-F12) por sobre xP41. Audit F4 (666, pos 0.1) ✅ cerrado (9/9 fixes). Audit F5 (679, pos 0.2) ✅ cerrado, awaiting-prod. xP41 F3 BE (persist request lifecycle) queda como siguiente tras cerrar Tier 0. Admin block cerrado (P62-P66 ✅/descartado). P50 F3a ✅ (331 closed).
...
- **Cierre previo**: 664 (BAudit2 F2 — seguridad passwords `users/`: regla de validación contradictoria (`.toUpperCase()` forzado vs. exigencia de minúscula, nunca pasaba) corregida; `generatePassword()` ahora agrega componente aleatorio real vía `crypto.getRandomValues` (antes 100% derivable de apellido+DNI); export de credenciales xlsx ya no incluye la password + gate `hasCapability('USUARIOS_EXPORT_CREDENCIALES_MANAGE')`; botón "Migrar Contraseñas" gateado con `hasCapability('USUARIOS_MIGRAR_CONTRASENAS_MANAGE')` + flag dev, fail-closed. Ambos gates quedan ocultos en todo ambiente hasta que `Educa.API` siembre las capabilities (comportamiento esperado). Hallazgo confirmado como BE (no FE): `UsuarioDetalleDto.Contrasena` descifra deliberadamente y expone en claro el GET de detalle — documentado y coordinado vía handoff cross-repo, `Educa.API` brief [667](../../../Educa.API/.claude/chats/open/667-be-handoff-audit-fe-664-password-plaintext.md) (3 secciones: decisión DTO + 2 seeds de capability). 8 tests nuevos (`password.utils.spec.ts`) + 2572/2573 tests existentes verdes (1 timeout flaky confirmado no-regresión, re-corrido aislado en 2.1s), lint 0 errores, build verde) → `awaiting-prod`, worktree `chat/664-audit-f2-seguridad-password` (pendiente `/wt-merge`), 2026-09-12.
