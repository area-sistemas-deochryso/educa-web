...
1. **[Plan 29 · Chat 3 · `—` · OPS]** — inspección CrossChex + negociación con hosting para subir `max_defer_fail_percentage` (5/h → 25-30/h). **No es código — no requiere brief**. Bloquea Chat 4 docs (los invariantes `INV-MAIL04` quedan con threshold final). Depende de tu coordinación con el admin cPanel.
2. **[Plan 29 · Chat 4 · `educa-web` · docs]** — formalizar `INV-MAIL01/02/03/04` en nueva sección `§18 Correos salientes y protección del canal SMTP` de `.claude/rules/business-rules.md` + registro de invariantes §15. Cierre documental del Plan 29. Ejecutar **después** de Chat 3 OPS para anclar el threshold negociado.
3. **[Plan 28 · Chat 3 · `Educa.API` · BE]** — reportes PDF/Excel + correos + bandeja + notificaciones para `TipoPersona='A'` (Asistente Administrativo). Desbloqueado por Chat 2 BE cerrado 2026-04-22. Requiere esperar validación post-deploy del Plan 27 antes de arrancar (evita PRs simultáneos sobre `AsistenciaPersona` + `EmailNotificationService`).

**Notas**:

- Chat 3 OPS del Plan 29 lo ejecuta el usuario con el hosting; cuando cierre, reabrir este orden.
- Plan 22 Chat B ambos widgets (throttle + defer-fail) están ✅ cerrados — fila 22 al 95%.
- Frentes abiertos sin bloqueo (Plan 26 F2 Chat 2, Design System F5.3, Carril D Olas 2+) no entran aquí hasta que desplacen prioridad a uno de los 3 ítems.

---

**Foco (actualizado 2026-04-23, post-cierre Plan 22 Chat B defer-fail-widget)**: 🟢 **Plan 22 Chat B defer-fail-widget ✅ cerrado** (educa-web `main`). Widget FE `<app-defer-fail-status-widget>` consume `GET /api/sistema/email-outbox/defer-fail-status` (Plan 29 Chat 2.6 BE, commit `7b2a962`) y renderiza 3 secciones empaquetadas en `/intranet/admin/email-outbox`: (1) hora actual vs techo cPanel con semáforo OK/WARNING/CRITICAL, (2) breakdown 24h por estado + tipo de fallo (diferencia visualmente los 3 tipos que no cuentan para cPanel via border dashed), (3) resumen blacklist por motivo. Polling 60s togglable, preferencias persistidas, feature flag `emailOutboxDeferFailWidget` (OFF prod, ON dev). Guard fail-safe muestra banner de telemetría cuando CRITICAL + counters en 0 (probable fallback INV-S07). **+10 tests FE** → **1519 FE verdes**. Lint + build OK. Plan 22 fila 22 a 95% — quedan pendientes Chat 4 (F3.FE) + Chats 5-6 (F4 BE+FE). 🟢 **Plan 29 Chat 2.6 BE ✅ cerrado 2026-04-23** (commit `7b2a962`, +21 tests, 1274 BE verdes). 🟢 **Plan 29 Chat 2.5 BE ✅ cerrado 2026-04-23** (commit `0580983`, +11 tests, 1253 verdes). 🟢 **Plan 29 Chat 2 BE ✅ cerrado 2026-04-22** (commit `674e86a`, +35 tests). 🔴 **Siguiente por prioridad**: Chat 3 OPS del Plan 29 (inspección CrossChex + negociación umbral hosting con el admin cPanel) + Chat 4 docs (§18 `business-rules.md` con `INV-MAIL01/02/03/04`). Post-deploy: 48-72h monitoreo 0 `SslHandshakeException` + contador defer/fail cPanel bajando antes de Chat 4 docs. Señal anotada para chat futuro: retry automático del worker puede ser peligroso con threshold bajo — evaluar migración a retry manual (el widget ya expone `Retrying` prominentemente para facilitar esto). 🟢 **Plan 28 Chat 3 desbloqueado** (espera validación Plan 27 post-deploy). 🟢 **Plan 27 cerrado en docs + código completo** — `INV-C11` formalizado, pendiente solo validación del jefe post-deploy. **Frentes abiertos sin bloqueo**: Plan 26 F2 Chat 2, Design System F5.3, Carril D Olas 2+.
