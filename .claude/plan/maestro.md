...
- [ ] **F3 — Lint de capas**
  ...
  - [ ] **F3.5 — Corregir violaciones destapadas + G1 + G5/G6** (lotes chat-sized)
    - [ ] F3.5.A Lote A — Components importando stores (12 archivos). Subdividido por tamaño:
      ...
      - [ ] F3.5.A.2 ctest-k6 (4 components): extender `CTestK6Facade` con pass-through methods (`updateConfigField`, `setBaseUrl`, `removeEndpoint`, `toggleEndpoint`, `updateEndpoint`, `toggleAllEndpoints`, `reorderEndpoints`, `addStage`, `removeStage`, `updateStage`, `reorderStages`, `updateStageEndpoints`, `removeCredential`, `updateRoleVUs`) y migrar los 4 components.
      - [ ] F3.5.A.3 profesor-calificaciones: exponer `CursoContenidoStore.setContenido/setSalonId` vía `CalificacionesFacade` y migrar component.
