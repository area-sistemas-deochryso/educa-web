...
| 22 | Endurecimiento correos de asistencia | BE+FE | `Educa.API/.claude/plan/asistencia-correos-endurecimiento.md` | 🟢 En progreso · Chat 1 (F1) ✅ 2026-04-21 — guard ASCII+RFC en `EmailOutboxService.EnqueueAsync` para tipos `Asistencia`/`AsistenciaProfesor` + helper `EmailValidator` + 30 tests verdes · pendiente Chat 2 (F2 SQL + `SmtpErrorClassifier`) | 17% |
...
#### Plan 22 — Endurecimiento correos de asistencia 🟢 (plan file 2026-04-21 · Chat 1 cerrado 2026-04-21 · pendiente Chat 2 `/execute`)
...
> **Estado**: plan file creado 2026-04-21 con desglose de 6 chats atómicos (4 BE + 2 FE). Secuencia sugerida F1 → F2 → F3.BE → F3.FE → F4.BE → F4.FE. Chat 1 (F1) cerrado 2026-04-21.
...
- [ ] **Chat 2 — F2** (BE + SQL, 1 chat) — Clasificación SMTP · helper `SmtpErrorClassifier` · columna `EO_TipoFallo` + índice filtrado · retry 0 para permanentes (5.1.1, 5.2.2, 5.7.x, 4.2.2) y 1 retry a 60s para transitorios (421, 4.x.x no-2.2) · política nueva SOLO para tipos de asistencia · cap 300 ln respetado en Worker
