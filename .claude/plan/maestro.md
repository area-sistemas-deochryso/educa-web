...
| **44** | **🟢 Reducción de volumen SMTP (consolidación / scheduling / opt-out)** | **BE+FE** | **(brief design abierto en `chats/open/141`, plan dedicado pendiente)** | **🟢 Plan abierto 2026-05-12. Hosting denegó subir techo `max_defer_fail_percentage` — jefe pide "soluciones desde acá". Planes 22/29/37/38/39 son defensivos; Plan 44 es ofensivo: reducir el volumen real de correos para no tocar el techo. 5 líneas de exploración (A consolidación diaria · B scheduling fuera de pico · C opt-out granular · D drop de eventos no críticos · E push como canal alternativo). `/design` produce ADR + plan dedicado + fases.** | **0%** |
