...
| **35** | **🟡 Rediseño UX/UI submódulo "Monitoreo"** | **FE** | **[plan/monitoreo-redesign.md](./monitoreo-redesign.md)** | **🟡 Plan abierto 2026-04-27. Origen: el dropdown "Sistema → Monitoreo" creció a 7 entradas planas (Errores + Bandeja + Dashboard día + Diagnóstico + Reportes Usuarios + Rate Limit + Auditoría Correos). Frase del usuario: *"Ya está creciendo mucho. Quiero un rediseño que priorice los flujos de trabajo obvios, simples y bien organizados sin perder las funciones actuales"*. Decisión cerrada: **1 sola entrada en menú "Monitoreo" → hub `/intranet/admin/monitoreo` con 3 cards (Correos / Incidencias / Seguridad) → shells con tabs internos** (Correos 4 tabs, Incidencias 2 tabs, Seguridad 1 página directa). Las 7 URLs viejas siguen vivas como redirects. **NO se toca lógica interna** de las 6 páginas existentes (sus stores, facades, services, tests). Refactor estructural puro. Alcance: 1 chat FE, ~9 archivos nuevos (~435 ln) + 2 modificados (`intranet.routes.ts` + `intranet-menu.config.ts`). Brief listo en `.claude/chats/open/053-plan-35-chat-1-fe-monitoreo-redesign.md`. **Aprovechable mientras Plan 31 Chat 2 BE está bloqueado por OPS**. No bloquea ni es bloqueado por otros planes activos.** | **5%** |
...
3. **[Plan 35 · Chat 1 · `educa-web` · FE]** — Rediseño UX/UI del submódulo "Sistema → Monitoreo": colapsar las 7 entradas planas del dropdown en **1 sola entrada** que abre un hub `/intranet/admin/monitoreo` con 3 cards (Correos / Incidencias / Seguridad). Shells con tabs internos para Correos (4 sub-páginas: Bandeja / Dashboard día / Diagnóstico / Auditoría) e Incidencias (2: Errores / Reportes Usuarios). Las 7 URLs viejas siguen vivas como redirects. **NO se toca lógica interna** de las 6 páginas (refactor estructural puro). **Aprovechable mientras el item #1 de esta cola está bloqueado por OPS**. Plan: `.claude/plan/monitoreo-redesign.md`. Brief: `.claude/chats/open/053-plan-35-chat-1-fe-monitoreo-redesign.md`. Estimado: ~9 archivos nuevos + 2 modificados, ~485 líneas total.
