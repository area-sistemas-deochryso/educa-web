...
1. **[Plan 38 · Chat 2 · `Educa.API` · BE]** 🔴 **PRIORIDAD ALTA** — `MailboxFullBlacklistHandler` + refactor `EmailBounceBlacklistHandler` (saca 4.2.2 del umbral 3 lifetime) + nuevo motivo `BOUNCE_MAILBOX_FULL` + migración SQL CHECK constraint. Brief en `.claude/chats/open/072-plan-38-chat-2-be-mailbox-full-handler.md`. Plan en `.claude/plan/blacklist-detection-admin.md`. Origen: incidente 2026-04-29 (dominio bloqueado 1h por 4.2.2 crónico repetido). **Pre-req duro**: ejecutar `Educa.API/sql/migrations/38-chat2-01-add-bounce-mailbox-full-motivo.sql` antes del deploy.
2. **[Plan 38 · Chat 3 · `Educa.API` · BE]** — `POST /api/sistema/email-blacklist` + `GET` paginado server-side + DTOs + tests controller. Brief en `.claude/chats/open/073-plan-38-chat-3-be-blacklist-crud-endpoints.md`. Depende de Chat 2 mergeado (motivo válido en BD).
3. **[Plan 38 · Chat 4 · `Educa.API` · BE]** — `BlacklistAutoCleanupJob` Hangfire diario para `INV-MAIL06`. Brief en `.claude/chats/open/074-plan-38-chat-4-be-blacklist-auto-cleanup-job.md`. Depende de Chat 2 mergeado.
4. **[Plan 38 · Chat 5 · `educa-web` · FE]** — Tab "Blacklist" en `/intranet/admin/email-outbox` con tabla server-paginated, filtros, dialog "Agregar", drawer detalle, despejar. Brief en `.claude/chats/open/075-plan-38-chat-5-fe-blacklist-tab-admin.md`. Depende de Chat 3 deployado.
5. **[Plan 38 · Chat 6 · `Educa.API`+`educa-web` · BE+FE]** — `EmailHub` SignalR + banner B9 + toast cuando `defer-fail-status >= 4/5` o `MailboxFullBlacklistHandler` dispara. Brief en `.claude/chats/open/076-plan-38-chat-6-fe-banner-b9-toast-signalr.md`. Depende de Chat 5 mergeado + Plan 37 Chat 3 (068).
6. **[Plan 39 · Chat A · `Educa.API` · BE]** 🔴 **PRIORIDAD ALTA** — `EmailMonitoreoService` + 5 endpoints dashboard (sender-stats, top-destinatarios, serie-temporal, dominios-receptores, candidatos-blacklist) con caché `IMemoryCache` por método + fail-safe INV-S07 + 12 tests con seed reproducible del incidente 2026-04-20→29 + script SQL `plan39_chat2_AddDashboardIndex.sql` (índice covering `IX_EmailOutbox_FechaReg_Estado_TipoFallo`). Brief en `.claude/chats/open/077-plan-39-chat-a-be-monitoreo-endpoints.md`. Plan en `.claude/plan/monitoreo-empirico-mejoras.md`. **Pre-req duro**: ejecutar el script SQL antes del deploy. Independiente de Plan 38 (puede ejecutar en paralelo).
7. **[Plan 39 · Chat B · `Educa.API` · BE]** — `EmailHub` SignalR (server-side) + `SignalREmailHubNotifier` real (reemplaza `NullEmailHubNotifier` registrado por Plan 38 Chat 2) + dispatch de 3 eventos (`BlacklistEntryCreated`, `DeferFailStatusUpdated`, `CandidatoBlacklistDetectado`) + invalidación de caché `defer-fail-status` antes de pushear. Brief en `.claude/chats/open/078-plan-39-chat-b-be-emailhub-signalr-push.md`. **Pre-req duro**: Plan 38 Chat 2 (072) deployado. Conviene Chat A 077 mergeado antes (no duro).
8. **[Plan 39 · Chat C · `educa-web` · FE]** — Tab "Mapa de envío" en `/intranet/admin/monitoreo/correos/dashboard` con 6 tiles (defer-fail-live, sender-mix, top-destinatarios, serie-temporal, dominios-receptores, candidatos-blacklist) + skeletons independientes + cross-links "Bloquear" a tab Blacklist con `?correo=` prefill + listener SignalR de los 3 eventos del hub. Brief en `.claude/chats/open/079-plan-39-chat-c-fe-dashboard-monitoreo-tiles.md`. **Pre-req duro**: Chat A (077) deployado + Plan 38 Chat 5 (075) deployado.
9. **[Plan 39 · Chat D · `educa-web` · FE]** — Banner B9 reusable cross-páginas (`/monitoreo/correos/*` 4 sub-páginas) + tile "Aceptado vs Entregado" en auditoría + alineación del listener SignalR de Plan 38 Chat 6 (076) con los 3 eventos del hub. Brief en `.claude/chats/open/080-plan-39-chat-d-fe-banner-b9-cross-paginas-auditoria-tile.md`. **Pre-req**: Chat B (078) + Chat C (079) + Plan 38 Chat 6 (076) deployados.
...
12. **[Plan 37 · Chat 2 · `Educa.API` · BE]** — cuarentena temporal + pause por dominio receptor consumiendo `EmailDeferEvent` del Chat 1. Brief en `.claude/chats/open/067-plan-37-chat-2-be-quarantine-domain-pause.md`. **Desbloqueable** tras verify de 066 + 069 (telemetría real entrando a `EmailDeferEvent`).
13. **[Plan 37 · Chat 3 · `educa-web` · FE]** — visibilidad admin de cuarentena en `/intranet/admin/email-outbox`. Brief en `.claude/chats/open/068-plan-37-chat-3-fe-quarantine-admin-visibility.md`. Depende del Chat 2.
