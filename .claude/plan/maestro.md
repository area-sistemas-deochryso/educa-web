...
1. **[Plan 31 · Chat 2 · `Educa.API` · BE]** — Parser IMAP + Hangfire job + tabla `BounceParserProcessed`. `BounceParserService` + partial `.Imap.cs` + `Ndr3464Parser` + `BounceCorrelator` + `BounceParserJob` recurring `*/5 * * * *`. Procesa `INBOX`/`Junk`/`Trash` de los 7 buzones del pool Plan 22 F6. **Promovido a #1 tras cierre Plan 31 Chat 1 (2026-04-24, commit `c46dfa0`)**. Recomendable esperar 24-48h post-deploy del Chat 1 para acumular correos en prod con `X-Educa-Outbox-Id` antes de validar correlator con NDRs reales.
2. **[Plan 30 · Chat 3 · `Educa.API` · BE]** — Gap asistencia-vs-correos: endpoint `GET /api/sistema/asistencia/diagnostico-correos-dia`. **Priorizado tras Plan 31** porque consume `EmailOutbox` — hoy ese dato miente ("SENT" para correos que rebotaron async). Con Plan 31 desplegado el reporte será confiable.
3. **[Plan 30 · Chat 4 · `Educa.API` · BE]** — Búsqueda diagnóstico por correo específico: endpoint `GET /api/sistema/email-outbox/diagnostico?correo={email}`. Cierra el frente de visibilidad admin sobre el outbox (M1-M8 del Plan 30).

**Notas**:

- 🟢 **Plan 31 Chat 1 BE ✅ cerrado 2026-04-24** (commit `c46dfa0` en `Educa.API master`) — foundation desplegable: header `X-Educa-Outbox-Id` activo + handler split (sync/async) + columnas BD ya escribiéndose desde el worker. 24/24 tests del scope verde + 1336 BE verdes (sin regresiones). Brief movido a `closed/`. Chat 2 desbloqueado.
...
- **Plan 24 Chat 4 BE estancado** — validación rate limit + `Task.Delay(30000)` queda fuera del top 3 hasta que se reactive (decisión 2026-04-24 del usuario).
