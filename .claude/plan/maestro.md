...
| **29** | **🔴 Corte de cascada SMTP (`max_defer_fail_percentage`)** | **BE+OPS** | **(inline en maestro — 4 chats, no justifica archivo dedicado)** | **🟢 Chat 2 BE ✅ cerrado 2026-04-22 (commit `674e86a` en Educa.API `master`). 3 scripts SQL ejecutados en Azure (tabla `EmailBlacklist` vacía, `EmailOutbox_Archive` 0 archivados consistente con Query A = 0, sanitize 13 FAILED + 2969 SENT sin purga). Código entregado: `EmailHelper.Mask` + `EmailValidator.Normalize` helpers puros · entity `EmailBlacklist` + `EmailBlacklistConfiguration` (unique filtrado activos + covering index lookup) · repo + service con normalización en el borde + upsert idempotente · `DELETE /api/sistema/email-blacklist/{correo}` con `[Authorize(Roles = Roles.Administrativos)]` (4 roles, no solo 2) · hook `EnqueueAsync` rechaza blacklisted silencioso con `LogWarning(Mask)` sin crear fila (INV-MAIL01) · `EmailBounceBlacklistHandler` en worker: tras 3 bounces `5.x.x` INSERT blacklist + `EO_Estado='FAILED_BLACKLISTED'` atómico en el mismo `SaveChanges` (sin `TransactionScope` explícito — equivalente funcional); guards SSL/auth/`exceeded the max defers` → `FAILED_THROTTLE_HOST` sin imputar (INV-MAIL02) · TLS fix: **divergencia técnica del brief** — `StartTls` no aplica en puerto 465 SMTPS, se implementó `SslProtocols.Tls12 \| Tls13` forzado manteniendo `SslOnConnect`. Feature flag `Email:TlsStrictMode` (NO `EmailService:*`) default `true`. **+35 tests** nuevos en 7 archivos → **1242 BE verdes** (target 1220 superado por 22). Respetado límite 300 líneas via partial `EmailOutboxService.Enqueue.cs`. Gaps conocidos → próximo chat BE: (a) validación de formato sigue restringida a tipos `Asistencia`/`AsistenciaProfesor` — tipos no-asistencia con formato inválido aún llegan al SMTP y consumen cuota; (b) agregados `DeferFailStatus` para widget Plan 22 Chat B (D4) NO quedaron expuestos como endpoint — la UI admin podrá contar vía query directa mientras tanto. 🟢 Chat 1 `/design` ✅ cerrado 2026-04-22. **Siguiente: Chat 3 OPS (inspección CrossChex + negociación umbral hosting)** + Chat 2.5 BE fix (extender validación formato a todos los tipos + exponer agregados `DeferFailStatus`).** | **~55%** |
...
**Foco (actualizado 2026-04-22, post-cierre Plan 29 Chat 2)**: 🟢 **Plan 29 Chat 2 BE ✅ cerrado** (commit `674e86a` en Educa.API `master`). 3 scripts SQL ejecutados en Azure. Código: helpers puros · entity+repo+service+controller blacklist · hook silencioso en `EnqueueAsync` · `EmailBounceBlacklistHandler` en worker con guards SSL/auth/throttle + auto-blacklist atómico al 3er bounce 5.x.x · TLS fix `SslProtocols.Tls12 \| Tls13` forzado (divergencia: `StartTls` no aplica en port 465 SMTPS — se mantiene `SslOnConnect`). Feature flag `Email:TlsStrictMode=true`. **+35 tests** → **1242 BE verdes** (target 1220). **Gaps detectados para próximo chat BE**: (a) validación de formato sigue restringida a `Asistencia`/`AsistenciaProfesor` — tipos no-asistencia con formato inválido aún cruzan al SMTP; (b) agregados `DeferFailStatus` (D4) no expuestos como endpoint — el widget Plan 22 Chat B necesita que se materialicen. 🔴 **Siguiente por prioridad**: Chat 3 OPS (inspección CrossChex + negociación umbral hosting con el admin cPanel) + Chat 2.5 BE (cerrar los 2 gaps anteriores en un solo chat corto). Post-deploy: 48-72h monitoreo 0 `SslHandshakeException` antes de Chat 4 docs. 🟢 **Plan 28 Chat 3 desbloqueado** (Plan 29 Chat 2 ya tocó `EmailOutboxService` y merge limpio — se puede retomar sin colisión de PRs). 🟢 **Plan 27 cerrado en docs + código completo** — `INV-C11` formalizado, pendiente solo validación del jefe post-deploy. **Frentes bloqueados por Plan 29**: Plan 22 Chat B (widget `DeferFailStatus`) sigue bloqueado hasta Chat 2.5 BE materialice los agregados. **Frentes abiertos sin bloqueo**: Plan 26 F2 Chat 2 (aplicar `[RateLimitOverride]` a 14+2 endpoints — dominio HTTP distinto), Design System F5.3, Carril D Olas 2+.
...
> **Estado**: 🟢 **Chat 2 BE ✅ cerrado 2026-04-22** (commit `674e86a`, +35 tests, 1242 BE verdes). 🟢 Chat 1 `/design` ✅ cerrado 2026-04-22. Siguiente: Chat 3 OPS + Chat 2.5 BE (fix gaps: validación formato a todos los tipos + endpoint `DeferFailStatus` para Plan 22 Chat B).
...
| D4 | Widget Plan 22 Chat B incluye `DeferFailStatus` además de `ThrottleStatus` (métricas defer/fail 5/h por dominio) | **Chat 2 BE ✅ dejó la BD preparada pero NO expuso endpoint** — `EmailOutbox` ahora tiene estados `FAILED_BLACKLISTED` y `FAILED_THROTTLE_HOST` + `EmailBlacklist.EBL_IntentosFallidos` queryable. El widget FE puede consultar por query directa hoy, pero el service/endpoint `GET /api/email-outbox/defer-fail-status` con agregados empaquetados queda para **Chat 2.5 BE** (junto con el fix de validación de formato a todos los tipos). |
...
**Gaps conocidos → cerrar en Chat 2.5 BE** (detectados en cierre del chat, no bloquean deploy):

1. La validación de formato sigue gobernada por el whitelist `TiposValidadosAlEncolar = {Asistencia, AsistenciaProfesor}` del Plan 22 F1. Tipos `NotificacionAdmin`, `ReporteUsuario`, `ReporteFallosCorreoAsistencia`, etc., aún encolan PENDING con formato inválido y consumen cuota cPanel al fallar en el SMTP. **Fix**: quitar el whitelist, validar siempre. Cambio additivo, ~15 min + tests.
2. Los agregados `DeferFailStatus` de la deuda D4 (widget Plan 22 Chat B) no quedaron expuestos como endpoint. Los contadores existen en `EmailOutbox` (`EO_Estado IN ('FAILED_BLACKLISTED','FAILED_THROTTLE_HOST','FAILED')` + `EO_TipoFallo`), pero falta el service/endpoint que los empaquete. Hoy la UI admin puede leer via query directa; para el widget FE necesita materializarse.
