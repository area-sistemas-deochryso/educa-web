...
| xP107 | → Entorno de desarrollo orientado a pruebas (coord P107) | F1 ✅ completo y verificado en vivo (618+617), bug lateral 619 ✅ cerrado. **F2 FE ✅ shipped y verificado en vivo** ([621](../chats/closed/621-fe-p107-f2-crosschex-trigger-outbox-link.md), 2026-09-03) — trigger + enlace a Email Outbox confirmados end-to-end (F2 BE 620 ya había cerrado). Hallazgo nuevo derivado: el correo dry-run no se encoló para el alumno de prueba usado — brief BE [624](../../../Educa.API/.claude/chats/open/624-be-asistencia-email-no-encolado-silencioso.md). F3 BE Salones+Cursos ✅ shipped ([622](../../../Educa.API/.claude/chats/closed/622-be-p107-f3-bulk-create-endpoints.md)). **F3 FE Salones+Cursos ✅ shipped y verificado en vivo** ([623](../chats/closed/623-fe-p107-f3-bulk-create-ui.md), 2026-09-03, worktree `chat/623-fe-p107-f3-bulk-create-ui`) — generación sintética + import de archivo confirmados end-to-end contra `TestConnection` real (5 cursos sintéticos + 2 vía import, marcado `::TEST` aplicado solo por el BE). 2 bugs de UI encontrados y corregidos en vivo: `errores` del backend no se mostraban en el resultado de generación, y label con capitalización indebida. Hallazgo derivado fuera de scope: `/intranet/admin/cursos` no invalida su stats/búsqueda tras altas nuevas — sin brief todavía. F3 BE Usuarios 🔲 pendiente, partido a [625](../../../Educa.API/.claude/chats/open/625-be-p107-f3b-bulk-create-usuarios.md) (`Educa.API/open/`) — F3 FE Usuarios queda como brief de seguimiento cuando 625 cierre | ver P107 |
...
- **`running/`**: vacío · **`open/`**: 2 briefs (577, 598) · **`awaiting-prod/`**: 8 briefs (462, 554, 559, 578, 588, 600, 601, 605) · **`waiting/`**: vacío · **`troubles/`**: vacío
