...
| **21** | **🟡 Asistencia de Profesores en CrossChex** | **BE+FE** | **`plan/asistencia-profesores.md`** | **✅ Chat 1 + Chat 1.5 + Chat 2 + Chat 3 + Chat 4 + Chat 6 cerrados (2026-04-20) · Chat 6 (armonización UX "Mi asistencia"): store extendido con `viewMode`+`selectedDate`, facade con `loadDia`/`setViewMode` consumiendo `/profesor/me/dia` existente. Vista propia ahora embebe `<app-attendance-legend />`, responde al pill día/mes del header cross-role (shell delega a `propiaComponent.setViewMode`), y usa `severity` coherente con el resto del módulo (stat-cards solo en modo mes). Trade-off: NO se reusa `AttendanceTableComponent`/`AttendanceDayListComponent` porque su shape de datos (`EstudianteAsistencia[]`) no se adapta a una lista cronológica de días de un solo profesor — se homologó styling sin duplicar componentes. Backend: 766 tests verdes (sin cambios). Frontend: 1341 tests verdes (+1 test nuevo `setViewMode delega a propia`), lint + tsc + build limpios. **Deuda UX del Chat 4 resuelta**. **Deuda técnica lateral** (chats futuros): `PermisoSaludAuthorizationHelper.cs:63` anti-pattern `DIR_DNI == dni`; `ErrorLog` en prueba le faltan 3 columnas. Deploy pendiente: `plan21_chat15_FkRepointAsistenciaPersona.sql` (Chat 5)** | **~90%** |
