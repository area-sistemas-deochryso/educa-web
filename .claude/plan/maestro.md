...
6. **[Plan 39 · Chat 1 · `Educa.API`+`educa-web` · design]** — Diseño cerrado del monitoreo basado en evidencia (dashboard 6 tiles, 5 SQL agregaciones, decisión Chat 4 SSH). Brief en `.claude/chats/running/071-plan-39-chat-1-monitoreo-empirico-design.md`. Plan en `.claude/plan/monitoreo-empirico-mejoras.md`. **Hilo en running** — se trabaja en chat aparte para no mezclar con Plan 38 ejecución.
7. **[Plan 24 · Chat 4 (B) · `Educa.API` · BE+OPS]** — subfase (A') cerrada 2026-04-24 con `CrossChexPollingOptions` + instrumentación Stopwatch (commit `862d4ca`). Falta (B): medir 48-72h de logs en prod (`ConfiguredMs / ElapsedMs / Drift`) y, si el proveedor lo permite, bajar `CrossChex:Polling:DelayBetweenPagesMs` de 30000 a 10000-15000 vía Azure App Configuration (sin redeploy). Cierra Plan 24 al 100%. **Postpuesto** por prioridad de Plan 38.
...
4. _(Plan 37 Chat 1 cerrado local 2026-04-29 — `awaiting-prod/066` esperando ejecución de SQL + 1er ciclo con NDR delayed real.)_
5. _(Plan 36 Chat 5 cerrado local 2026-04-27 — `awaiting-prod/` esperando smoke test del menú Auditoría en prod.)_
