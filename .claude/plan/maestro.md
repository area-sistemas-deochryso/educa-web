...
| **36** | **🟡 Rediseño UX/UI páginas internas de Monitoreo** | **FE+BE** | **[plan/monitoreo-pages-redesign.md](./monitoreo-pages-redesign.md)** | **🟡 Chat 7 FE (#6 Reportes de usuarios — métricas) ✅ cerrado local 2026-04-28 (awaiting-prod). Chat 4 FE (#3 Diagnóstico typeahead) 🔁 v1 rolled back 2026-04-28 (typeahead funcionaba con texto pero no con DNI numérico — submit caía en `CORREO_INVALIDO` engañoso) → fix v2 ✅ cerrado local 2026-04-28 (awaiting-prod): submit smart cuando input no tiene `@` → 1 sugerencia autopick · >1 toast warning · 0 mensaje específico DNI vs texto. Iconos por tipo persona (graduation-cap/book/shield/users), badge tipo color-coded espejo del avatar, item template a 3 líneas. **6 archivos**, **+5 tests**. Lint ✅ · build ✅ · vitest **1694/1694 verdes**. Chat 2 (#1 Bandeja) ✅ cerrado local 2026-04-27 (awaiting-prod) — 9 archivos tocados (`styles.scss` global agrega override `p-tabs` transparente; stats con tokens design-system; Excel a `p-button-success` filled; layout grid stats+chart side-by-side >1100px; filters con labels uppercase + placeholders distintivos; paginador `rows=5 / [5,10,15]`; eliminación de overrides de transparencia local ahora cubiertos por global). Lint ✅ · build ✅ · tests 1682/1683 (1 fail preexistente en `attendance-scope-banner` no relacionado, verificado contra `git stash`). **Plan 36 Chat 3 (#2 Dashboard tab Detalle) ✅ cerrado local 2026-04-27 (awaiting-prod)** — 4 KPI cards arriba del tab Detalle (Fallos · Bouncers únicos · Próximos a blacklist ≥3 · Tipos afectados) con variants success/danger/warning/info y zero-state, mirror del estilo de `dashboard-resumen`. Sin tocar facade/store/sub-tablas. 3 archivos modificados (`email-outbox-dashboard-dia.component.{ts,html,scss}`). Lint ✅ · tsc ✅ · vitest 19/19 ✅ · build ✅. **Plan 36 Chat 4b BE (#3 Diagnóstico — endpoint `buscar-personas`) ✅ cerrado local 2026-04-28 (awaiting-prod)** en `Educa.API master`. Nuevo endpoint `GET /api/sistema/email-outbox/diagnostico/buscar-personas?q={texto}` para typeahead del FE Chat 4: lookup polimórfico libre en las 4 tablas de persona (Estudiante/Profesor/Director/Apoderado) por `_Nombres LIKE %q%` OR `_Apellidos LIKE %q%`, con búsqueda adicional por `*_DNI_Hash == ComputeHash(q)` cuando `q` son 8 dígitos. Cap 10 por tabla → 10 final ordenado por apellido. Response devuelve correo plano + masked + DNI masked. Filtra `_Estado = true` (INV-D09) + correo no-vacío. INV-S07 fail-safe. Reusa patrón sibling de `EmailDiagnosticoPersonaLookup` (Plan 30 Chat 4) — nuevo `EmailDiagnosticoPersonaSearch.cs` (216 ln) + service convertido a `partial class` (`EmailDiagnosticoService.cs` 293 ln + `*.Busqueda.cs` 53 ln) para respetar cap 300. **+12 tests** (4 tipos persona + DNI hash + cap 10 + sin correo + inactivo + normalización + INV-S07) → **1482 BE verdes** (baseline 1470). Decisión revisada en pre-work SQL: el modelo expone `EST_DNI_Hash`/`PRO_DNI_Hash`/`DIR_DNI_Hash`/`APO_DNI_Hash` con SHA256, así que DNI search se incluyó en scope (originalmente brief lo dejaba como pregunta abierta). Limitación conocida documentada: búsqueda accent-sensitive (CI_AS de SQL Server + `.ToLower().Contains` de C# son ambos accent-sensitive) — "perez" no matchea "Pérez"; admin debe tipear con tilde si el dato la tiene. Follow-up menor: aplicar `EF.Functions.Collate(field, "Latin1_General_CI_AI")` si hay fricción. **Plan 36 Chat 6 FE (#5 Trazabilidad Kanban completar) ✅ cerrado local 2026-04-28 (awaiting-prod)** — flipear default `_hideResolvedIgnored: true → false` para que las 5 columnas (NUEVO/VISTO/EN_PROGRESO/RESUELTO/IGNORADO) se vean por defecto en el Kanban. Plumbing del Plan 34 ya tenía transitions map (INV-ET07) y predicates de drag-drop completos — el bug era solo el default ON del toggle filtraba RESUELTO/IGNORADO antes del render. Grid CSS `repeat(5, ...)` fijo → `grid-auto-flow: column + grid-auto-columns: minmax(240px, 1fr)` adaptativo (3 cuando toggle ON, 5 cuando OFF). 3 archivos: `error-groups.store.ts` (default + clearFilters), `error-groups-kanban-board.component.scss` (grid adaptativo), `error-groups.store.spec.ts` (+2 tests: default false + 5 estados visibles). Lint ✅ · tsc ✅ · build ✅ · vitest **1684/1685 verdes** (1 fail preexistente attendance-scope-banner no-relacionado). Tab transparente ya cubierto globalmente desde Chat 2 — verificado, sin fix adicional. **Plan 36 Chat 4 FE (#3 Diagnóstico — typeahead) ✅ cerrado local 2026-04-28 (awaiting-prod)** — `p-autoComplete` reemplaza el input simple del header del tab "Diagnóstico por correo" en `email-outbox-diagnostico/tab-correo-individual/`. Consume el endpoint `GET /api/sistema/email-outbox/diagnostico/buscar-personas?q={texto}` cerrado en Chat 4b BE (058). Pipeline en facade: `Subject` + `debounceTime(300)` + `distinctUntilChanged` + `switchMap` + `catchError → of(null)` (fail-safe INV-S07) — typeahead nunca rompe la página ni bloquea el submit libre. `forceSelection=false` permite seguir buscando por correo exacto sin elegir item; `seleccionarPersona` autosubmite el diagnóstico al click. Item template visual rico: avatar 36×36 con tinte por tipo persona (E azul, P verde, D púrpura, APO amarillo via `color-mix(in srgb, var(--X-500) 14%, transparent)`) + doble línea (nombre+tag tipo / correo enmascarado · DNI · campo origen). Footer "refiná tu búsqueda" cuando `total ≥ 10` (BE cap, honesto sobre el límite). 11 archivos tocados (models +`PersonaConCorreoDto`/`BuscarPersonasResponseDto`, service +`buscarPersonas`, store +state typeahead +`clearSugerencias`, facade +pipeline +`onTypeaheadQuery`/`seleccionarPersona`, correo-header ts/html/scss reescritos, tab parent wire-up, 2 specs ampliadas). **+5 tests** (1 service contract, 4 facade: < 2 chars no llama BE, debounce + happy path, seleccionarPersona autosubmit, error fail-safe). Lint ✅ · build ✅ · vitest **1689/1690 verdes** (1 fail preexistente `attendance-scope-banner` no-relacionado, mismo que en Chats 5/6 previos). Aprendizaje transferible: `fakeAsync` de Angular no funciona en Vitest del proyecto — usar `vi.useFakeTimers() + vi.advanceTimersByTime()` para tests de pipelines RxJS con `debounceTime`. Pendiente verificación post-deploy: smoke test browser → typear "gar" → ver typeahead, click sugerencia → autosubmit, typear DNI 8 dígitos → ver match, typear correo libre + Enter → submit directo. **Plan 36 Chat 7 FE (#6 Reportes de usuarios — métricas) ✅ cerrado local 2026-04-28 (awaiting-prod)** — 5 stat cards de `/intranet/admin/monitoreo/incidencias/reportes` migradas al patrón B3 del design-system (content-left `.stat-content` con label + valor + sublabel + icon-right 48×48 `--surface-200`). HTML reescrito con PrimeIcons semánticos (`pi-inbox` Total · `pi-flag` Nuevos · `pi-clock` En Progreso · `pi-check-circle` Resueltos · `pi-times-circle` Descartados). SCSS migrado de `.stats-grid` a `.stats-section` (grid `auto-fit minmax(200px, 1fr)`, sin background blanco), variantes con `border-left` + color del ícono coordinado por severidad (`--blue-800` / `--yellow-700` / `--green-600` / `--surface-500`), responsive `1fr 1fr` y `1fr` en mobile. Restricción dura respetada: tabla, drawer, filtros y lógica intactos — solo cards. 2 archivos modificados (`feedback-reports.component.html`, `feedback-reports.component.scss`). Lint ✅ · build ✅ · sin specs del componente (skip tests). Pendiente verificación post-deploy: visual del bloque de cards en browser. **Plan 36 Chat 8 FE (#7 Rate Limit Events — intentos vs umbral) ✅ cerrado local 2026-04-28 (awaiting-prod)** — columna nueva "Intentos / Umbral" en `<app-rate-limit-table>` con formato `tokensConsumidos / limiteEfectivo` + highlight rojo (`var(--red-600)` + bold) cuando `consumidos >= efectivo`. Tooltip explicativo en el `<th>` ("Intentos consumidos / umbral del policy en su ventana"). Drawer detalle reorganizado: 2 filas separadas ("Límite efectivo" + "Tokens consumidos") fusionadas en 1 fila combinada con la misma semántica visual. Skeleton actualizado (+1 columna `text 90px`). Pre-work confirmó que `RateLimitEventListaDto` BE ya devolvía `tokensConsumidos` + `limiteEfectivo` — **no requirió Chat 8b BE**. Lapso temporal queda implícito en el nombre del policy (login=10/min, biometric=30/min, etc) — sin campo nuevo. 5 archivos modificados: `rate-limit-table.component.{html,scss}` · `rate-limit-detail-drawer.component.{html,scss}` · `rate-limit-events.component.ts` (skeleton). Lint ✅ · build ✅ · vitest 1688/1690 (2 fail preexistentes en `attendance-scope-banner` no-relacionados, mismos que en Chats 4/6/7). Pendiente verificación post-deploy: visual de la columna nueva con datos reales en `/intranet/admin/monitoreo/seguridad/rate-limit`. **Plan 36 a ~100% (Chats 1+2+3+4+4b+5+6+7+8 de 9 cerrados)**. Pendiente verificación post-deploy: smoke test del nuevo endpoint con `q=garcia`, `q=12345678` (DNI hash), `q=""` (400), antes de desbloquear Chat 4 FE (059). Plan creado 2026-04-27 — continuación de Plan 35. Alcance: las 7 sub-páginas del submódulo Monitoreo (Bandeja / Dashboard día / Diagnóstico / Auditoría / Trazabilidad de Errores / Reportes de Usuarios / Rate Limit). El plan **arranca con Chat 1 `/investigate`** que pregunta al usuario los problemas concretos de diseño de cada página (3 preguntas guía por página) y produce decisión binaria ✅ rediseñar / 🟡 ajuste menor / ⏭️ no tocar. Cantidad y agrupación de chats de ejecución se decide DESPUÉS del Chat 1 según la respuesta del usuario (heurística: 1 chat/página si profundo, 1 chat/dominio si 3+, 1 chat de polish global si solo cambios visuales menores). Restricción dura: ningún chat toca funcionalidad — solo HTML/SCSS y refactor presentacional. Riesgo principal: Trazabilidad de Errores recién rediseñado en Plan 34 — el Chat 1 lo pregunta explícitamente, ⏭️ obligatorio si está bien. Reversibilidad 100%. No bloquea ni es bloqueado por otros planes.** | **100%** |
| **40** | **🟡 Load Control Layers (concurrency + bulkheads + timeouts + backpressure)** | **BE** | **(inline en maestro — chat 096 cerrado el `/adr+/design`)** | **🟡 Plan abierto 2026-05-05. F1 ✅ cerrado local 2026-05-05 (`awaiting-prod/103`, commit `Educa.API@8aa99ec`) — chained `rate + concurrency:global` con N=140 + 429/503 distinguidos + telemetría 503 + 2 tests integración. F2 ✅ cerrado local 2026-05-05 (`awaiting-prod/104`) — 5 bulkheads `concurrency:{pagos,reports,notif,uploads,bio}` con caps 15/8/15/10/20 configurables, 8 controllers anotados (mix class/method por `AllowMultiple=false`), 6 tests integración (saturación + aislamiento), suite 1647/1647. Desbloquea F2-F6.** Chat 096 (`/adr` → `/design`) ✅ cerrado: 6 ADRs en `Educa.API/.claude/decisions/0001-0006` con paper trail completo de cada número. Briefs F1-F5+F6a+F6b en `chats/open/103-109`. Decisiones clave: (1) modelo de 6 capas (rate limit + concurrency + bulkheads + timeouts + backpressure + resilience HTTP), capa 4 cola descartada por overlap con WAL del FE; (2) **híbrido pragmático** — handcrafted in-process (`SemaphoreSlim`/`RateLimiter` built-in para capa 2-3) + Polly 8 (`Microsoft.Extensions.Http.Resilience`) solo para HttpClient externos (BlobStorage/CrossChex/JaaS/MailKit con caveat); (3) capa 2-3 vía extensión de `RateLimitingExtensions.cs` con `ConcurrencyLimiter` paralelo a `FixedWindow` — reutiliza telemetría existente Plan 36 Chat 8c; (4) **N global = 140** = `MaxPoolSize 200 × headroom 0.7`, justificación granular: 60 conexiones reservadas para Hangfire (~5-10) + MinPoolSize warm-up (12) + SignalR persistentes (5-10) + outbox sync (2) + diagnostics (5) + emergencias (~20-30); validación contra carga proyectada 12m con features write-heavy (chat/mensajería/uploads/cursos/foro): peor combinado proyectado 100-160 → 140 cubre con margen para Default; (5) **5 bulkheads + Default implícito**: `pagos`=15 (cierre/aprobación irreversible INV-T01/INV-AD03), `reports`=8 (50% del rate limit teórico, latencia 1-3s), `notif`=15 (chat/mensajería/SignalR/outbox, +5 vs propuesta original por features write-heavy), `uploads`=10 (NUEVO, deadline tareas worst case 50-100 simultáneos → reintento ordenado), `bio`=20 (NUEVO, webhook CrossChex externo no controlable), Default sin cap propio (recibe 72-130 según ocupación); (6) timeouts default 30s HttpClient + 30s EF (60s en reportes pesados) + propagación incremental de `CancellationToken` en 5 repos críticos (ConsultaAsistencia, AsistenciaAdmin, ReporteAsistencia, EmailMonitoreo, UsuariosQuery); (7) backpressure 503 con `Retry-After = max(1, ceil(p95 × 1.5))` calculado dinámicamente desde Plan 102 RuntimeHealth, fallback 5s; reportes NO se degradan (rechazo limpio, no respuesta parcial). **Plan de ejecución**: F1 (chat 103) concurrencia global → F2 (104) bulkheads → F3 (105) timeouts → F4 (106) backpressure → F5 (107) resilience HTTP → F6a (108) calibración sintética k6 → F6b (109) calibración real prod 30d. F2/F3 paralelizables tras F1. F6b en HOLD (espera 30d datos prod).** | **~5%** |
...
1. _(Plan 40 · F1 · BE cerrado local 2026-05-05 — `awaiting-prod/103` esperando deploy del BE + smoke. Local commit `Educa.API@8aa99ec`. `GlobalLimiter` ahora es chained `rate + concurrency:global` con N=140 (default) configurable vía `ConcurrencyLimits:Global` sin redeploy. `OnRejected` distingue 429 (rate) vs 503 (concurrency) leyendo `KindItemKey` stashado por la factory. Telemetría `RateLimitTelemetryMiddleware` ahora captura 429+503 y resuelve policy desde `PolicyItemKey` (concurrency:global no lleva `[EnableRateLimiting]`). `StashRateLimit` helper para que las named policies (login/refresh/biometric/feedback/heavy/reports/batch) sobre-stasheen `kind=rate` y no sean confundidas con concurrency. 2 tests integración nuevos (saturación N=3 → 503+Retry-After · regresión 250 GETs → 429 sin 503 espurio). Suite total **1641/1641 verdes**. Desbloquea F2-F6.)_
2. _(Plan 40 · F2 · BE cerrado local 2026-05-05 — `awaiting-prod/104` esperando deploy del BE + validación prod (saturar `/api/AprobacionEstudiante/masivo` con N+ε → confirmar 503+Retry-After · telemetría `RateLimitEvents` registra los 5 nuevos `kind=concurrency` · `ConcurrencyLimits:Reports` ajustable en Azure App Configuration sin redeploy). Local commit `Educa.API@bae4983`. Brief con resultado completo + aprendizaje transferible. 5 policies `concurrency:{pagos,reports,notif,uploads,bio}` en `RateLimitingExtensions.cs` (caps 15/8/15/10/20 default vía `IConfiguration`), helper privado `AddBulkhead`. 8 controllers anotados (mix class/method según target homogéneo o mixto). **Aprendizaje**: `EnableRateLimitingAttribute.AllowMultiple=false` impide stack en mismo target — el "Plan B" del brief (policy combinada) NO fue necesario porque class+method son targets CLR distintos y .NET runtime los acumula. Regla práctica: bulkhead a class-level si el controller es homogéneo, method-level si es mixto. 6 tests integración (`Plan40F2BulkheadIntegrationTests`) — saturación por bulkhead Theory×5 + aislamiento reports↔pagos. Suite total **1647/1647 verdes** (+6 sobre baseline F1 1641). Deuda anotada: SignalR Hubs (`ChatHub`/`AsistenciaHub`/`EmailHub`) no aceptan `[EnableRateLimiting]` directo — capa 3 para hubs queda para F3 vía middleware. Doc: sección "Clasificación de endpoints por bulkhead" en `educa-web/.claude/rules/backend.md`. Desbloquea F6a (calibración sintética k6).)_
3. _(Plan 40 · F3b · BE cerrado local 2026-05-05 — `awaiting-prod/110` esperando deploy del BE + smoke (cancelación libera EF inmediato bajo carga · UNION ALL paginado de usuarios + reportes admin honran timeout 60s en SQL Server real). Local commit `Educa.API@<pendiente>` + FE doc commit `educa-web@<pendiente>`. Cierre del scope split de F3a: 4 repos restantes (`ConsultaAsistencia` 11 firmas + 3 SetCommandTimeout · `AsistenciaAdminRepository` partials con SetCommandTimeout en `ListarAsistenciasDelDia` y `CalcularEstadisticasDelDia` · `UsuariosRepository` 11 firmas de lectura + 2 SetCommandTimeout · `EmailMonitoreoService` 5 Load*Async con SetCommandTimeout) propagaron `CancellationToken cancellationToken = default` y SetCommandTimeout(60) donde aplica. Helper nuevo `Helpers/Extensions/DatabaseFacadeExtensions.SetCommandTimeoutIfRelational` para no romper InMemory tests (provider InMemory lanza `InvalidOperationException` en `SetCommandTimeout` real). Aplicado retroactivo a F3a `ReporteAsistenciaRepository`. Test `Plan40F3CancellationTests` (3/3 verde) valida `cts.Cancel()` antes de query → `OperationCanceledException` + recuperación del DbContext. 3 tests existentes ajustados con `It.IsAny<CancellationToken>()` en Mock Setups (Moq no tolera default param en Expression tree, error `CS0854`). Doc nueva sección "Timeouts y CancellationToken" en `educa-web/.claude/rules/backend.md` con política, patrón, cobertura, reglas operativas y referencia al test. **Deuda anotada**: `ConsultaAsistenciaController` + `IConsultaAsistenciaService` no propagan CT (cascade no-trivial); `IUsuarioQueryStrategy` tampoco (5+ archivos). Documentado como "⏳ Parcial" / "❌ Ninguno" en backend.md. **Aprendizaje**: Mock setups con `Expression<Func<>>` rompen al agregar param con default — fix mecánico (`It.IsAny<CancellationToken>()`) que conviene anticipar al planear cascade de CT. Suite total **1650/1650 verdes** (+3 sobre baseline F2 1647). Desbloquea F5 (Polly).)_

3a. _(Plan 40 · F3a · BE cerrado local 2026-05-05 — `awaiting-prod/105` esperando deploy del BE + smoke (timeouts default 30s en `WhatsAppService` y `CrossChexApiService` no rompen integraciones · reporte asistencia día/rango bajo cliente que cierra conexión libera EF inmediato). Local commit `Educa.API@d0a6b46`. **Scope split mid-chat**: brief original asumía F3 entero (5 repos × cascade CT + test + doc). Realidad N×LOC ~1500 entró en zona de riesgo de error 500. Cerrado parcial con `Extensions/ServiceExtensions.cs` consolidando `AddHttpClient<>(c => c.Timeout = 30s)` + `ReporteAsistenciaRepository` (interface + impl) con `CancellationToken cancellationToken = default` propagado a `.ToListAsync()` / `.FirstOrDefaultAsync()` y `SetCommandTimeout(60)` en las 2 queries pesadas (`ObtenerEstudiantesConAsistenciaDiaAsync` con joins + 3 subqueries · `ObtenerEstudiantesConAsistenciaRangoAsync` con subquery de rango). Constante `HeavyReportTimeoutSeconds = 60` en el repo. `BlobStorageService` (Azure SDK) y `JaaSTokenService` (JWT) auditados — no aplica. **Aprendizaje**: cuando un brief de F-fase asume "patrón mecánico replicable", auditar `N archivos × LOC` antes de aceptar el modo; >~1500 = pre-split en `/next-chat`. **Resto del scope** (4 repos `ConsultaAsistencia/AsistenciaAdmin/Usuarios/EmailMonitoreo` + cascade desde controllers + test de cancelación + sección "Timeouts y CancellationToken" en `backend.md`) movido a brief `open/110-be-load-control-f3b-cancellation-cascade.md`. Build local verde, no se corrió suite por ser cambio aditivo con `default` (no rompe callers). Desbloquea avance hacia F5 una vez 110 cierre.)_
4. _(Plan 38 Chat 6 FE cerrado local 2026-05-02 — `awaiting-prod/076` esperando deploy del FE + smoke. BE ya commiteado por Plan 39 Chat B (078). El banner local efímero fue **reemplazado por el shared `app-email-defer-fail-banner` de Plan 39 Chat D** en el mismo día; este chat conserva los toasts (`MessageService` con key `email-outbox-alerts`) en bandeja admin y la feature flag `emailDeferAlerts` (prod=`false`).)_
5. _(Plan 39 Chat B BE cerrado local 2026-04-30 — `awaiting-prod/078` esperando deploy del BE.)_
6. _(Plan 39 Chat C FE cerrado local 2026-04-30 — `awaiting-prod/079` esperando deploy del FE + 5 pasos de smoke test browser. 21 archivos nuevos + 4 modificados. Suite 1752/1752 verdes (+14 sobre baseline 1738). Sin SQL nuevo. Decisión: polling fallback 30s solo si hub no conecta; tile defer-fail-live consume `DeferFailStatus` reusando modelo Plan 22/29; `email-hub.service` queda bajo `dashboard-dia/services` para Plan 38 Chat 6 (076).)_ — Tab "Mapa de envío" en `/intranet/admin/monitoreo/correos/dashboard` con 6 tiles (defer-fail-live, sender-mix, top-destinatarios, serie-temporal, dominios-receptores, candidatos-blacklist) + skeletons independientes + cross-links "Bloquear" a tab Blacklist con `?correo=` prefill + listener SignalR de los 3 eventos del hub. Brief en `.claude/chats/open/079-plan-39-chat-c-fe-dashboard-monitoreo-tiles.md`. **Pre-req duro**: Chat A (077) deployado + Plan 38 Chat 5 (075) deployado.
7. _(Plan 39 Chat D FE cerrado local 2026-05-02 — `awaiting-prod/080` esperando deploy del FE. Banner B9 cross-páginas (`app-email-defer-fail-banner` smart) montado en 5 páginas admin: bandeja, dashboard-día, diagnóstico, auditoría, monitoreo-hub. Refactor del email-outbox.component (removida la state local del banner Chat 6, conservados toasts). Tile "Aceptado vs Entregado" pospuesto — depende de Plan 39 Chat E (HOLD) para confirm-delivery. 6 tests del banner verdes, suite admin email/monitoreo 195/195.)_
8. _(DS1-tests `educa-web` FE ✅ cerrado local 2026-05-04 — brief `closed/086-wal-sync-engine-tests-hot-loop.md`. Spec `wal-sync-engine.service.spec.ts` con 24 tests cubriendo `init()` race condition (recovery antes de listeners + emit migrations + processAllPending si online + `leader.start`), `stop()`, `processEntry` happy path + outer-catch que resetea IN_FLIGHT a PENDING, `processAllPending` guards (leader/online) + drain loop + coalescer, `processRetryable` con `recoverInFlight` safety net, los 4 paths post-DS1 de `handleError` (conflict / permanent / retryable-ok / retryable-max × 2 — `incrementRetry` retorna FAILED y null), `handleCrossTabCommit`, callback registration. WAL suite 58/58 · suite full **1822/1822** (+24 sobre baseline 1798). Engine queda con red de seguridad para refactors futuros sin QA manual.)_
9. _(DS1-smoke `educa-web` QA browser ✅ cerrado 2026-05-04 — brief `closed/087-wal-integration-smoke-test.md`. Doc estable en `claude-cowork/wal-integration-smoke.md` con los 8 casos (precondición · pasos · resultado verificable · DevTools query) + política de merge: PRs que tocan `core/services/wal/` corren el smoke antes del merge. 1ra ronda ejecutada en commit `478df42`: 7/8 ✅. Caso 3 (Conflict 409) bloqueado por bug BE — `PUT /api/sistema/cursos/.../actualizar` no enforce concurrencia con `rowVersion` stale; seguimiento en `tasks/wal-smoke-fail-3-be-no-rowversion-409.md`. 3 findings menores (toast genérico 4xx, follower no auto-refetch, falta banner REQUIRES_MIGRATION) anexados a `tasks/wal-cache-audit-fixes.md`. Subsistema WAL queda con red de integración real complementando los 24 specs unitarios del engine. **Fixes**: F-S04 toast → chat 089 `closed/`; F-S06 cross-tab → chat 090 `closed/`; F-S08 banner → chat 097 `awaiting-prod/` (originalmente 091, renumerado 2026-05-04).)_
10. _(097 FE WAL banner REQUIRES_MIGRATION ✅ cerrado local 2026-05-04 — renumerado de 091 a 097 el 2026-05-04 por colisión con `091-fe-wal-cross-tab-wire-remaining-facades`. Brief `awaiting-prod/097-fe-wal-banner-requires-migration.md`. `WalMigrationBannerComponent` B9 dismissible montado en `intranet-layout` entre `<app-offline-indicator />` y `<main>`. Consume `WalStatusFacade.hasMigrations` + `discardMigrationEntries()`. Dismiss local (signal, reaparece en reload). 10 tests verdes (comportamiento + a11y). Suite **1832/1832** (+10 sobre baseline 1822). Lint 0 errores · build limpio.)_
11. **[Plan 39 · Chat E · `Educa.API` · BE]** 🔒 **HOLD** — `ExpiredEximLogImportJob` (importador SSH logs Exim). Brief en `.claude/chats/open/081-plan-39-chat-e-be-importador-ssh-exim-logs.md` con flag `<!-- on-hold-criteria-pending -->`. Criterios reactivación documentados (Plan 38 captura <80% / OPS aprueba SSH / 2+ incidentes en trimestre). NO-GO definitivo si en 90d ningún criterio dispara.
12. **[Plan 24 · Chat 4 (B) · `Educa.API` · BE+OPS]** — subfase (A') cerrada 2026-04-24 con `CrossChexPollingOptions` + instrumentación Stopwatch (commit `862d4ca`). Falta (B): medir 48-72h de logs en prod (`ConfiguredMs / ElapsedMs / Drift`) y, si el proveedor lo permite, bajar `CrossChex:Polling:DelayBetweenPagesMs` de 30000 a 10000-15000 vía Azure App Configuration (sin redeploy). Cierra Plan 24 al 100%. **Postpuesto** por prioridad de Plan 38+39.
13. _(Plan 37 Chat 2 BE cerrado local 2026-05-02 — `awaiting-prod/067` esperando deploy del BE. Local commit `Educa.API@e32153f`. SQL `EmailQuarantine` + `EmailRecipientDomainPause` ya ejecutado en Azure por el usuario. 21 tests nuevos verdes (suite 1615/1615). Cuarentena temporal 24-72h con auto-release `*/15`, promoción a blacklist al 3er hit, pause por dominio receptor capado a 5 simultáneos. INV-MAIL09 documentado en business-rules.md §15.14 (4 capas: formato → blacklist → quarantine → domain-pause).)_
14. _(Plan 37 Chat 3 FE cerrado local 2026-05-02 — `awaiting-prod/068` esperando deploy del FE + smoke admin con telemetría de Chat 2 desplegada. Local commit `educa-web@b8dd4d5`. 44 archivos nuevos + 8 modificados. Suite 1784/1784 verdes (+46 sobre baseline 1738). 3 routing-based tabs nuevos en `correos-shell` (cuarentena, dominios pausados, eventos de defer) con feature flag + permiso dedicado por tab — patrón consolidado de Plan 38 Chat 5. Banner DOMAIN_BLOCKED por polling fallback (ver decisión 2 abajo). Cross-link `defer-fail-status-widget` WARNING/CRITICAL → tab eventos con `?desde=hoy`.)_
15. _(Plan 39 Chat A cerrado local 2026-04-29 — `awaiting-prod/077` esperando deploy del BE. Local commit `Educa.API@17099d1`. SQL `plan39_chat2_AddDashboardIndex.sql` ya ejecutado en Azure por el usuario. 30 tests nuevos verdes (suite 1559/1559).)_
16. _(Plan 38 Chat 2 cerrado local 2026-04-29 — `awaiting-prod/072` esperando deploy + 2 hits 4.2.2 sandbox + verificar fila `EmailBlacklist` con motivo `BOUNCE_MAILBOX_FULL`. SQL `plan38_chat2_AddBounceMailboxFullMotivo.sql` ya ejecutado en Azure.)_
17. _(Plan 38 Chat 3 cerrado local 2026-04-29 — `awaiting-prod/073` esperando deploy + smoke test 5 endpoints (POST manual idempotente, GET paginado con filtros estado/motivo/q, cap pageSize=100, DELETE despeje sigue OK, POST motivo automático → 400). 10 tests nuevos (8 controller + 2 service) → suite total 1573/1573 verdes. Sin SQL nuevo — depende del CHECK ya migrado por Chat 2.)_
18. _(Plan 38 Chat 4 cerrado local 2026-04-29 — `awaiting-prod/074` esperando deploy + 4 pasos de verificación: `/hangfire` muestra `blacklist-auto-cleanup` con próximo run 03:00 Perú · trigger manual + revisar logs · fila prueba `EBL_FechaUltimoFallo = NOW() - 10 days, EBL_Estado = 1, EBL_MotivoBloqueo = 'BOUNCE_MAILBOX_FULL'` queda con `EBL_Estado = 0, EBL_UsuarioMod = 'blacklist-auto-cleanup'` · logs `JobMarkedRows=N` visibles. Local commit `Educa.API@d90bb72`. 9 tests nuevos (6 job + 3 cron resolution) → suite total 1582/1582 verdes. Sin SQL nuevo.)_
19. _(Plan 38 Chat 5 cerrado local 2026-04-30 — `awaiting-prod/075` esperando deploy del FE + 7 pasos de smoke test browser: tab `/monitoreo/correos/blacklist` activo · filtro `motivo=BOUNCE_MAILBOX_FULL` · search `q=gmail` reduce filas · agregar dialog → POST → fila aparece sin refresh · despejar → DELETE → fila desaparece sin refresh · ver drawer con audit · deep-link `?action=add&correo=foo@x.com` prefilled. 44 tests nuevos → suite total 1738/1738 verdes (+55 netos sobre baseline 1683). Sin SQL nuevo. Decisión arquitectónica: routing-based tab consistente con `correos-shell` en lugar de tab interno al `email-outbox.component`.)_
20. _(Plan 37 Chat 1 cerrado local 2026-04-29 — `awaiting-prod/066` esperando ejecución de SQL + 1er ciclo con NDR delayed real.)_
21. _(Plan 36 Chat 5 cerrado local 2026-04-27 — `awaiting-prod/` esperando smoke test del menú Auditoría en prod.)_
22. _(Brief 102 ✅ cerrado local 2026-05-05 — `awaiting-prod/102` esperando deploy coordinado BE (`Educa.API master fbb239e`) + FE (`educa-web main`). Ship: BE +18 tests / 1639 verdes · FE +13 tests / 1921 verdes · lint+build OK ambos repos. `GET /api/sistema/runtime-health [Authorize(Director)]` con cache `IMemoryCache` 10s + INV-S07 fail-safe a UNKNOWN, `RuntimeHealthSnapshot` DTO, `UpDownCounter educa.requests.in_flight`, `RequestPercentileTracker` (sliding 5min, buckets log), `SqlClientCounterListener : EventListener` (active-hard + pooled), `SaturationClassifier` puro mapea ThreadPool/Requests/DB/GC → OK/STARVATION/EXTERNAL_BOTTLENECK/OVERLOAD. Página FE smart `/intranet/admin/sistema/runtime-health` + widget presentacional (4 cards ThreadPool/Requests/DB/GC, polling 60s togglable, banner fail-safe `isProbableTelemetryFailure`) + permiso `ADMIN_SISTEMA_RUNTIME_HEALTH` + entrada menú Sistema > Monitoreo + feature flag `runtimeHealth` (OFF prod, ON dev). `/verify` post-deploy: prender feature flag para Director en prod → ver snapshot real con counters > 0 → confirmar pattern OK con tráfico bajo. **Desbloquea** brief `096-be-load-control-layers` (capas 2-6: SemaphoreSlim global + bulkheads + timeouts + Retry-After) que ahora puede calibrar contra estas métricas.)_

23. _(Brief 099 ✅ cerrado local 2026-05-05 — `awaiting-prod/099` esperando deploy del FE. Fix de visibilidad de las 3 tabs del Plan 37 Chat 3 (`Cuarentena`, `Dominios pausados`, `Eventos defer`) en `/intranet/admin/monitoreo/correos`. **Causa raíz** (hipótesis #1 del brief): `emailQuarantineTab` + `emailDomainPausesTab` + `emailDeferEventsTab` quedaron en `false` en `environment.ts` (prod) aunque rutas, componentes, permisos y flags de dev estaban completos desde el commit `b8dd4d5` del Chat 3 (068). El `correos-shell.component.ts` filtra tabs por `environment.features[t.featureFlag]` → tabs ocultas en prod, click en URL inexistente `/cuarentena` (path real es `/quarantine`) caía al wildcard `**` y redirigía a home pública. **Fix mínimo**: 3 booleans `false → true` en `environment.ts:37-39`. Hipótesis 2 (permisos), 3 (rutas) y 4 (chunk roto) descartadas — todo lo demás estaba correcto. 1 archivo modificado, lint ✅. **Aprendizaje transferible**: cuando un Chat de FE introduce features tras un flag y el flag prod queda en `false` "hasta que el BE esté listo", anotar en el brief la condición exacta de activación (ej: "encender post-deploy del Chat N BE"); sin esa nota, el flag puede quedar dormido indefinidamente y el QA reporta el bug semanas después. `/verify` post-deploy → ver las 3 tabs en `/intranet/admin/monitoreo/correos` con cuenta Director y confirmar que `/quarantine`, `/domain-pauses`, `/defer-events` cargan.)_

**Notas**:

- 🆕 **Brief 094 FE+BE WAL Resilience M4 ✅ cerrado local 2026-05-04** — `educa-web main` + `Educa.API master` (commits coordinados). Schema fingerprint en cache: cada GET `/api/*` mapeado lleva header `X-Schema-Version` (BE → FE), persistido por el SW con la entrada de IndexedDB. Lectura compara cached.schemaVersion vs el map embebido del SW; mismatch → discard + fetch fresh con `console.info` para diagnóstico de deploys (INV-WAL-RES12). FE: `src/app/shared/constants/api-schema-versions.ts` (37 endpoints registrados con longest-prefix matcher case-insensitive + helper `getSchemaVersion`/`extractPathname`), interceptor nuevo `schema-version` (entre `requestTrace` y `swCacheInvalidation`), update `public/sw.js` con `SCHEMA_VERSIONS` embebido + `getSchemaVersionForPath`/`pathnameOf`/`parseSchemaHeader`, `saveToCache(url, data, responseSchemaVersion)` skip cache si endpoint no mapeado (INV-WAL-RES11), `getFromCache` valida schemaVersion vs map en lectura. BE: `Constants/Sistema/ApiSchemaVersions.cs` espejo del map FE con `Resolve()` longest-prefix, `Middleware/SchemaVersionMiddleware.cs` que setea header vía `OnStarting` solo en GET `/api/*` mapeados, registrado en `PipelineExtensions` después de `CorrelationIdMiddleware`. Decisión: **versión persistida viene del header BE** (no del map FE) para detectar deploys desincronizados — si FE expecta v=2 y BE responde v=1, mismatch dispara discard. Default v=1 cuando BE no envía header (compat). `dotnet build` limpio · 1908 FE tests verdes (sin regresiones) · ESLint + tsc OK. Brief en `awaiting-prod/094` esperando deploy coordinado FE+BE + smoke: bumpear `/api/sistema/usuarios` a v=2 en ambos repos → primera carga hace network, resto del cache intacto. **Deuda menor reportada**: tests dedicados (`sw.service.spec.ts` con flujo fingerprint, `SchemaVersionMiddlewareTests.cs`) no agregados — la lógica nueva del SW corre en contexto Service Worker, no fácilmente testeable con vitest+jsdom; el middleware BE quedó cubierto solo por build. **Plan WAL Resilience completo**: M1 (091 reconciliación) + M2 (092 circuit breaker) + M3 (093 IndexedDB fallback) + M4 (este chat). Próximo paso del plan: monitoreo 30d en prod para validar métricas (reportes "datos viejos" -80%, WAL_DEGRADED <5min/día, cache hits con schema mismatch post-deploy 100% primer hit / 0% después). **Aprendizaje transferible**: invariantes que requieren coordinación FE+BE en deploys (como bumps de schema fingerprint) necesitan single source of truth replicada en N puntos físicos — registrar la replicación como deuda técnica explícita (TODO: build step que genere `sw.js` desde el `.ts`) en lugar de confiar en disciplina humana entre repos. La nota de bumpeo va en cada uno de los 3 archivos.
- 🆕 **Brief 098 FE ✅ cerrado local 2026-05-05** — `educa-web main`. Fix end-to-end del cross-tab refetch que el smoke browser reportó roto post-091 (Tab 2 contadores stale tras CRUD en Tab 1). **Causa raíz** (no estaba en las 4 hipótesis del brief): asimetría de contrato entre el callback local del leader (`cb.onCommit` refresca items + stats por separado) y el callback registrado en el canal cross-tab (`silentRefreshAfterCrud()` solo refresca items, omite stats silenciosamente). **Fix sistémico (scope C)**: firma de `WalCrossTabRefetchService.subscribe` endurecida → `refetchItems` obligatorio + `refetchStats` opcional. TypeScript ahora fuerza la decisión explícita en cada facade. Cambios: 1 helper + BaseCrudFacade + 17 facades manuales + 1 spec + 6 docs (`rules/{debug,optimistic-ui,service-worker}.md` + `documentacion-subsistemas/{wal-write-ahead-log §7.1,service-worker-scope §3}.md` + `claude-cowork/SETUP-COWORK.md` §6.4). 3 facades (usuarios-data, attendances-data, feedback-reports) ganaron `refetchStats:` explícito; `usuarios-data` adicionalmente con nuevo método `refreshEstadisticasOnly()`. Validación: typecheck ✅ + 165/165 tests ✅ + lint 0 errors ✅ + smoke browser end-to-end Cowork ✅ (Tab 2 counter 27 → 28 + tabla refrescó). **Hallazgos colaterales documentados**: (1) SW agresivo en `npm run start` sirve chunks JS stale incluso con hot-reload — workaround unregister + clear caches; (2) `logger.log()` no emite en `ng serve` porque Angular 21 esbuild no expone `ngDevMode` → `isDevMode()` retorna `false` → workaround `console.log` directo con tag `// TEMP debug NNN`; (3) la asimetría items/stats es exactamente la clase de bug que pasa code review porque "casi funciona". Brief movido a `awaiting-prod/098` esperando deploy del FE + smoke real post-deploy. `/verify` cuando pase. **Aprendizaje transferible**: endurecimiento de contrato (typing estricto) vence al tag-and-fix manual en bugs de asimetría — convierte el olvido silencioso en error de compilación.

- 🆕 **Brief 091 FE ✅ cerrado local 2026-05-04** — `educa-web main`. WAL cross-tab refetch wireado en los 18 facades restantes que no extienden `BaseCrudFacade` (handoff de chat 090). 12 archivos físicos modificados que cubren los 16 items del inventario A+B+C+D del brief vía suscripciones en data facades hermanos: `usuarios-data` + `permisos-usuarios-data` + `permisos-roles` + `feedback-reports` + `error-groups-data` + `blacklist-data` + `email-quarantine-data` + `email-domain-pause-data` + `admin-health-permissions` (2 subs) + `attendances-data` (2 subs: `asistencia-admin` + `cierre-asistencia`) + `events-calendar` + `notificaciones-admin` + `horarios-data` + `calificaciones` + `grupos` + `health-permissions` profesor (2 subs) + `curso-contenido-data` (5 subs: `CursoContenido` + `Semana` + `Tarea` + `Archivo` + `TareaArchivo`). 5 facades de Section E (mensajeria, estudiante-cursos, feedback-report core, attachments-modal, curso-contenido-crud) recibieron comentario de decisión documentado (no aplica refetch o cubierto por hermano). 2 archivos cruzaron cap 300 ln por wiring legítimo (`horarios-data` 302 + `grupos` 301) → `eslint-disable max-lines` con justificación. Lint ✅ (0 errores propios), `npx vitest run` **1834/1834 verdes**, 185 archivos. Brief movido a `awaiting-prod/091` esperando smoke manual cross-tab en al menos 3 features (`/intranet/admin/usuarios`, `/intranet/admin/schedules`, `/intranet/profesor/cursos`) — abrir 2 tabs, mutar en uno, ver refetch en el otro sin navegar. `/verify` cuando pase. **Aprendizaje transferible**: cuando el refetch del feature requiere un ID contextual (`contenidoId` para calificaciones, `salonId` para health-permissions), capturarlo del store del feature dentro del closure del helper en lugar de promoverlo al brief o al helper genérico.

- 🆕 **Brief 089 FE ✅ cerrado local 2026-05-04** — `educa-web main`. Fix WAL: toast en errores 4xx ahora muestra el mensaje específico del BE cuando existe, en lugar del genérico "La solicitud contiene datos inválidos". Root cause: `[ApiController]` de ASP.NET Core retorna `ValidationProblemDetails` (`{ errors: { Campo: ["mensaje"] } }`) en lugar de `ApiResponse` (`{ message: "..." }`) para errores de model binding — `extractErrorMessage` no buscaba el path `errors[firstField][0]`. Fix en 2 puntos: `extractErrorMessage` (`error.utils.ts`) y `getHttpErrorMessage` (`error-handler.service.ts`) — ambos agegan extracción de `ValidationProblemDetails` después del check `message`, antes del fallback por HTTP status. Con ambos puntos corregidos, interceptor + facade producen el mismo mensaje → dedup de `ErrorHandlerService` suprime el segundo toast. **3 archivos** (2 fix + 1 spec nuevo `error.utils.spec.ts`, 9 tests cubriendo ApiResponse, ValidationProblemDetails multi-campo, y fallbacks). Lint ✅, tests 9/9 ✅. No requiere deploy BE — es fix FE puro. Origen: F-S04 del WAL Integration Smoke 1ra ronda (chat 087, commit `9c39d40`).

- 🆕 **Brief 088 BE ✅ cerrado local 2026-05-04** — `Educa.API master`, commit `9951d44`. Fix de concurrencia optimista que destrababa el Caso 3 del WAL smoke (chat 087). El override `ApplicationDbContext.SaveChangesAsync` aplicaba "last-write-wins" (try/catch + retry hasta 3x sobre `DbUpdateConcurrencyException` + refresh `OriginalValues` desde DB) — convertía toda mutación con rowVersion stale en sobreescritura silenciosa, contradiciendo el contrato del WAL FE que necesita ver el 409 para marcar `CONFLICT` y rollback. Fix: borradas ~30 líneas del retry, queda solo `ApplyAuditFields() + base.SaveChangesAsync()`. `GlobalExceptionMiddleware` ya mapea `DbUpdateConcurrencyException → 409 + errorCode CONCURRENCY_CONFLICT`. Aplica transversalmente a todas las entidades con `[Timestamp]`, no solo cursos. **2 archivos** (1 fix + 1 test nuevo `Educa.API.Tests/Persistence/ApplicationDbContextConcurrencyTests.cs` con `SaveChangesInterceptor` que tira la excepción y verifica propagación + `SaveAttempts == 1`). Suite total **1616/1616 verdes** (+1 sobre baseline 1615 del Plan 37 Chat 2). **Hallazgos del agente**: las 3 hipótesis del task fuente (`tasks/wal-smoke-fail-3-be-no-rowversion-409.md`) eran incorrectas — `[Timestamp]` estaba presente, middleware OK, `BaseRepository.UpdateAndSaveAsync` no era el bug. La causa real estaba un nivel más arriba en el SaveChanges del DbContext. Brief movido a `awaiting-prod/088` esperando deploy del BE + re-ejecución de Caso 3 del WAL smoke vía Cowork. `/verify` cuando pase. **Aprendizaje transferible**: las hipótesis del repro son punto de partida, no oráculo — un override de `SaveChangesAsync` puede neutralizar invariantes de concurrencia configurados correctamente a nivel modelo + repository. Vale revisar el DbContext base cuando un sistema "ignora" rowVersion stale.

- 🆕 **Plan 37 Chat 3 FE ✅ cerrado local 2026-05-02** — `educa-web main`, commit `b8dd4d5`. Visibilidad admin de cuarentena, pause de dominio y eventos de defer en `/intranet/admin/monitoreo/correos`. **44 archivos nuevos + 8 modificados** (3 modelos, 11 servicios — 5 quarantine multi-facade + 5 domain-pause multi-facade + 1 defer-events read-only —, 12 carpetas de componentes con ts/html/scss + spec, banner DOMAIN_BLOCKED). 3 nuevas rutas hijas en `correos-shell` (`/quarantine`, `/domain-pauses`, `/defer-events`) con feature flag dedicado (`emailQuarantineTab`/`emailDomainPausesTab`/`emailDeferEventsTab` — todos `false` en producción) y permiso dedicado por tab (`ADMIN_EMAIL_QUARANTINE`/`ADMIN_EMAIL_DOMAIN_PAUSES`/`ADMIN_EMAIL_DEFER_EVENTS`) siguiendo plantilla consolidada de Plan 38 Chat 5. WAL optimista deterministico con `apply`/`rollback` para `release` y `addManual`; presentational tables paginadas server-side variante A para Quarantine y DeferEvents (25/pág), client-side para DomainPauses (≤50 filas activas). Cross-link `defer-fail-status-widget` WARNING/CRITICAL → navega a `/defer-events?desde=hoy` (output `(goToEvents)` agregado al widget existente). Design system completo: B1 contenedor con border, B4 tabla, B6 filter bar, B8 dialogs (separación `[visible]`+`(visibleChange)` por `dialogs-sync.md`), B9 alert banners con `color-mix()`, B10 drawer detalle. Aria-label vía `pt` en todos los icon-only. Tipos semánticos `const + type` para 4 enums. UiMappingService extendido (D12 + D20) con label + severity para `QuarantineMotivo`/`DomainPauseMotivo`/`DeferEventTipo`/`MotivoLiberacion`. Mayor archivo nuevo: 236 ln (cap 300). 46 tests nuevos en 12 spec files → suite total **1784/1784 verdes** (+46 sobre baseline 1738). `npm run lint` 0 errores. `npm run build` production OK. **Decisiones del agente sobre la marcha**: (1) **Routing-based tabs en `correos-shell`** en lugar de tabs internos al `email-outbox.component` que el brief original sugería — alineado con el patrón consolidado de Plan 38 Chat 5 (`correos-shell` ya tenía 5 tabs así). (2) **`DomainBlockedDetected` SignalR event NO existe en BE** — `EmailHub` (Plan 39 Chat B) expone solo 3 eventos (`BlacklistEntryCreated`, `DeferFailStatusUpdated`, `CandidatoBlacklistDetectado`). Aplicado fallback: `DomainBlockedAlertBannerComponent` polea `GET /defer-events?tipo=DOMAIN_BLOCKED&desde=<now-12h>` al init. La estructura del componente queda lista para wirear el observable cuando el BE agregue el evento. (3) **Quarantine release** usa `ConfirmationService.confirm` con `motivoLiberacion='OTRO'` por defecto en lugar de un release-confirmation dialog dedicado — el brief no definía dialog específico para release; UX más rico (selector de motivo) puede agregarse cuando aparezca la necesidad. (4) **Colisión de signal `_filterEstado`** en `BaseCrudStore` resuelta renombrando el local a `_filterEstadoQ` (caught at build, fixed). Brief movido a `awaiting-prod/068` esperando deploy del FE + 4 condiciones de smoke (ver brief). **Aprendizaje transferible**: cuando el FE necesita reaccionar a un evento BE en tiempo real y ese evento aún no está publicado, el fallback de polling sobre el endpoint que registra ese evento (mismo dato, vía pull en lugar de push) deja la UI funcional sin bloquear el chat — el SignalR push se cablea después con un cambio mínimo cuando el BE lo agregue. **Pendiente de seguimiento (no parte de este chat)**: extender `EmailHub` BE con evento `DomainBlockedDetected` para sustituir el polling — ticket potencial de Plan 39 si se prioriza.

- 🆕 **Plan 37 Chat 2 BE ✅ cerrado local 2026-05-02** — `Educa.API master`, commit `e32153f`. Cuarentena temporal por destinatario + pause por dominio receptor consumiendo `EmailDeferEvent` del Chat 1. **17 archivos nuevos + 8 modificados** (la realidad de la arquitectura del proyecto sumó capa `Repositories/Notifications/` + `Data/Configurations/` que el brief original no contemplaba; conteo final 33 archivos creados incluyendo tests). 2 modelos (`EmailQuarantine` con FK a `EmailDeferEvent`, `EmailRecipientDomainPause`), 2 constantes de motivos con `MotivosLiberacion`, 2 EF configurations, 4 archivos repository + interfaces, evaluador puro `QuarantinePromotionEvaluator` (Singleton, sin DbContext) con 5 reglas + cap 5 dominios pausados, 2 services + interfaces (CRUD + auto-promoción), 2 gates (`EmailQuarantineGate` + `EmailDomainPauseGate`) consumidos por `EnqueueWithResultAsync`, 2 Hangfire jobs cron `*/15` con flags hot-config en `EmailSettings`, controller con 7 endpoints + DTOs (`Director,AsistenteAdministrativo`). Modificados: `EmailOutboxService.Enqueue.cs` (introdujo `EnqueueResult`+`EnqueueOutcome` + compat layer `EnqueueAsync`→`Guid?`), `DeferEventService.cs` partido en partial (145 ln + `DeferEventService.Evaluate.cs` 218 ln) con try/catch INV-S07 que preserva el insert del evento si el evaluador rompe, `ApplicationDbContext.cs` (2 DbSets), `HangfireJobs.cs` (2 constantes), `HangfireExtensions.cs` (registro 2 jobs), `ServiceExtensions.cs` (9 servicios scoped + evaluator singleton), `EmailSettings.cs` (`QuarantineRelease`/`DomainPauseRelease`), `Educa.API.Tests/Helpers/Db/TestDbContextFactory.cs` (relax ROWVERSION en InMemory). 21 tests nuevos en 5 archivos → suite total **1615/1615 verdes** (+21 sobre baseline 1594). `dotnet build` 0 errores. Mayor archivo nuevo: 218 ln (cap 300). **Decisiones del agente sobre la marcha**: (1) **3ra cuarentena promueve a blacklist** reusando motivo existente `EmailBlacklistMotivos.Bounce5xx` con contexto en `EBL_UltimoError = "Promoted from quarantine, hit #N"` — evita un `ALTER TABLE` adicional al CHECK constraint de `EmailBlacklist`. Si en algún momento se quiere distinguir promoción-por-quarantine de bounce 5.x.x directo, abrir chat aparte con su SQL. (2) **`EnqueueResult`** simple record `(Outcome, OutboxId?)`; legacy `EnqueueAsync(...) → Guid?` delega y descarta — 0 cambios en los ~12 callers existentes. (3) **Cap de 5 domain pauses** se enforza en el evaluador puro (`ActiveDomainPausesCount >= 5 → return null`) con `LogWarning` — no rechaza envíos, solo no agrega más; admin puede liberar manualmente vía endpoint GET. (4) **`QuarantinePromotionEvaluator` Singleton** porque es puro (sin DbContext, sin IO). (5) **`DeferEventService` partido en partial** para respetar cap 300 ln sin perder cohesión: el archivo principal mantiene CRUD del evento, el `.Evaluate.cs` carga la orquestación del evaluador. SQL `EmailQuarantine` + `EmailRecipientDomainPause` ya ejecutado por el usuario en Azure por adelantado. Brief movido a `awaiting-prod/067` esperando deploy del BE + smoke admin endpoints + 1er evento `MAILBOX_FULL_TRANSIENT` real produciendo fila `EmailQuarantine` con `EQU_OriginEventId` poblado. **Aprendizaje transferible**: cuando un brief proyecta N archivos contra un layout sin contar capas Repository/Configuration que el repo sí usa, el conteo real puede subir 50-70% sin que sea scope creep — vale validar la arquitectura existente antes de cerrar el conteo del brief en `/design`. INV-MAIL09 documentado en `business-rules.md` §15.14 (4 capas: formato → blacklist → quarantine → domain-pause; rechazo nunca toca el MTA y no consume el contador `INV-MAIL03`). **Desbloquea Plan 37 Chat 3 (068)** tras verify del 067 con telemetría real.

- 🆕 **Plan 38 Chat 5 FE ✅ cerrado local 2026-04-30** — `educa-web main` (commit pendiente de hash). Tab "Blacklist" admin de correos: nueva ruta hija `/intranet/admin/monitoreo/correos/blacklist` montada en el `correos-shell` con feature flag `emailBlacklistTab`, permiso `ADMIN_EMAIL_BLACKLIST` y los 5 servicios completos (HTTP gateway + store extends `BaseCrudStore` + multi-facade data/crud/ui) + 4 componentes (smart `blacklist-tab` + presentational `blacklist-table` server-paginated + `blacklist-add-dialog` con catálogo D17.8 sólo MANUAL/BULK_IMPORT + `blacklist-detail-drawer` B10). WAL optimista con apply/rollback deterministas: CREATE apply cierra dialog y onCommit agrega item + stats; DELETE apply quita item + ajusta stats, rollback restaura **posición original** del item (no prepend/refetch). Cross-link Plan 39 Chat C cumplido: `?action=add&correo=...` abre el dialog prefilled. Catálogo de motivo con label humano + severity en `UiMappingService` (D12 + D20). 12 archivos nuevos + 8 modificados; 44 tests nuevos (9 store + 4 service + 7 data-facade + 7 crud-facade + 6 table + 7 dialog + 4 drawer) → suite total **1738/1738 verdes** (+55 netos sobre baseline 1683). Lint 0 errores. Build production OK. **Decisión arquitectónica reportada**: el brief asumía tabs internos al `email-outbox.component` pero el `correos-shell` (Plan 36 Chat 1) ya implementaba routing-based tabs — implementé el patrón consistente para que Plan 37 Chat 2 (Cuarentena) y Chat 3 (Dominios) sigan la misma plantilla sin introducir patrón híbrido. **Aprendizaje transferible**: la plantilla `routing-based tab + feature flag + permiso dedicado` está consolidada en `correos-shell` (5 tabs ahora) e `incidencias-shell` (2 tabs); cualquier tab nuevo en `monitoreo` se construye con esa receta. Brief movido a `awaiting-prod/075` esperando deploy del FE + 7 pasos de smoke test browser.

- 🆕 **Plan 38 Chat 4 BE ✅ cerrado local 2026-04-29** — `Educa.API master`, commit `d90bb72`. `BlacklistAutoCleanupJob` Hangfire diario para `INV-MAIL06`: cron `"0 3 * * *"` Lima, despeja entradas con motivo `BOUNCE_MAILBOX_FULL` cuyo `EBL_FechaUltimoFallo` predata `MailboxFullCleanupDays` (default 7d). Otros motivos (`BOUNCE_5XX`/`MANUAL`/`BULK_IMPORT`/`FORMAT_INVALID`) no se tocan automáticamente. Soft-delete (INV-D03) — UPDATE batch trackeado para preservar auditoría INV-D02 vía `ApplyAuditFields`. Try/catch global INV-S07 evita marcar el job como Failed en Hangfire. `EmailSettings.BlacklistAutoCleanup.Enabled=false` hot-config switches a `Cron.Never()` para rollback rápido sin redeploy. Helper internal `HangfireExtensions.ResolveCleanupCron(BlacklistAutoCleanupSettings)` testeado vía `InternalsVisibleTo("Educa.API.Tests")` — patrón replicable para futuros jobs Hangfire que mezclan config + cron sin levantar storage. 4 archivos nuevos + 6 modificados; 9 tests (6 job + 3 cron resolution) → suite total **1582/1582 verdes** (+9 netos sobre baseline 1573). Sin SQL nuevo. Brief movido a `awaiting-prod/074` esperando deploy + verificación 4 pasos (`/hangfire`, trigger manual, fila prueba `-10d`, logs `JobMarkedRows=N`).

- 🆕 **Plan 39 Chat B BE ✅ cerrado local 2026-04-30** — `Educa.API master`, commit `1ce220e`. `EmailHub` SignalR server-side + `SignalREmailHubNotifier` real reemplazando al `NullEmailHubNotifier` (NoOp queda en código para rollback rápido vía cambio de `services.AddScoped`). Hub mapeado en `/hubs/email-alerts` con `[Authorize(Roles = Roles.Administrativos)]` a nivel clase + `JoinAlertsGroup` / `LeaveAlertsGroup` explícito + `IEmailHubClient` strongly-typed con 3 eventos. 3 DTOs nuevos en `DTOs/Notifications/Monitoreo/EmailHubEventDtos.cs` (`BlacklistEntryCreatedDto`, `DeferFailStatusUpdatedDto`, `CandidatoBlacklistDetectadoDto`). Notifier cumple INV-S07 (cada push envuelto en try/catch + LogWarning, jamás propaga al caller — un fallo del backplane SignalR no debe romper al `MailboxFullBlacklistHandler` ni al worker) e INV-MAIL08 (invalida `IMemoryCache` key `defer-fail-status:dominio` ANTES de pushear `DeferFailStatusUpdated`, el `Remove` es idempotente y deja la pieza lista para Chat C/D si `EmailOutboxStatusService` se cachea más adelante). 5 archivos nuevos + 2 modificados (`ServiceExtensions.cs` swap + `PipelineExtensions.cs` `MapHub`); 7 tests nuevos (3 hub + 4 notifier — incluye verificación de orden cache-Remove ANTES del push, fail-open con backplane caído) → suite total **1589/1589 verdes** (+30 sobre baseline 1559 de Chat A). Sin SQL nuevo. **Decisiones de scope vs brief**: (1) la interfaz `IEmailHubNotifier` quedó con primitivos (firma definida por Plan 38 Chat 2 072) en lugar de los DTOs propuestos por el brief — los DTOs viajan al cliente vía `IEmailHubClient` strongly-typed; los handlers existentes invocan al notifier sin cambios. Refactor a DTOs en la interfaz queda como deuda menor si alguna vez se necesita pasar más campos. (2) **No cableé el push de `DeferFailStatusUpdated` desde el worker en este chat** — el brief sugería detectar transición de banda OK→WARNING→CRITICAL después de cada fail/retry. El servicio referenciado (`RateLimitDeferFailStatusService`) no existe con ese nombre; el real (`EmailOutboxStatusService`) es read-only y agregar detección de transición persistente cruza varias capas (worker hooks + singleton de last-band + recompute on-demand). El push está implementado y funcional, pero por ahora el único trigger automático activo es el del polling del widget. Cableado del trigger queda como gap conocido para una iteración cuando haya un hook claro. (3) `EmailOutboxStatusService` no cachea hoy — el `_cache.Remove` defensivo es idempotente y se alinea con INV-MAIL08 sin requerir refactor del service. Brief movido a `awaiting-prod/078` esperando deploy del BE para `/verify`. **Aprendizaje transferible**: cuando un chat tiene que reemplazar una registración DI en `ServiceExtensions.cs`, dejar la implementación NoOp en el código (no borrar) habilita rollback de 1 línea sin redeploy; vale el peso del archivo extra cuando el componente es nuevo y crítico (alertas push, no se puede reproducir el bug en local fácil).

- 🆕 **Plan 39 Chat A BE ✅ cerrado local 2026-04-29** — `Educa.API master`, commit `17099d1`. `EmailMonitoreoService` + 5 endpoints (`/api/sistema/email-outbox/monitoreo/{sender-stats|top-destinatarios|serie-temporal|dominios-receptores|candidatos-blacklist}`) con `IMemoryCache` por método (TTL 30s-10min según volatilidad), fail-safe INV-S07 (lista vacía + LogWarning si query falla), defaults de `candidatos-blacklist` inyectados desde `EmailSettings.MailboxFullThreshold*` (alineado con INV-MAIL07), caps defensivos `ventanaDias<=30 / limit<=50 / ventanaHoras<=720`. SQL `plan39_chat2_AddDashboardIndex.sql` ya ejecutado en Azure (índice covering `IX_EmailOutbox_FechaReg_Estado_TipoFallo`). 30 tests nuevos (8 service + 2 cache + 14 authorization theory + 3 query-params + 3 NullEmailHubNotifier no-throw) → suite total 1559/1559. Snapshot del incidente 2026-04-20→29 en `Educa.API/outputs/cpanel-fallos-historicos-2026-04-20-29.md`. Brief movido a `awaiting-prod/077` esperando deploy del BE para `/verify`.

- 🆕 **Plan 39 Chat 1 `/design` ✅ cerrado 2026-04-29** — Monitoreo de correos basado en evidencia (dashboard 6 tiles + push SignalR). 18 decisiones cerradas (D1-D18): ventana default 7d con caps 30d/50, umbral candidatos alineado con `EmailSettings.MailboxFullThresholdHits` (Plan 38 D1), caché `IMemoryCache` por método con TTL 30s-10min según volatilidad, 5 endpoints paralelos (no agregado), `EmailHub` server-side **owned por Plan 39 Chat B 078** con NoOp registrado por Plan 38 Chat 2 072 hasta el reemplazo, Chat 4 SSH **HOLD** con criterios reactivación (Plan 38 captura <80% / OPS / 2+ incidentes), índice covering `IX_EmailOutbox_FechaReg_Estado_TipoFallo` nuevo, `INV-MAIL08` agregada a `business-rules.md`. 5 chats derivados creados (077-081 BE+FE+Hub+HOLD). Brief movido a `closed/071-plan-39-chat-1-monitoreo-empirico-design.md`. Cross-link con Plan 38: el dialog "Agregar a blacklist" del Chat 5 (075) debe leer `?correo=...` para que los CTAs "Bloquear" del dashboard prefillen.

- 🆕 **Plan 38 Chat 1 `/design` ✅ cerrado 2026-04-29** — Auto-blacklist 4.2.2 + UI admin de blacklist. 20 decisiones cerradas (umbrales 2/24h cleanup 7d, separación de handlers `MailboxFullBlacklistHandler` aparte del `EmailBounceBlacklistHandler` existente, motivo `BOUNCE_MAILBOX_FULL` con CHECK constraint nuevo, endpoints POST/GET paginado wrapper variante A, `EmailHub` aparte para SignalR, `INV-MAIL06` cleanup 7d + `INV-MAIL07` umbral 2/24h agregadas a `business-rules.md`, `INV-MAIL02` editado para excluir 4.2.2). 5 chats derivados creados (072-076 BE+FE+SignalR). Plan 38 entra a la cola top-3 con prioridad alta — empuja Plan 24 Chat 4(B) y Plan 37 a posición 7-9. Brief movido a `closed/070-plan-38-chat-1-blacklist-investigacion-design.md`. Plan 39 Chat 1 design (071) sigue **running** en chat aparte para no mezclar con la ejecución de Plan 38.

- 🟢 **Plan 36 Chats 8 + 8b + 8c ✅ cerrados 100% 2026-04-28 (validados en prod)** — trío rate-limit telemetry. Chat 8c BE (`Educa.API master 5165b62`) deployado y validado vía 429 real en `POST /api/Auth/login` desde DevTools: BD persiste `REL_LimiteEfectivo=30, REL_TokensConsumidos=30, REL_FueRechazado=1` en 5 filas; UI columna `INTENTOS / UMBRAL` muestra `30 / 30` (ya no `— / —`); cards superiores `45 total / 45 rechazados / Top rol Director / Top endpoint /api/Auth/login`. Chat 8 (063) FE columna agregada validada. Chat 8b (064) cerrado como "absorbido por 065" — fue investigación pura sin código, su conclusión "FE OK, fix BE" fue correcta. Hallazgo lateral: en prod `loginLimit=30` por override de App Configuration (no `60` del code) — el stash captura el `PermitLimit` real de runtime, invariante respetado. Plan 36 a **~33%** (3 de 9 chats cerrados al 100%; los 5 restantes en queue + Chat 5 awaiting-prod del menú Auditoría smoke test).

- 🟢 **Plan 36 Chat 8c BE ✅ cerrado local 2026-04-28** (`Educa.API master`, commit `5165b62`) — Persiste `REL_LimiteEfectivo` y `REL_TokensConsumidos` en eventos `RateLimitEvent` rechazados. Resuelve causa raíz identificada en Chat 8b investigación FE (064): `LogRejectedAsync` no recibía ni persistía los campos → admin veía `— / —` en columna `INTENTOS / UMBRAL` para todo 429. **Approach**: `RateLimitingExtensions` expone constantes `LimitItemKey`/`TokensItemKey` + helper estático `StashLimit(context, limit)`; las 6 partition factories fijas (global GET/POST + login/refresh/biometric/feedback/heavy) y `RateLimitPartitionResolver.Resolve` (policies `reports`/`batch` con role multiplier × endpoint override) stashan el `PermitLimit` efectivo en `HttpContext.Items` antes de devolver el partition descriptor. Middleware lee con `ReadIntItem` defensivo y forwardea al service. **Invariante FixedWindow**: rechazo ocurre al agotar permits → `tokensConsumidos == limiteEfectivo` en el momento del 429 (aproximación válida y coherente con el dolor del admin). Firma de `IRateLimitTelemetryService.LogRejectedAsync` extendida con `int? limiteEfectivo, int? tokensConsumidos` opcionales (backward compatible). INV-S07/INV-ET02 preserved: si por algún motivo no hay stash, persiste NULL como hoy. **+6 tests** (2 service + 2 middleware + 2 resolver) → **91/91 RateLimit verdes**. Suite total 1483/1488 — las 5 fallas son pre-existentes en `Excel.*Plan27_INV_C11` (deuda commit `2ea7830`, no atribuibles a este chat — verificado vía `git stash` + re-run). Build limpio. 5 archivos producción + 3 tests. Brief movido a `awaiting-prod/065-plan-36-chat-8c-be-rate-limit-persistir-tokens-y-limite.md`. **Pendiente post-deploy**: forzar 429 real en prod (login spam o endpoint con policy) → verificar columna `INTENTOS / UMBRAL` muestra valores reales en `/intranet/admin/monitoreo/seguridad/rate-limit`. Cuando ✅, cerrar 063 (awaiting-prod/) y 064 (waiting/) juntos vía `/verify`. Desbloquea ambos chats FE en una sola validación.
...
- 🟢 **Plan 37 Chat 1 BE ✅ cerrado local 2026-04-29** en `Educa.API master 4011c02` — telemetría `EmailDeferEvent` para defers crónicos. **5 archivos producción nuevos** (`EmailDeferEventTipos`, `EmailDeferEvent` model + Configuration, `DeferEventDetector` puro 5 reglas, `DeferEventService` con lookup soft-bounce 7d) + **5 modificados** (`Ndr3464Parser` extrae `Action`, `NdrParsedResult` con `Action`+`Subject`, `BounceParserService` salta handler en `Action: delayed` + try/catch INV-S07, `ApplicationDbContext.DbSet`, `ServiceExtensions` DI). **+10 tests** (7 detector + 2 service + 1 BounceParser delayed) → suite **1514/1514 BE verdes**. `dotnet build` 0 errores. Todos los .cs bajo 300 ln. **Decisiones del usuario aplicadas en auto mode**: pre-req Plan 31 Chat 2 confirmado vía `git log` (commit `b399399` presente), fixtures sintéticos siguiendo wording típico Postfix/Exim (validación post-deploy con NDRs reales queda como criterio de cierre), patrón 2 SaveChangesAsync para tener `BPR_CodID` real en FK lógico. Brief movido a `awaiting-prod/066-plan-37-chat-1-be-parser-extension-defer-events.md`. **Pendiente verificación post-deploy**: (1) ejecutar SQL `CREATE TABLE EmailDeferEvent` + 3 índices filtrados en Azure SQL ANTES del deploy BE; (2) deploy a Azure App Service; (3) confirmar tras 1er ciclo con NDR `Action: delayed` real ≥1 fila en `EmailDeferEvent` con tipo `WARNING_DELAYED_*` o `MAILBOX_FULL_TRANSIENT`. Cuando ✅, cerrar 066 vía `/verify` + memoria nueva "EmailDeferEvent activo desde {fecha} — alimenta cuarentena del Chat 2 (067)". **Desbloquea Plan 37 Chat 2 (067)** tras telemetría limpia + 48h sin regresiones.
- 🟢 **Plan 31 Chat 2b BE ✅ cerrado local 2026-04-28** en `Educa.API master` — wiring Hangfire del bounce parser. `BounceParserJob.cs` (37 ln, INV-S07 con try/catch defensivo en el wrapper aunque el parser ya implementa INV-S07 internamente). `HangfireJobs.BounceParser = "bounce-parser-imap"` agregada. `ServiceExtensions.cs` registra `BounceParserJob` como Scoped. `HangfireExtensions.UseHangfireServices` lee `IOptions<EmailSettings>.BounceParser`: cron `*/{PollingIntervalMinutes} * * * *` cuando `Enabled=true`, `Cron.Never()` cuando `false` (rollback rápido sin redeploy — solo cambiar env var + restart). **Decisiones del usuario aplicadas**: 1A reusar `EmailSettings.Senders` del pool SMTP (parser desde Chat 2a ya consume esa fuente), 2A carpeta `Processed/` auto-creada por el parser. **+2 tests** (delegación + INV-S07) → suite completa **1504/1504 BE verdes** sin regresiones. `dotnet build` 0 errores. 5 archivos producción + 1 test, todos cap-OK (mayor: Hangfire Extensions ≈140 ln). Brief movido a `awaiting-prod/069-plan-31-chat-2b-be-bounce-parser-hangfire-deploy.md` por gate post-deploy. **Pendiente verificación post-deploy**: deploy a Azure App Service → confirmar `/hangfire` muestra `bounce-parser-imap` activo con cron `*/5` → tras 5-10 min ≥1 fila en `BounceParserProcessed` → carpeta `Processed/` creada en `sistemas6@` (Roundcube) → NDR fresco end-to-end (correo a destinatario inválido → bounce 5.x.x → `BounceParserProcessed` con `BPR_CorrelationSource="header"` + `EmailOutbox.EO_BounceSource="async-imap"` + `EO_BounceDetectedAt` poblado). Cuando ✅, cerrar 069 vía `/verify` + memoria nueva "Bounce parser IMAP activo desde {fecha}". **Desbloquea Plan 37 Chat 1 (066)** tras 48h de telemetría limpia (decisión 3 del usuario).
