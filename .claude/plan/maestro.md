...
3. **[Plan 28 · Chat 4 · `educa-web` · FE]** — Spliteado 2026-05-08 en sub-chats por dependencias BE descubiertas durante /go (faltan endpoints `/asistente-administrativo/me/*` y `director/asistentes-admin-asistencia-dia` en BE). **4b parcial + 4c ✅ shipped 2026-05-08** — FE puro sobre BE Chat 2 polimórfico ya shipped: 9 archivos. (a) `TipoPersonaAsistencia = 'E'|'P'|'A'` + estadísticas con `totalAsistentesAdmin`/`completasAsistentesAdmin`; (b) admin asistencias UI muestra badge AA `tag-neutral` en tabla mixta + opción `'A'` en filtro y form selectButton + sublabel stats con conteo profesor/AA + label/placeholder del form vía `tipoPersonaLabel`; (c) `notificarExito` en crud-facade soporta `'A' → "Asistente Administrativo"`; (d) `EMAIL_OUTBOX_TIPOS` extendido con `ASISTENCIA_CORRECCION_PROFESOR` + `ASISTENCIA_CORRECCION_ASISTENTE_ADMIN` + nuevo `EMAIL_OUTBOX_TIPO_LABELS` con strings cortos; (e) bandeja outbox `email-outbox-table` y filtro usan `getTipoLabel` para mostrar label humano; (f) composer notificaciones ya tenía "Asistente Administrativo" en `destinatarioRolOptions` (sin cambio). Lint OK + tsc OK. Escape hatch `max-lines` agregado a `attendances.component.ts` con justificación (orchestrador 8-dominios). **4a (self-service AA generalizado: rename + ruta + menú + widget home) y 4b-tab (tab dedicada AA en director-profesores) pendientes — desbloqueados por Chat 3d BE ✅** (controllers `/asistente-administrativo/me/{mes,dia}` y `director/asistentes-admin-asistencia-dia` ya disponibles). **Razón del split**: minimal-from-go reveló endpoints inexistentes; FE puro despachado igual.

> **Plan 28 Chat 5 docs ✅ ship 2026-05-07** (brief 129) — INV-AD08/AD09 formalizados + INV-AD05/06 ampliados + §1.1/§15.4/§17 actualizados. Plan 28 docs al 100%. **Plan 28 Chat 3c ✅ ship 2026-05-08** (brief 128) — BE 100% con cero código de prod (bandeja + notif ya polimórficas). **Plan 28 Chat 3d ✅ awaiting-prod 2026-05-08** (brief 133, en `Educa.API/master`) — endpoints faltantes para Chat 4 FE: nuevo `AsistenteAdministrativoController` (`/me/dia` + `/me/mes`) + `director/asistentes-admin-asistencia-dia` en `ConsultaAsistenciaController`. 9 archivos BE (1 controller nuevo + 1 DTO wrapper + 1 test class nuevo + 6 modificados: repo/service/calculator/estadísticas/controller existente/test existente). +16 tests verdes (13 AA controller con auth reflection × 6 + 3 endpoint admin) → **1709 ✅ / 8 ❌** preexistentes Plan40 documentados. Falta deploy de 3a/3b/3d + Chat 4a/4b-tab FE para 100% funcional. Chat 6 BE gap-fix opcional (autorización condicional INV-AD08, patrón Plan 27) reservado.

### 🟢 Media — mejoras de UX y deuda técnica con dueño claro

4. **[Plan 41 · Chat 2 · `Educa.API` · BE]** ✅ awaiting-prod (brief 132, 2026-05-08) — F2 BE DTO ampliado: `errorGroupCode` (LEFT JOIN ErrorGroup, primeros 12 chars del fingerprint = `FingerprintCorto` del Kanban) + `relatedCorrelationIds` (últimos 5 distinct correlationIds del mismo usuario en ventana 2h, match por últimos 4 dígitos del DNI con `EndsWith` para cubrir storages mixtos masked/raw). **Boundary extraction**: `CorrelationRelatedResolver` (119 líneas, stateless, INV-S07 fail-safe) + `ICorrelationRelatedResolver` registrado en DI; `CorrelationService` quedó en 253 líneas (era 332 inline). Sin migración SQL — campos opcionales. **+8 tests** (7 nuevos del Chat 2 + 1 contract `entidadOrigen`) → 1689/1697 verdes (8 fallos preexistentes Plan 40 F2 sin tocar). FE espejo (`correlation.models.ts`) actualizado con campos opcionales sin romper Chat 1. Pendiente: smoke con BD real (correlation con ErrorGroup + usuario con ≥2 correlations en 1h) → `/verify 132`. Resuelve brechas #4/#5/#7 parcialmente — cierran con F2 FE Chat 3. Siguiente: **Chat 3 FE** (botones "Ver grupo de errores" / "Ver bandeja" / "Ver reporte" + chips `<app-correlation-id-pill>` para `relatedCorrelationIds`). **Feedback al usuario**: ¿alcance del `relatedCorrelationIds` (mismo usuario / ventana 2h / cap 5) OK, o lo cambiamos antes de Chat 3?
5. **[Plan 41 · Chat 1 · `educa-web` · FE]** ✅ awaiting-prod (brief 131, 2026-05-08) — F1 timeline cronológico unificado shipped. Resuelve brechas #1 y #12. Pendiente smoke manual + `/verify`.
6. **[Plan 26 · F3 · `Educa.API` · BE]** — Time-of-day modifier para rate limiting. F2.6 ya cumplió ventana de observación post-deploy (~2026-05-06). Diseño aprobado: margen suave fuera de franja escolar (ej: x1.5 dentro / x1.2 fuera), no corte duro. Calibración con telemetría real `RateLimitEvent`.
7. **[Plan 33 · Chat 1 · BE+FE]** — Auditoría de paginación de tablas. Clasificar 8 features candidatas (`attendances admin`, `attendance-reports cross-role`, `email-outbox-diagnostico`, `email-outbox-dashboard-dia`, `attendance-day-list`, `responsive-table`, `student-attendance-tab`, `attendance-summary-panel`) y aplicar fix tipo `error-logs admin` donde corresponda. Plan a 5%.
8. **[WAL Cache Audit · H7 · `educa-web` · FE]** — Normalizar naming `WAL_CACHE_MAP` a camelCase. P1, 1 chat dedicado, riesgo medio-alto (rename cross-archivo).
9. **[WAL Cache Audit · H2-H6+H10 · `educa-web` · FE]** — Cleanup cosmético + duplicación de patrones. P2, 1 chat, bajo riesgo.
