...
3. **[Plan 30b · FE · `educa-web` · FE]** — espejar DTO `EntradaConCorreoEnviado` y campo `entradasConCorreoEnviado` en `correos-dia.models.ts`, crear componente `entradas-con-correo-table` (simétrico a `entradas-sin-correo-table`, variante verde/success), agregar 4to sub-tab "Entradas con correo (N)" en `tab-correos-dia.component.html`, actualizar `correos-dia.store.ts` y tests. Brief a generar al arrancar el chat. Habilitado por commit BE `0ee5848` (Plan 30b BE cerrado 2026-04-27).
