...
| **29** | **🔴 Corte de cascada SMTP (`max_defer_fail_percentage`)** | **BE+OPS** | **(inline en maestro — 4 chats, no justifica archivo dedicado)** | **🟢 Chat 2.5 BE ✅ cerrado 2026-04-23 (commit `0580983` en Educa.API `master`). Gap (1) del Chat 2 resuelto: removido whitelist `TiposValidadosAlEncolar`, `EmailValidator.Validate` corre sin condicional en todos los `EnqueueAsync`. `NotificacionAdmin`/`ReporteUsuario`/`ReporteFallosCorreoAsistencia`/`AsistenciaCorreccion*` con formato inválido ahora producen `EO_Estado='FAILED'` + `ErrorLog` vía `EmailFailureLogger` (INV-ET02) en vez de cruzar al SMTP. Hallazgo: `Caso D` del `EmailOutboxServiceTests` existente asumía el bug (`NotificacionAdmin + ñ → PENDING`) — ajustado al comportamiento correcto. **+11 tests parametrizados** → **1253 BE verdes** (baseline 1242). `INV-MAIL01` universal. Gap (2) `DeferFailStatus` sigue pendiente → Chat 2.6 BE. 🟢 Chat 2 BE ✅ cerrado 2026-04-22 (commit `674e86a`, +35 tests, 1242 verdes). 🟢 Chat 1 `/design` ✅ cerrado 2026-04-22. **Siguiente: Chat 3 OPS (inspección CrossChex + negociación umbral hosting)** + Chat 2.6 BE fix (endpoint `DeferFailStatus` para Plan 22 Chat B).** | **~60%** |
...
**Foco (actualizado 2026-04-23, post-cierre Plan 29 Chat 2.5)**: 🟢 **Plan 29 Chat 2.5 BE ✅ cerrado** (commit `0580983` en Educa.API `master`). Removido el whitelist `TiposValidadosAlEncolar`: `EmailValidator.Validate` corre en cada `EnqueueAsync` sin condicional. Tipos no-asistencia con formato inválido ya no cruzan al SMTP. Hallazgo: el `Caso D` del `EmailOutboxServiceTests` asumía el bug explícitamente (`NotificacionAdmin + ñ → PENDING`) — ajustado al nuevo comportamiento correcto. **+11 tests parametrizados** → **1253 BE verdes** (baseline 1242). `INV-MAIL01` ahora universal. 🟢 **Plan 29 Chat 2 BE ✅ cerrado 2026-04-22** (commit `674e86a`, +35 tests). 🔴 **Siguiente por prioridad**: Chat 3 OPS (inspección CrossChex + negociación umbral hosting con el admin cPanel) + Chat 2.6 BE (exponer endpoint agregado `DeferFailStatus` — deuda D4 para desbloquear Plan 22 Chat B widget). Post-deploy: 48-72h monitoreo 0 `SslHandshakeException` + contador defer/fail cPanel bajando antes de Chat 4 docs. 🟢 **Plan 28 Chat 3 desbloqueado**. 🟢 **Plan 27 cerrado en docs + código completo** — `INV-C11` formalizado, pendiente solo validación del jefe post-deploy. **Frentes bloqueados por Plan 29**: Plan 22 Chat B (widget `DeferFailStatus`) sigue bloqueado hasta Chat 2.6 BE materialice los agregados. **Frentes abiertos sin bloqueo**: Plan 26 F2 Chat 2, Design System F5.3, Carril D Olas 2+.
...
> **Estado**: 🟢 **Chat 2.5 BE ✅ cerrado 2026-04-23** (commit `0580983`, +11 tests, 1253 BE verdes). Gap (1) validación formato universal. 🟢 **Chat 2 BE ✅ cerrado 2026-04-22** (commit `674e86a`, +35 tests, 1242 BE verdes). 🟢 Chat 1 `/design` ✅ cerrado 2026-04-22. Siguiente: Chat 3 OPS + Chat 2.6 BE (endpoint `DeferFailStatus` para Plan 22 Chat B — gap 2 restante).
...
| D4 | Widget Plan 22 Chat B incluye `DeferFailStatus` además de `ThrottleStatus` (métricas defer/fail 5/h por dominio) | **Chat 2 BE ✅ dejó la BD preparada pero NO expuso endpoint** — `EmailOutbox` ahora tiene estados `FAILED_BLACKLISTED` y `FAILED_THROTTLE_HOST` + `EmailBlacklist.EBL_IntentosFallidos` queryable. El widget FE puede consultar por query directa hoy, pero el service/endpoint `GET /api/email-outbox/defer-fail-status` con agregados empaquetados queda para **Chat 2.6 BE** (separado del Chat 2.5 que ya cerró el gap de validación de formato). |
...
2. Los agregados `DeferFailStatus` de la deuda D4 (widget Plan 22 Chat B) no quedaron expuestos como endpoint. Los contadores existen en `EmailOutbox` (`EO_Estado IN ('FAILED_BLACKLISTED','FAILED_THROTTLE_HOST','FAILED')` + `EO_TipoFallo`), pero falta el service/endpoint que los empaquete. Hoy la UI admin puede leer via query directa; para el widget FE necesita materializarse. **Pendiente Chat 2.6 BE** (o fusionar con Plan 22 Chat B cuando se retome).
