...
> **Fecha**: 2026-04-14 (última revisión: 2026-04-22, **Plan 27 Chat 5c ✅ cerrado** — gap fix final `INV-C11` en envío masivo admin. `AsistenciaEmailDataRow` + `AsistenciaAdminRepository.GetEmailDataByIds/ById` ahora proyectan `int? GraOrden` vía subquery correlacionada (INV-D09). `AsistenciaAdminBulkEmailService` propaga `GraOrden` en las 2 invocaciones a `EnviarNotificacionAsistencia`. 1167 BE + 1509 FE tests verdes. **Pendiente solo validación del jefe post-deploy** para declarar Plan 27 100% archivable)
