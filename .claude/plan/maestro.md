...
| **32** | **🟢 Centralización de errores vía Correlation ID** | **BE+FE** | **[plan/correlation-id-links.md](./correlation-id-links.md)** | **🟢 Chat 1 `/design` ✅ cerrado 2026-04-24. 11 decisiones acordadas + 4 chats definidos (1 design + 2 BE + 1 FE). Hub-and-spoke con pantalla dedicada `/intranet/admin/correlation/:id` + pill reusable `<app-correlation-id-pill>` (design-system A1 tag-neutral). Endpoint unificado `GET /api/sistema/correlation/{id}` devuelve 4 fuentes (ErrorLog + RateLimitEvent + ReporteUsuario + EmailOutbox) con fail-safe INV-S07 por query. **Scope extendido vs brief original**: EmailOutbox gana columna `EO_CorrelationId` + hook en `EmailOutboxService.EnqueueAsync` desde `HttpContext.Items["CorrelationId"]` (middleware ya lo provee) — sin backfill, históricos quedan NULL. Plan file dedicado (no inline en maestro) por decisión del usuario. **Siguiente**: Chat 2 BE — `ALTER TABLE EmailOutbox ADD EO_CorrelationId` + hook en EnqueueAsync + tests (precondición de Chat 3). Chat 3 BE luego — SELECT previo sobre `sys.indexes` + hasta 4 CREATE INDEX filtrados + endpoint + service con `CorrelationSnapshotFactory.BuildEmpty` (patrón Plan 30). Chat 4 FE último — hub page + pill + wiring + query param sync en los 4 dashboards (error-logs, rate-limit-events, feedback-reports, email-outbox).** | **~10%** |
