...
1. **[Plan 31 · Chat 2 · `Educa.API` · BE]** — Parser IMAP + Hangfire job + tabla `BounceParserProcessed`. `BounceParserService` + partial `.Imap.cs` + `Ndr3464Parser` + `BounceCorrelator` + `BounceParserJob` recurring `*/5 * * * *`. Procesa `INBOX`/`Junk`/`Trash` de los 7 buzones del pool Plan 22 F6. **🟢 DESBLOQUEADO 2026-04-28** — header `X-Educa-Outbox-Id` validado en Roundcube post-deploy de Chat 1 (sample real `EO_CodID=3617` en correo `sistemas@laazulitasac.com → rosmeryrodrigodiaz@gmail.com`). Brief en `.claude/chats/038-plan-31-chat-2-be-bounce-parser-imap-job.md` listo para arrancar.
