...
| BAudit2 | Auditoría buenas prácticas Angular 22/TS 6.0 | F1 ✅ · F2 FE ✅ awaiting-prod (664, BE handoff 667 ya resuelto) · F3 FE ✅ awaiting-prod (665) · F4 ✅ closed (666) · F5 ✅ awaiting-prod (679) · F6 ✅ closed (680) · F7 ✅ awaiting-prod (681, pendiente merge 681-B en Educa.API) · **F8-F12 briefs abiertos** (682-686) | 🟡 |
...
**Siguiente accionable**: **BAudit2 F8** (Tier 0, pos 0.5) — performance `@defer`/lazy-loading/leaks, brief [682](../chats/open/682-audit-f8-performance-defer-lazy-loading-leaks.md). Tier 0 reordenado 2026-09-12 a pedido explícito del usuario para priorizar el resto del audit Angular 22/TS 6.0 (F4-F12) por sobre xP41. Audit F4 (666, pos 0.1) ✅ cerrado (9/9 fixes). Audit F5 (679, pos 0.2) ✅ cerrado, awaiting-prod. Audit F6 (680, pos 0.3) ✅ cerrado y mergeado a `main`. Audit F7 (681, pos 0.4) ✅ completo (SEO + contacto validados end-to-end, incl. regresión encontrada y corregida en `PublicSeoService`) — pendiente únicamente el merge de brief backend 681-B a `main` de Educa.API (chat aparte, one-repo-one-chat). xP41 F3 BE (persist request lifecycle) queda como siguiente tras cerrar Tier 0. Admin block cerrado (P62-P66 ✅/descartado). P50 F3a ✅ (331 closed).
...
- ⏳ **665** — 8 fixes de race conditions/cancelación en facades (BAudit2 F3). Verificación visual del punto 3 (`campus-admin`) hecha en local contra `TestConnection`, falta confirmar en `educa.com.pe/intranet` real.
- ⏳ **679** — 9 fixes `setDisabledState` + 5 fixes accesibilidad teclado/ARIA en `edu-ui` (BAudit2 F5), incluyendo fix de `aria-activedescendant` en select/multi-select. Verificado en vivo local (backend+FE reales), falta smoke test visual/teclado en `educa.com.pe/intranet` real por ser librería compartida en toda la intranet.

### Notas operativas

- **`running/`**: vacío · **`open/`**: 6 briefs (658, 682-686) — **excede intencionalmente** el soft cap de `backlog-hygiene.md` (≤5): generación completa de briefs del audit FE a pedido explícito del usuario 2026-09-12, excepción documentada para fases de audit deliberadamente pesadas · **`awaiting-prod/`**: 7 briefs (462, 578, 664, 665, 679, 681, 688) · **`waiting/`**: vacío · **`troubles/`**: vacío
- **Último cierre**: 681 (BAudit2 F7 — retomado desde un worktree interrumpido con trabajo real previo: SEO (punto 3, `PublicSeoService`) ya commiteado, formulario de contacto (punto 1) listo en FE pero pausado esperando brief backend 681-B. 681-B resultó ya cerrado en Educa.API (branch `chat/681-B-endpoint-contacto`, sin mergear a `main` todavía). Validación end-to-end real: BE levantado local en esa branch + FE `ng serve`, formulario enviado desde la UI, `POST /api/Contacto` → 200, email encolado en outbox. **Hallazgo y fix durante la validación**: `PublicSeoService.deepestRouteData()` rompía **todas** las rutas públicas en runtime (pantalla en blanco, no solo `/contacto`) por leer `ActivatedRoute.snapshot` en un punto donde podía ser `undefined` (carrera agravada por el zoneless de brief 680); fix cambiando a `router.routerState.snapshot` (síncrono) + test de regresión nuevo (`public-seo.service.spec.ts`). Lint 0 errores, build verde, 2576/2576 tests verdes (1 timeout flaky no relacionado en `eslint-config-guards.spec.ts`, confirmado pasando en re-run aislado)) → `awaiting-prod/` (pendiente merge de 681-B a `main` de Educa.API antes de validar en `educa.com.pe`), worktree `chat/681-audit-f7-sitio-publico-contacto-seo` (pendiente `/wt-merge`), 2026-09-15.
