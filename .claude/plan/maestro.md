...
19. **[095 → 102 · `Educa.API` + `educa-web` · BE+FE]** — Runtime health monitoring. `095` (`/design`, cerrado 2026-05-05) cerró las 4 decisiones de arquitectura (sliding window p95 in-process, EventListener `Microsoft.Data.SqlClient.EventSource` para pool stats, página dedicada `/intranet/admin/sistema/runtime-health`, snapshot puntual sin serie). `102` (`/execute`, brief en `chats/open/102-be-runtime-health-execute.md`) implementa: endpoint `GET /api/sistema/runtime-health` con cache `IMemoryCache` 10s + `[Authorize(Roles = "Director")]` + `RuntimeHealthSnapshot` DTO, `UpDownCounter educa.requests.in_flight` en `AppMetrics`, `RequestPercentileTracker` (sliding 5min con buckets logarítmicos), `SqlClientCounterListener : EventListener`, `SaturationClassifier` (función pura testeable que mapea ThreadPool/Requests/DB/GC → A/B/C: STARVATION / EXTERNAL_BOTTLENECK / OVERLOAD / OK), página FE smart + widget presentacional con polling 60s + feature flag `runtimeHealth` (OFF prod, ON dev) + permiso `ADMIN_SISTEMA_RUNTIME_HEALTH`. **Bloquea** brief `096-be-load-control-layers` (capas 2-6: SemaphoreSlim global + bulkheads + timeouts + Retry-After). **Prioridad**: media — después de que se vacíe el `awaiting-prod` actual (items 1-7 + 10-18). Sin dependencias activas.
