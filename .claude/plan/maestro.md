...
> **Fecha**: 2026-04-14 (última revisión: 2026-04-22, **Plan 28 🟢 Chat 1 `/design` ✅ cerrado** — 8 decisiones resueltas: alcance B-amplio acotado al rol "Asistente Administrativo" (4 personas hoy) + `TipoPersona='A'` en `AsistenciaPersona` + dispatch `Profesor → Director(AA) → Estudiante → rechazar` + correos diferenciados (`INV-AD09`) + self-service generalizado + horarios idénticos a profesor + principio `INV-AD08` "ningún rol administrativo corrige asistencia de su propio rol". Plan queda **inline** (6 chats no justifican archivo dedicado). **Chat 2 BE bloqueado hasta validación del jefe del Plan 27 post-deploy** (evita PRs simultáneos sobre `AsistenciaPersona` + `EmailNotificationService`). Plan 27 Chat 5c ✅ cerrado el mismo día — gap fix final `INV-C11` en envío masivo admin, 1167 BE + 1509 FE tests verdes, pendiente solo validación del jefe post-deploy)
...
**Foco (actualizado 2026-04-22, post-Chat 1 Plan 28)**: 🟢 **Plan 28 Chat 1 `/design` ✅ cerrado** con 8 decisiones resueltas. Plan inline, 6 chats confirmados. **Chat 2 BE bloqueado hasta validación del jefe del Plan 27 post-deploy** — ambos planes tocan `AsistenciaPersona` + `EmailNotificationService`, arrancar Chat 2 antes generaría PRs simultáneos sobre los mismos archivos con merge conflicts asegurados. Cuando se desbloquee: Chat 2 = migración SQL (CHECK `('E','P')` → `('E','P','A')` + script de verificación cross-tabla DNI) + extensión del dispatch polimórfico + 6 queries Plan 21/23 extendidas + lookup `Director(rol=AA)` en `AsistenciaRepository`. 🟢 **Plan 27 cerrado en docs + código completo** — `INV-C11` formalizado en `business-rules.md` (§1.11 + §15.4), pendiente solo validación del jefe post-deploy para archivarse en `history/planes-cerrados.md`. **Frentes abiertos sin bloqueo** (pueden avanzar en paralelo mientras Plan 28 Chat 2 espera): Plan 22 F4/F5.6 BE, Plan 26 F2 Chat 2 (aplicar `[RateLimitOverride]` a 14+2 endpoints), Design System F5.3, Carril D Olas 2+.
...
| **Chat 2 — BE: modelo + dispatch + queries** | **🔒 Bloqueado hasta validación del jefe Plan 27 post-deploy.** Script SQL de verificación cross-tabla DNI (Estudiante + Profesor + Director activos sin colisiones) ejecutado ANTES del deploy. Migración: CHECK `('E','P')` → `('E','P','A')`. Extensión del dispatch polimórfico en `AsistenciaService.ClasificarYRegistrarMarcacionAsync` con 3er paso `Director(rol=AA)` entre Profesor y Estudiante. Lookup nuevo `GetAsistenteAdminPorDniAsync` en `IAsistenciaRepository`. Actualizar 6 queries de `ConsultaAsistenciaRepository` + 2 de `AsistenciaAdminQueryRepository` + 3 de `ReporteAsistenciaRepository` para contemplar `'A'`. Tests: dispatch, lookup, coherencia horaria reusando branch `'P'`. | BE | 1 chat |
