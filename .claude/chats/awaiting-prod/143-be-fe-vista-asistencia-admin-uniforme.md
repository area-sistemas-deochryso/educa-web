...
> **Validación prod**: ⚠️ regresión 2026-05-19 (Cowork local BD prueba) — 3/4 roles OK uniformes en `/intranet` widget + `/intranet/asistencia` panel admin (Director, Promotor MEDINA, Coord.Académico TREJO). **AA YARUPAITA rompe**: en `/intranet/asistencia` ve panel admin completo (tabs Estudiantes/Profesores/AA + filtro INV-C11 + dropdowns grado/sección) cuando el brief 134 parte 4a definió que AA debe ver self-service. Ver hallazgo F-022 en `chats/open/`.
