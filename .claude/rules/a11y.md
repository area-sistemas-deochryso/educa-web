...
| Uso | Fondo | Texto/Borde | Contraste |
|-----|-------|-------------|-----------|
...
| **Acento azul sobre fondo claro** | fondo claro | `#1e40af` (blue-800) | 7:1+ |
| **Fondo de sección destacada** | `#dbeafe` (blue-100) | `#1e40af` (blue-800) | 7:1+ |

## Azul sobre fondo claro — NUNCA celeste

> **"Si el fondo es claro, el acento es azul oscuro (#1e40af), no celeste."**

El tema Aura usa indigo/celeste como `--primary-color`. Sobre fondos claros, el celeste tiene bajo contraste visual y se ve lavado. **SIEMPRE** usar colores explícitos en lugar de variables del tema para acentos sobre fondo claro.

```scss
// ✅ CORRECTO - Azul oscuro explícito sobre fondo claro
.stat-card--highlight {
...
// ❌ INCORRECTO - Variables del tema que renderizan celeste
.stat-card--highlight {
  border-color: var(--primary-color);   // Celeste ~#6366f1
}
.section-header {
  background: var(--surface-100);       // Celeste claro del tema
  color: var(--primary-color);          // Celeste
}
```

| Caso de uso | ✅ Usar | ❌ NO usar |
|-------------|---------|-----------|
| Texto/borde destacado sobre fondo claro | `#1e40af` (blue-800) | `var(--primary-color)` |
| Fondo de sección/header ligero | `#dbeafe` (blue-100) | `var(--surface-100)` |
| Botón primario con texto blanco | `#4f46e5` (indigo-600) OK | — |
| Texto sobre fondo oscuro | `#ffffff` OK | — |
