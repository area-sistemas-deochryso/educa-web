...
| **Regular** | Profesor (P) **o Asistente Admin (A)** | Mar-Dic | `[05:00, 07:31)` | `[07:31, 09:30)` | `≥ 09:30` o sin marcación |
| **Verano**  | Ambos (E/P/A)  | Ene-Feb | `08:30 – 09:20` (inicio + 50min) | `09:20 – 10:30` (inicio + 2h) | `> 10:30` o sin marcación |

> **Plan 28 Chat 1 — decisión 6**: `'A'` (Asistente Administrativo) reusa los thresholds de `'P'`. Zero branch nuevo en `EstadoAsistenciaCalculator` ni `CoherenciaHorariaValidator` — `'A'` cae en el branch de `'P'`.
...
**Ventana mínima de salida para estudiantes (INV-C09)**: Marcaciones de salida de estudiantes **antes de 13:55** se descartan silenciosamente (solo periodo regular). Profesores y Asistentes Administrativos no tienen ventana mínima de salida — pueden salir tarde sin restricción. Verano no aplica guard de salida temprana. INV-C09 es `'E'`-only por construcción.
...
**Vista unificada para los 4 administrativos (Brief 143/145, 2026-05-12)**: por orden de jefatura, los 4 roles administrativos (Director, Asistente Administrativo, Promotor, Coordinador Académico) comparten la misma vista en `/intranet` (home con `AttendanceSummaryWidget`) y `/intranet/asistencia` (panel `AttendanceDirectorComponent` con sub-tabs Estudiantes/Profesores/AsistentesAdmin). Las mutaciones desde `AsistenciaAdminController` también están abiertas a los 4 — no hay guard fino por `TipoPersona='A'`. Esta decisión revierte el modelo "AA self-service read-only" introducido por Plan 28 Chat 3d/4a; el `AsistenteAdministrativoController` y sus endpoints `/me/dia`, `/me/mes` fueron eliminados. La auditoría de quién mutó qué se mantiene vía el correo de corrección (INV-AD05/AD09) y los campos `UsuarioMod` / `FechaMod` de cada registro.
...
| Consultas admin día (`ConsultaAsistenciaRepository`, 4 queries + `AsistenciaAdminQueryRepository` 2 queries) | Filtran `GRA_Orden >= 8`. Listado del día, estadísticas, reporte filtrado, tabla mensual profesor/director, día puntual profesor/director. Profesores (`TipoPersona = 'P'`) intactos. |
...
| Correos (`EmailNotificationService.EnviarNotificacionAsistencia` y `EnviarNotificacionAsistenciaCorreccion`, además del **envío masivo** admin `AsistenciaAdminBulkEmailService.EnviarCorreosMasivosAsync` que propaga `GraOrden` desde la proyección `AsistenciaEmailDataRow` vía subquery en `AsistenciaAdminRepository.GetEmailDataByIdsAsync`, filtrada por `INV-D09`) | Early-return cuando `graOrden < 8`. Cubre correos de marcación en tiempo real, correcciones admin (INV-AD05) **y** reenvío masivo de correos históricos (Chat 5c). El outbox queda genérico. |
...
| Vistas profesor (Chat 5b, 2026-04-22) | `attendance-profesor-estudiantes` (tabla mensual + día puntual) y `profesor-attendance-widget` del home renderizan `<app-attendance-scope-student-notice [nombre]="salonLabel">` cuando el salón del profesor tiene `GRA_Orden < 8` (consume `SalonProfesorDto.GraOrden`). La lista de salones del profesor **no** se filtra — el salón sigue visible para que pueda verse el notice. |
| Widget "Asistencia de Hoy" (home director) | Sin código propio de filtro — consume `/api/ConsultaAsistencia/director/estadisticas` que ya filtra en backend. Numerador y denominador vienen del mismo objeto filtrado. |
...
| `INV-C11` | Asistencia | Marcaciones CrossChex de estudiantes (`TipoPersona = 'E'`) con `GRA_Orden < UmbralGradoAsistenciaDiaria` (hoy `8` = 5to Primaria) se descartan silenciosamente (log `Information` con DNI enmascarado, sin registro). Consultas admin, reportes PDF/Excel y correos aplican el mismo filtro. Admin conserva edición sobre registros históricos (no existe `INV-AD07`). Profesores y Asistentes Administrativos no aplican (exentos por construcción — no tienen `GRA_Orden`). Revertir = bajar `UmbralGradoAsistenciaDiaria` en BE + `UMBRAL_GRADO_ASISTENCIA_DIARIA` en FE (mirror) y redeployar | Webhook: `CoherenciaHorariaValidator.Clasificar` → `MarcacionAccion.IgnorarGradoFueraAlcance` consumido por `AsistenciaService.ClasificarYRegistrarMarcacionAsync`. Lectura: filtros en `ConsultaAsistenciaRepository` (4 queries: `ObtenerEstudiantesReporteAsync`, `ObtenerEstadisticasDiaRawAsync`, `ObtenerEstudiantesPorGradoConAsistenciasAsync`, `ObtenerEstudiantesPorDiaAsync`), `AsistenciaAdminQueryRepository` (2 queries), `ReporteAsistenciaRepository` (3 queries). Correos: early-return en `EmailNotificationService.EnviarNotificacionAsistencia` y `EnviarNotificacionAsistenciaCorreccion`; `AsistenciaAdminBulkEmailService.EnviarCorreosMasivosAsync` (bulk admin) propaga `GraOrden` desde `AsistenciaEmailDataRow` (subquery en `AsistenciaAdminRepository.GetEmailDataByIdsAsync` + `GetEmailDataByIdAsync`, filtrada por `INV-D09`). FE profesor: `salonFueraAlcance` computed en `attendance-profesor-estudiantes` y `profesor-attendance-widget` consume `SalonProfesorDto.GraOrden` para renderizar notice | 1.11 |
...
| `INV-AD05` | Asistencia | `AsistenciaAdminService` envía correo diferenciado en cada operación, distinto del correo de marcación en tiempo real. Destinatarios polimórficos por `TipoPersona`: **E** → apoderado (`EST_CorreoApoderado`); **P** → profesor (`PRO_Correo`); **A** → el propio Asistente Administrativo (`Director.DIR_Correo` filtrado por rol). Destinatario único (sin BCC). Sin correo del destinatario → se omite silenciosamente. | `EmailNotificationService.EnviarNotificacionAsistenciaCorreccion()` + `EnviarNotificacionAsistenciaCorreccionProfesor()` + `EnviarNotificacionAsistenciaCorreccionAsistenteAdmin()`, vía `IAsistenciaAdminEmailNotifier`. Fire-and-forget (INV-S07). | 1.8 |
| `INV-AD06` | Asistencia | Un profesor no puede autojustificarse ni justificar/corregir asistencia de un colega profesor. Toda mutación sobre `AsistenciaPersona` con `TipoPersona = 'P'` requiere rol administrativo (Director, Asistente Administrativo, Promotor o Coordinador Académico). El profesor NO es rol administrativo, por eso esta separación se mantiene aunque INV-AD08 haya sido revertida para `'A'`. | `AsistenciaAdminController` con `[Authorize(Roles = Roles.Administrativos)]` (4 roles). Verificado por `AsistenciaAdminControllerAuthorizationTests` (6 tests por reflection). | 1.8 |
| `INV-AD08` | Asistencia | **Revisada (Brief 143/145, 2026-05-12)**: los 4 roles administrativos pueden mutar `AsistenciaPersona` independientemente del `TipoPersona` del registro, incluida su propia asistencia. La separación de poderes para `'A'` introducida por Plan 28 Chat 1 (decisión 7) fue revertida por orden de jefatura — el guard fino server-side nunca se implementó y la UI pasó a estar unificada para los 4. INV-AD06 (profesor no se autojustifica) **sigue vigente** porque el profesor NO es rol administrativo. La auditoría operativa de quién mutó qué la cubre el correo INV-AD09 + los campos `UsuarioMod`/`FechaMod`. | `[Authorize(Roles = Roles.Administrativos)]` único en `AsistenciaAdminController` cubre los 4. La constante `Roles.SupervisoresAsistenteAdmin` queda como label histórico en `Constants/Auth/Roles.cs:34` — no enforza nada. | 1.8 |
| `INV-AD09` | Asistencia | Mutaciones admin sobre `AsistenciaPersona` con `TipoPersona='A'` envían correo de corrección al AA destinatario del registro (`Director.DIR_Correo` filtrado por `DIR_UsuarioReg='Asistente Administrativo'`) como destinatario único. Tras la reversión de INV-AD08 (Brief 143/145), el **mutador puede ser el mismo AA destinatario** — el correo y los campos `UsuarioMod`/`FechaMod` siguen siendo la traza de auditoría aunque mutador y destinatario coincidan. Etiquetas outbox: `EO_Tipo='ASISTENCIA_CORRECCION_ASISTENTE_ADMIN'`, `EO_TipoEntidadOrigen='AsistenciaAsistenteAdmin'`. Sin BCC, sin apoderado. Plantilla azul administrativa con saludo `'Estimado/a {nombre}'` y descripción `'su asistencia administrativa fue {operacion} manualmente por la dirección del colegio'`. Si `DIR_Correo` está vacío, se omite silenciosamente (fire-and-forget INV-S07). Simetría con INV-AD05 ampliado a profesores (Plan 23 Chat 4). | `EmailNotificationService.EnviarNotificacionAsistenciaCorreccionAsistenteAdmin()` + template, vía `IAsistenciaAdminEmailNotifier.NotificarCorreccionAsistenteAdminAsync` / `NotificarEliminacionAsistenteAdminAsync` (Plan 28 Chat 3b BE, awaiting-prod brief 127). | 1.8 |
...
| `INV-ET06` | ErrorGroup | `ERG_Fingerprint` se computa con `SHA-256(severidad ‖ httpMethod ‖ normalize(url) ‖ httpStatus ‖ errorCode ‖ messageContrib(errorCode, mensaje))` por `ErrorFingerprintCalculator`. `messageContrib` retorna `""` para errorCodes en la whitelist `{SLOW_REQUEST, SLOW_REQUEST_FAILED}` (mensaje variable por naturaleza, no aporta a identidad del bug) y `NormalizeMensaje(mensaje)` para el resto. Separador NUL `\0`. Cambiar el algoritmo requiere job de re-fingerprint + merge documentado (ver ADR-0007) | `Helpers/Sanitization/ErrorFingerprintCalculator.cs` (Chat 2) + `Services/Sistema/ErrorGroupBackfillService.cs` (ADR-0007) | 19 |
...
| `INV-CORR01` | Correlation ID | El correlation id (`ERL_CorrelationId`, `REU_CorrelationId`, `EO_CorrelationId`, `REL_CorrelationId`) correlaciona eventos **dentro del mismo request HTTP del servidor**. Requests independientes — típicamente acción del admin en intranet (request del FE) vs webhook CrossChex que encola correo automático (request del CrossChex) — tienen ids distintos por diseño. El hub `/intranet/admin/correlation/:id` solo cruza carriles cuando los eventos comparten request boundary. **Formato canónico uniforme**: GUID-36 con guiones (`Guid.NewGuid().ToString()` D format) en los 4 carriles. El FE manda `X-Request-Id: crypto.randomUUID()`; el BE middleware lo usa o genera GUID-36 si el header viene vacío (webhooks externos, background jobs). OTel `Activity.TraceId` queda como canal separado para Azure Monitor — NO se usa como fuente del correlation id de BD | `CorrelationIdMiddleware.cs` (fallback GUID D) + `RateLimitTelemetryMiddleware.cs` (lee de `HttpContext.Items[CorrelationIdItemKey]`) + `EmailOutboxService.ResolveCorrelationId` + tests regresión GUID-36 (Plan 43 Chat 1.3) | — |
...
| `INV-MAIL02` | EmailBlacklist | Cuando un destinatario acumula ≥ 3 rebotes permanentes 5.x.x del mismo destinatario (`FailedInvalidAddress` o `FailedRejected`), `EmailBounceBlacklistHandler` lo inserta en `EmailBlacklist` (`MotivoBloqueo = 'BOUNCE_5XX'`) dentro de la misma transacción que actualiza el registro a `EO_Estado = 'FAILED_BLACKLISTED'`. **`FailedMailboxFull` (4.2.2 / 5.2.2) NO cuenta para este umbral** (Plan 38 Chat 2 — D2/D14) — tiene handler propio con ventana temporal y motivo separado, ver `INV-MAIL07`. SSL handshake, timeouts, `535 auth fail` y rechazos tipo `max defers and failures per hour` del MTA del hosting **no** cuentan. | `EmailBounceBlacklistHandler.HandleSyncFailureAsync` / `HandleBounceAsync` con `HashSet<string> TiposBouncePermanente = { FailedInvalidAddress, FailedRejected }` | 18.2 |
| `INV-MAIL03` | Hosting (externo) | El hosting cPanel descarta silenciosamente todo correo del dominio `laazulitasac.com` cuando acumula <!-- TBD post-OPS: Chat 3 puede subir a 25-30/h --> 5 defers+fails en una hora. Este umbral es política del hosting y no es configurable. `INV-MAIL01`, `INV-MAIL02` y `INV-MAIL07` son las únicas defensas disponibles — el sistema no controla el contador, solo qué envía al MTA. Cualquier fallo evitable (SSL handshake, auth, formato inválido) consume el contador y se considera deuda que agota la cuota para correos legítimos. | **No enforced en código** — política del hosting. Se honra vía INV-MAIL01/02/07 que evitan consumir el contador | 18.1 |
| `INV-MAIL04` | EmailOutbox + widget admin | El sistema expone `GET /api/sistema/email-outbox/defer-fail-status` con el contador de la hora actual vs threshold configurable `Email:DeferFailThresholdPerHour` (default <!-- TBD post-OPS: alinear con INV-MAIL03 si OPS sube el techo --> 5, ajustable en caliente sin redeploy cuando el hosting negocia un valor superior). El semáforo se calcula por bandas — OK (<60%), WARNING (60-100%), CRITICAL (≥100%). El widget admin `DeferFailStatusWidget` (Plan 22 Chat B) consume este endpoint cada 60s para visibilizar el riesgo antes de que el hosting bloquee el dominio. Service BE con fallback-CRITICAL en `try/catch` global (INV-S07) — un fallo interno de telemetría nunca falla el envío de correos, pero tampoco oculta un problema real. Plan 38 Chat 6 agrega push SignalR vía `EmailHub` cuando el contador o el handler 4.2.2 disparan. | `GET /api/sistema/email-outbox/defer-fail-status` (Plan 29 Chat 2.6) + `DeferFailStatusWidget` FE (Plan 22 Chat B) + `EmailHub` (Plan 38 Chat 6) | 18.2, 18.4 |
| `INV-MAIL06` | EmailBlacklist | Una entrada con `EBL_MotivoBloqueo = 'BOUNCE_MAILBOX_FULL'` y sin actividad (`EBL_FechaUltimoFallo`) en los últimos `EmailSettings.MailboxFullCleanupDays` (default 7) pasa a `EBL_Estado = false` automáticamente vía `BlacklistAutoCleanupJob` (Hangfire diario 03:00 hora Perú). Otros motivos (`BOUNCE_5XX`, `MANUAL`, `BULK_IMPORT`, `FORMAT_INVALID`) no se despejan automáticamente — requieren `DELETE` explícito. Permite que destinatarios que liberaron espacio sean re-permitidos sin intervención manual; si el problema persiste, el handler 4.2.2 los re-blacklistea al 2º hit. | `BlacklistAutoCleanupJob` (Plan 38 Chat 4) + repo `MarcarDespejadasPorAntiguedadAsync` filtrando `motivo = 'BOUNCE_MAILBOX_FULL'` | 18.2 |
| `INV-MAIL07` | EmailBlacklist | Cuando un destinatario acumula `EmailSettings.MailboxFullThresholdHits` (default 2) hits con `EO_TipoFallo = 'FAILED_MAILBOX_FULL'` en `EmailSettings.MailboxFullThresholdHours` (default 24h), `MailboxFullBlacklistHandler` lo inserta en `EmailBlacklist` con `MotivoBloqueo = 'BOUNCE_MAILBOX_FULL'`. Aplica tanto en flujo sync (`EmailOutboxWorker`) como async (`BounceParserService` con `bounceSource = "async-imap"`). El despeje automático lo provee `INV-MAIL06`. Razón: 4.2.2 transitorio repetido es síntoma de buzón lleno crónico — Exim local del hosting reintenta el mismo mensaje cada 15-30 min hasta agotar política (4d), y cada reintento contra Gmail consume el contador `INV-MAIL03` (5/h). Cortar al 2º hit evita agotar el techo. | `MailboxFullBlacklistHandler` (Plan 38 Chat 2) registrado como `Scoped` + invocado desde el worker antes que `EmailBounceBlacklistHandler` y desde `BounceParserService` cuando el código del NDR es 4.2.2 / 5.2.2 | 18.2 |
| `INV-MAIL08` | EmailMonitoreoService | Las 5 agregaciones del dashboard de monitoreo (`sender-stats`, `top-destinatarios`, `serie-temporal`, `dominios-receptores`, `candidatos-blacklist`) cachean su resultado en memoria via `IMemoryCache` con TTL configurable por método (defaults: 5min sender-stats, 5min top-destinatarios, 60s serie-temporal hora / 10min día, 10min dominios, 30s candidatos). Caps defensivos en input: `ventanaDias <= 30`, `limit <= 50`, `umbralHits >= 1`. Fail-safe (INV-S07) — si la query falla, retorna lista vacía sin romper el dashboard. La caché de `defer-fail-status` (Plan 29 Chat 2.6) se invalida ANTES de pushear `DeferFailStatusUpdated` por SignalR (alineado con INV-MAIL04). El default de `umbralHits` y `ventanaHoras` del endpoint `candidatos-blacklist` se inyecta desde `EmailSettings.MailboxFullThresholdHits` / `MailboxFullThresholdHours` (alineado con INV-MAIL07) — los query params solo overridean para inspección manual. | `EmailMonitoreoService.GetXAsync` con `IMemoryCache.GetOrCreateAsync` por endpoint + `SignalREmailHubNotifier.NotifyDeferFailStatusUpdatedAsync` invalida `_cache.Remove("defer-fail-status:dominio")` antes del push (Plan 39 Chat A 077 + Chat B 078) | 18.2 |
| `INV-MAIL09` | EmailOutboxService.EnqueueAsync | `EnqueueAsync` rechaza al destinatario en 4 capas evaluadas en orden: (1) **formato** (`EmailValidator.Validate`) — INV-MAIL01; (2) **blacklist permanente** (`IEmailBlacklistService.IsBlacklistedAsync`) — INV-MAIL01/02/07; (3) **cuarentena temporal** (`IEmailQuarantineGate.IsActiveAsync` con `EQU_Estado=1 AND EQU_RetryAfter > now`) — Plan 37 Chat 2; (4) **pause por dominio receptor** (`IEmailDomainPauseGate.IsActiveAsync` con `ERP_Estado=1 AND ERP_PausedUntil > now`) — Plan 37 Chat 2. Cualquier rechazo en las capas 1-4 produce `EnqueueOutcome.RejectedFormat` / `RejectedBlacklist` / `RejectedQuarantine` / `RejectedDomainPause` y NO crea fila en `EmailOutbox`, por lo que **nunca toca el MTA del hosting** y **no consume el contador** `INV-MAIL03` (5 defers+fails/h por dominio). El método legacy `EnqueueAsync` retornando `Guid?` se preserva como compat layer y delega a `EnqueueWithResultAsync`. La capa 3 promueve a blacklist permanente al 3er ingreso a cuarentena del mismo destinatario (`EQU_QuarantineCount >= 3`); la capa 4 está capada a 5 dominios pausados simultáneos para no apagar el universo Gmail/Outlook ante falsos positivos. | `EmailOutboxService.EnqueueWithResultAsync` con `IEmailQuarantineGate` + `IEmailDomainPauseGate` (Plan 37 Chat 2) | 18.2 |
| `INV-MAIL10` | EmailOutboxWorker | **Cesión automática del worker al contador del dominio**. Cuando `EmailOutboxWorker` consulta `/defer-fail-status` (endpoint del Plan 29 Chat 2.6) y el `usage_pct` reportado por el hosting está `≥ EmailSettings.DeferFailCessionThresholdPercentage` (default `80`), el worker **pausa el procesamiento** de filas con `EO_EntidadOrigen ∈ {"Asistencia", "AsistenciaProfesor"}` (correos informativos al apoderado). Las filas pausadas **no se marcan FAILED** — quedan en estado `PENDING` con `EO_UltimoError = "[DEFER_FAIL_CESSION] contador hosting al X%, esperando ventana"` y se reintentan en el próximo tick del worker cuando el contador baje del threshold. Las filas con `EO_EntidadOrigen ∈ {"AsistenciaCorreccion", "Justificacion", "ReporteFallosCorreoAsistencia", "RESET_PASSWORD", "REPORTE_USUARIO"}` (críticos administrativos / operativos) **se siguen procesando normalmente** incluso con contador caliente — son los correos donde el costo de la latencia supera el costo del bloqueo eventual. **Razón**: ante restricción explícita del usuario (CrossChex se queda + hosting denegó subir techo + sin aprobación para SMTP relay externo o subdominio dedicado), el contador 5/h del dominio es compartido con CrossChex y Educa no puede asegurar margen permanente. La cesión automática transforma a Educa en un consumidor "buen ciudadano" del contador: cuando CrossChex u otro factor satura el techo, Educa deja de sumar correos informativos hasta que la ventana se libere. Esto **no reduce volumen total** (objetivo descartado del Plan 44 — el contador mira FAILED, no SENT, y Educa ya tiene 0.27% FAILED rate por INV-MAIL01/02/07/09), pero **reduce probabilidad de que Educa sea el que tumbe al dominio en bursts compartidos**. Sin UI, sin opt-in del apoderado, sin ack del jefe — palanca puramente defensiva en backend. Configurable en caliente vía `EmailSettings:DeferFailCessionThresholdPercentage` para subir/bajar el umbral según telemetría real post-deploy (recomendado iniciar en 80, observar 30 días, ajustar). Fail-safe (INV-S07): si la consulta a `/defer-fail-status` falla, el worker procede como si el contador estuviera en 0% — la cesión es **best-effort**, nunca bloquea correos cuando la telemetría no responde. | `EmailOutboxWorker.ProcessSingleEmailAsync` con guard nuevo al inicio del loop (Plan 22 Chat 7 — brief 146) | 18.2 |
...
**Plan 28 Chat 3a (2026-05-07)** extendió los 14 endpoints PDF/Excel a `tipoPersona='A'` (Asistente Administrativo). La paridad estructural se mantiene — ambos formatos consumen el mismo data service (`ReporteFiltradoAsistenciaService.AsistentesAdmin.cs`), las mismas columnas, sin nuevos endpoints. Badge textual *"Asistente Administrativo"* en la sección dedicada de cada reporte distingue las filas AA de las de Profesor. INV-RE01/02/03 se cumplen sin cambios — la regla universal cubre la extensión.
...
| 2a — Bounce permanente 5.x.x | `EmailOutboxWorker` → `EmailBounceBlacklistHandler` dentro del `ProcessSingleEmailAsync.catch` (también invocado desde `BounceParserService` IMAP) | `INV-MAIL02` | Que un destinatario con ≥3 bounces `5.x.x` permanentes se siga intentando. Al tercer `5.x.x`, se inserta en `EmailBlacklist` (`MotivoBloqueo = 'BOUNCE_5XX'`) en la misma transacción que pone `EO_Estado = 'FAILED_BLACKLISTED'`. A partir de ahí, INV-MAIL01 se encarga de bloquear futuros envíos. |
| 2b — Mailbox full crónico (4.2.2) | `EmailOutboxWorker` + `BounceParserService` → `MailboxFullBlacklistHandler` (Plan 38 Chat 2) | `INV-MAIL07` | Que un destinatario con buzón lleno crónico siga generando reintentos del Exim local cada 15-30 min hasta agotar la política (4d), cada uno consumiendo el contador `INV-MAIL03`. Al 2º hit `FAILED_MAILBOX_FULL` en 24h, se inserta en `EmailBlacklist` (`MotivoBloqueo = 'BOUNCE_MAILBOX_FULL'`). Cleanup automático tras 7d sin actividad (`INV-MAIL06`). |
| 3 — Monitoreo | `GET /api/sistema/email-outbox/defer-fail-status` (Plan 29 Chat 2.6) + `DeferFailStatusWidget` FE (Plan 22 Chat B) + `EmailHub` SignalR push (Plan 38 Chat 6) | `INV-MAIL04` | Que el dominio se acerque al techo sin que el admin lo vea. Polling 60s con semáforo OK (<60%) / WARNING (60-100%) / CRITICAL (≥100%) + push instantáneo cuando el contador o el handler 4.2.2 disparan. Fallback CRITICAL en `try/catch` global del service (INV-S07). |
...
- **`INV-MAIL02`** — Auto-blacklist por 3+ bounces `5.x.x` permanentes (`FailedInvalidAddress` + `FailedRejected`) lifetime. `FailedMailboxFull` queda fuera (Plan 38 — ver INV-MAIL07). Ver §15.14.
- **`INV-MAIL03`** — Techo cPanel <!-- TBD post-OPS: Chat 3 puede subir a 25-30/h --> 5 defers+fails/h por dominio, no configurable. Defensa solo vía INV-MAIL01/02/07. Ver §15.14.
- **`INV-MAIL04`** — Monitoreo vía endpoint `defer-fail-status` + widget admin 60s polling + push SignalR (Plan 38 Chat 6), fallback CRITICAL por INV-S07. Ver §15.14.
- **`INV-MAIL06`** — Auto-cleanup 7d para entradas `BOUNCE_MAILBOX_FULL` sin actividad (Plan 38 Chat 4). Otros motivos requieren DELETE explícito. Ver §15.14.
- **`INV-MAIL07`** — Auto-blacklist `BOUNCE_MAILBOX_FULL` por 2+ hits `FailedMailboxFull` en ventana 24h, sync o async-imap (Plan 38 Chat 2). Razón: cortar reintentos del Exim local antes de agotar `INV-MAIL03`. Ver §15.14.
- **`INV-MAIL08`** — Caché en memoria por método del dashboard de monitoreo con TTL diferenciado por volatilidad (Plan 39 Chat A). Caps defensivos en input. Fail-safe INV-S07. Invalida caché del `defer-fail-status` antes de pushear `DeferFailStatusUpdated` (alineado con INV-MAIL04). Defaults de `candidatos-blacklist` inyectados desde `EmailSettings.MailboxFullThreshold*` (alineado con INV-MAIL07). Ver §15.14.
- **`INV-MAIL09`** — `EnqueueAsync` rechaza en 4 capas en orden (formato → blacklist → quarantine → recipient-domain pause). Cualquier rechazo no toca el MTA y no consume el contador `INV-MAIL03`. Plan 37 Chat 2 introduce las capas 3 y 4 — la cuarentena con auto-release y promoción a blacklist al 3er hit, y la pausa por dominio capada a 5 simultáneos. Ver §15.14.
- **`INV-MAIL10`** — Cesión automática del `EmailOutboxWorker` cuando el contador del dominio reportado por `/defer-fail-status` está `≥ EmailSettings.DeferFailCessionThresholdPercentage` (default 80). Pausa correos informativos al apoderado (`Asistencia`, `AsistenciaProfesor`) sin marcarlos FAILED y mantiene críticos administrativos. Configurable en caliente. Fail-safe ante telemetría caída. Absorbe scope mínimo viable del Plan 44 descartado 2026-05-12 — no reduce volumen total (innecesario por 0.27% FAILED rate actual con INV-MAIL01/02/07/09) pero reduce probabilidad de que Educa sea quien tumbe el dominio compartido con CrossChex en bursts del contador 5/h. Ver §15.14.
