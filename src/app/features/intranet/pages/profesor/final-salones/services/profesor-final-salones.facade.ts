...
export class ProfesorFinalSalonesFacade {
	// #region Dependencias
	private api = inject(ProfesorFinalSalonesApiService);
	private store = inject(ProfesorFinalSalonesStore);
	...
	loadAll(): void {
		...
		forkJoin({
			...
			.pipe(
				withRetry({ tag: 'ProfesorFinalSalonesFacade:loadAll' }),
	...
	refreshSalones(): void {
		...
		this.api
			...
			.pipe(
				withRetry({ tag: 'ProfesorFinalSalonesFacade:refreshSalones' }),
				takeUntilDestroyed(this.destroyRef),
			)
			.subscribe({
				next: (salones) => {
				...
				next: (resultado) => {
					if (resultado) {
						this.errorHandler.showSuccess(
							...
							`${resultado.aprobados} aprobados, ${resultado.desaprobados} desaprobados, ${resultado.errores} errores`,
							5000,
						);
