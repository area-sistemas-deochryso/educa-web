...
export type UserRole = 'Estudiante' | 'Apoderado' | 'Profesor' | ''
...
export class AttendanceFacade {
	...
	// Usuario
	readonly userRole = signal<UserRole>('')
	...
	// Para profesor
	readonly nombreProfesor = signal<string | null>(null)
	...
	initialize(destroyRef: DestroyRef): void {
		...
		if (user) {
			this.userRole.set(user.rol as UserRole)
			this.studentName.set(user.nombreCompleto)
			this.nombreProfesor.set(this.storage.getUser()?.nombreCompleto ?? null)
