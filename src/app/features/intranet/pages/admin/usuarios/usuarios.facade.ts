...
import { DebugService, logger, withRetry } from '@core/helpers';
...
export class UsuariosFacade {
	...
	loadData(): void {
		...
		forkJoin({
			estadisticas: this.usuariosService.obtenerEstadisticas().pipe(
				withRetry({ tag: 'UsuariosFacade:loadEstadisticas' }),
				catchError((err) => {
			...
			salones: this.asistenciaService.getSalonesDirector().pipe(
				withRetry({ tag: 'UsuariosFacade:loadSalones' }),
				catchError((err) => {
	...
	openDetail(usuario: UsuarioLista): void {
		this.usuariosService
			...
			.pipe(
				withRetry({ tag: 'UsuariosFacade:openDetail' }),
				takeUntilDestroyed(this.destroyRef),
			)
			.subscribe({
				next: (detalle) => {
	...
	editUsuario(usuario: UsuarioLista): void {
		this.usuariosService
			.obtenerUsuario(usuario.rol, usuario.id)
			.pipe(
				withRetry({ tag: 'UsuariosFacade:editUsuario' }),
				takeUntilDestroyed(this.destroyRef),
			)
			.subscribe({
				next: (detalle) => {
	...
	/**
	 * Construye el payload para crear usuario (objeto puro, no Observable).
	...
	private buildCreatePayload(
		...
		if (!data.rol || !data.contrasena) {
			this.log.warn('buildCreatePayload - falta rol o contraseña', {
				rol: data.rol,
	...
	/**
	 * Construye el payload para actualizar usuario (objeto puro, no Observable).
	...
	/** Actualizar estadística por rol (DRY helper) */
