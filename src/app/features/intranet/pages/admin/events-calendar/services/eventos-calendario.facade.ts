...
export class EventsCalendarFacade {
	...
	delete(item: EventoCalendarioLista): void {
		this.wal.execute({
			...
			optimistic: {
				apply: () => {
					// BE hace soft delete (`EVT_Estado=false`): item queda en lista como inactivo.
					// `total` no cambia.
					const { activosDelta, inactivosDelta } = getEstadoToggleDeltas(item.estado, 'delete-soft');
					this.store.updateItem(item.id, { estado: false } as Partial<EventoCalendarioLista>);
					this.store.incrementarEstadistica('activos', activosDelta);
					this.store.incrementarEstadistica('inactivos', inactivosDelta);
				},
				rollback: () => {
					const { activosDelta, inactivosDelta } = getEstadoRollbackDeltas(item.estado, 'delete-soft');
					this.store.updateItem(item.id, { estado: item.estado } as Partial<EventoCalendarioLista>);
					this.store.incrementarEstadistica('activos', activosDelta);
					this.store.incrementarEstadistica('inactivos', inactivosDelta);
				},
			},
