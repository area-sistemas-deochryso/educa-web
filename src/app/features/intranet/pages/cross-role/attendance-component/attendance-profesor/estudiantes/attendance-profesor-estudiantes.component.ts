 ...
 * exponga un tabView "Mi asistencia" / "Mis estudiantes".
 *
...
export class AttendanceProfesorEstudiantesComponent implements OnInit {
	 ...
	 * El shell usa esto para decidir si mostrar el tab "Mis estudiantes".
	 */
	readonly salonesLoaded = output<SalonProfesor[]>();
		...
		forkJoin({
			...
			.subscribe({
				next: ({ tutoria, horario }) => {
					...
					this.salonesLoaded.emit(merged);
	...
	// #endregion
	// #region Delegados al shell (llamados vía @ViewChild)

	// * Shell component calls these via ViewChild.
