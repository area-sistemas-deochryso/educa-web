...
// #endregion
// #region Helpers internos — Bloques comunes

function pushImports(lines: string[], hasAuth: boolean): void {
...
function pushBaseUrl(lines: string[], baseUrl: string): void {
	lines.push(`const BASE_URL = '${baseUrl}';`);
	lines.push('');
}

function pushOptionsBlock(lines: string[], config: K6TestConfig): void {
...
function pushCredentialsAndSetup(
	lines: string[],
	credentials: K6Credential[],
	roleDistribution: K6RoleDistribution[],
): void {
...
function pushParamsHelper(lines: string[], hasAuth: boolean): void {
	if (hasAuth) {
...
// #endregion
// #region Helpers internos — Endpoints

/**
 * Detecta el rol requerido por un endpoint según su path y nombre.
 * Convención API: /api/Estudiante/*, /api/.../estudiante/..., etc.
 */
...
/**
 * Genera las llamadas a endpoints, agrupando por rol
 * para que cada VU solo ejecute endpoints de su propio rol.
 */
function pushEndpointCalls(
	lines: string[],
	endpoints: K6Endpoint[],
	config: K6TestConfig,
	indent: string,
...
// #endregion
// #region Helpers internos — Autenticación y roles

function pushSetupFunction(lines: string[]): void {
...
function pushRoleArraysAndRanges(lines: string[], roleDistribution: K6RoleDistribution[]): void {
...
function pushUserAssignment(lines: string[], roleDistribution: K6RoleDistribution[], indent: string): void {
	if (roleDistribution.length > 0) {
...
// #endregion
// #region Helpers internos — Utilidades
...
function buildScenariosMeta(stages: K6Stage[]): ScenarioMeta[] {
