...
export class AttendanceReportsFacade {
	...
	async exportarExcel(): Promise<void> {
		...
		try {
			...
			if (!WorkbookClass) {
				logger.error('[AttendanceReports] ExcelJS no disponible');
				return;
			}
	...
	private escribirLeyenda(sheet: any, startRow: number): number {
		...
		for (let i = 0; i < AttendanceReportsFacade.NIVELES_LEYENDA.length; i++) {
			const nivel = AttendanceReportsFacade.NIVELES_LEYENDA[i];
